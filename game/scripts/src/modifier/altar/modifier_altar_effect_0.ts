...
export class ModifierAltarEffect extends BaseModifier {
    ...
    OnCreated(params: any): void {
        ...
        const effect_fx = ParticleManager.CreateParticle('particles/custom/altar/altar.vpcf', ParticleAttachment.ABSORIGIN_FOLLOW, this.GetParent());
        ParticleManager.SetParticleControl(effect_fx, 2, this.color);
        this.AddParticle(effect_fx, false, false, -1, false, false);
        this.effect_fx = effect_fx;

        const icon_effect = ParticleManager.CreateParticle(
            'particles/title_fx/title00028/title00028.vpcf',
            ParticleAttachment.OVERHEAD_FOLLOW,
            this.GetParent()
        );
        ParticleManager.SetParticleControl(icon_effect, 1, Vector(this.icon_index, 0, 0));
        this.AddParticle(icon_effect, false, false, -1, false, false);
...
export class modifier_altar_effect_1 extends ModifierAltarEffect {
    ...
    OnDeath(event: ModifierInstanceEvent): void {
        ...
        const health = event.unit.GetMaxHealth();
        GameRules.BasicRules.Heal(event.attacker, health);
    }
}

// 2 神速圣坛[]	持续15秒，玩家英雄移动速度提高100%，且免疫减速效果。
@registerModifier()
