...
export class modifier_talent_effect_skywrath extends modifier_talent_effect {
    ...
    OnIntervalThink(): void {
        ...
        if (this.object['99']) {
            const hp_heighest = this.GetObject('99', 'hp_heighest');
            const find_dmg = this.GetObject('99', 'final_dmg');
            const final_reduction = this.GetObject('99', 'final_reduction');
        ...
        if (this.object['101']) {
            const mana_low = this.GetObject('101', 'mana_low');
            if (caster_mp_pct < mana_low) {
                const res_mana = this.GetObject('101', 'res_mana');
        ...
        if (this.object['102']) {
            this.timer_zhiyu += 1;
            const interval = this.GetObject('102', 'interval');
            if (this.timer_zhiyu >= interval) {
                this.timer_zhiyu = 0;
                const heal_maxhp_pct = this.GetObject('102', 'heal_maxhp_pct');
    ...
    OnCriticalStrike(hTarget: CDOTA_BaseNPC) {
        ...
        if (this.object['112']) {
            const value = this.GetObject('112', 'value');
            const duration = this.GetObject('112', 'duration');
