...
import type * as TalentTreeConfig from '../../json/config/game/hero/talent_tree/talent_tree_config.json';
...
export class modifier_talent_effect extends BaseModifier {
    ...
    _OnCreated() {
        const heroName = this.caster.GetUnitName();
        const hero_data = HeroCustomJson[heroName as 'npc_dota_hero_drow_ranger'];
        if (hero_data && hero_data.talent_mdf_name) {
            this.link_mdf = this.caster.AddNewModifier(this.caster, this.GetAbility(), hero_data.talent_mdf_name, {}) as modifier_talent_effect;
        }
    }
    ...
    GetObject<Key extends keyof typeof TalentTreeConfig, T2 extends (typeof TalentTreeConfig)[Key]>(name: Key, key: keyof T2['AbilityValues']) {
