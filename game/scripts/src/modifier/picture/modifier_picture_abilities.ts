...
export class modifier_picture_abilities extends BaseModifier {
    ...
    UpdateMdf() {
        ...
        if (this.object["1"] != null) {
            ...
            let value = this.Get_Object("1", "value");
        ...
        if (this.object["11"] != null) {
            let value = this.Get_Object("11", "value");
            this.caster.SetAttributeInKey("picture_1", {
                "PickItemRadius": {
                    "Fixed": value
                }
        ...
        if (this.object["15"] != null) {
            let radius = this.Get_Object("15", "radius")
            let ms_pct = this.Get_Object("15", "ms_pct")
            let hp_regen = this.Get_Object("15", "hp_regen")
    ...
    OnIntervalThink(): void {
        ...
        if (this.object["4"] != null) {
            if (this.caster.GetManaPercent() >= 100) {
                let mana_amount = this.caster.GetHealthRegen() * this.Get_Object("4", "value") * 0.01;
        ...
        if (this.object["19"] != null) {
            ...
            if (this.no19_timer >= this.Get_Object("19", "interval")) {
                this.no19_timer = 0;
                let heal = this.Get_Object("19", "value") * this.caster.GetMaxHealth() * 0.01;
    ...
    OnKillEvent(hTarget: CDOTA_BaseNPC) {
        ...
        if (this.object["5"] != null) {
            let chance = this.Get_Object("5", "chance");
            if (RollPercentage(chance)) {
                let speed_pct = this.Get_Object("5", "speed_pct");
                let duration = this.Get_Object("5", "duration");
                let max_stack = this.Get_Object("5", "stack");
    ...
    OnBeInjured(params: ApplyCustomDamageOptions): boolean {
        ...
        if (this.object["6"] != null && params.damage >= this.caster.GetHealth()) {
            let picture_6_cd = (this.caster.CustomVariables["picture_6"] ?? 0)
            let dota_time = GameRules.GetDOTATime(false, false)
            if (picture_6_cd < dota_time) {
                let heal = this.Get_Object("6", "max_heal_pct");
                let cd = this.Get_Object("6", "cd")
        ...
        if (this.object["8"] != null) {
            let duration = this.Get_Object("8", "duration");
            let max_heal_pct = this.Get_Object("8", "max_heal_pct");
        ...
        if (this.object["16"] != null) {
            let chance = this.Get_Object("16", "chance");
            if (RollPercentage(chance)) {
                let ms_base_pct = this.Get_Object("16", "ms_base_pct");
                let duration = this.Get_Object("16", "duration");
    ...
    _ExecutedBaseAbility(params?: any) {
        if (this.object["7"] != null) {
            ...
            let chance = this.Get_Object("7", "chance");
            if (RollPercentage(chance)) {
                let attackspeed_pct = this.Get_Object("7", "attackspeed_pct");
                let duration = this.Get_Object("7", "duration");
    ...
    _CritEvent(params: ApplyCustomDamageOptions) {
        ...
        if (this.object["17"] != null) {
            let health = this.Get_Object("17", "value") * 0.01 * params.damage;
    ...
    _OnTakeDamage(params: ApplyCustomDamageOptions) {
        ...
        if (this.object["20"] != null) {
            let picture_20_cd = (this.caster.CustomVariables["picture_20_cd"] ?? 0);
            let chance = this.Get_Object("20", "chance");
            if (picture_20_cd < dotatime && RollPercentage(chance)) {
                let duration = this.Get_Object("20", "duration");
                let cd = this.Get_Object("20", "cd") + duration;
        ...
        if (this.object["23"] != null && params.element_type == ElementTypes.FIRE) {
            let picture_cd = (this.caster.CustomVariables["picture_23_cd"] ?? 0);
            if (picture_cd < dotatime) {
                let cd = this.Get_Object("23", "cd");
                this.caster.CustomVariables["picture_23_cd"] = dotatime + cd;

                let value1 = this.Get_Object("23", "value1");
                let value2 = this.Get_Object("23", "value2");
                let duration = this.Get_Object("23", "duration");

                this.caster.SetAttributeInKey("picture_23", {
                    "FirePent": {
                        "Base": value1,
                    },
                    "FireDamageBonus": {
        ...
        if (this.object["24"] != null && params.element_type == ElementTypes.ICE) {
            let picture_cd = (this.caster.CustomVariables["picture_24_cd"] ?? 0);
            if (picture_cd < dotatime) {
                let cd = this.Get_Object("24", "cd");
                this.caster.CustomVariables["picture_24_cd"] = dotatime + cd;

                let value1 = this.Get_Object("24", "value1");
                let value2 = this.Get_Object("24", "value2");
                let duration = this.Get_Object("24", "duration");

                this.caster.SetAttributeInKey("picture_24", {
                    "IcePent": {
                        "Base": value1,
                    },
                    "IceDamageBonus": {
        ...
        if (this.object["25"] != null && params.element_type == ElementTypes.THUNDER) {
            let picture_cd = (this.caster.CustomVariables["picture_25_cd"] ?? 0);
            if (picture_cd < dotatime) {
                let cd = this.Get_Object("25", "cd");
                this.caster.CustomVariables["picture_25_cd"] = dotatime + cd;

                let value1 = this.Get_Object("25", "value1");
                let value2 = this.Get_Object("25", "value2");
                let duration = this.Get_Object("25", "duration");

                this.caster.SetAttributeInKey("picture_25", {
                    "ThunderPent": {
                        "Base": value1,
                    },
                    "ThunderDamageBonus": {
        ...
        if (this.object["26"] != null && params.element_type == ElementTypes.WIND) {
            let picture_cd = (this.caster.CustomVariables["picture_26_cd"] ?? 0);
            if (picture_cd < dotatime) {
                let cd = this.Get_Object("26", "cd");
                this.caster.CustomVariables["picture_26_cd"] = dotatime + cd;

                let value1 = this.Get_Object("26", "value1");
                let value2 = this.Get_Object("26", "value2");
                let duration = this.Get_Object("26", "duration");

                this.caster.SetAttributeInKey("picture_26", {
                    "WindPent": {
                        "Base": value1,
                    },
                    "WindDamageBonus": {
    ...
    _OnDeath(killer?: CDOTA_BaseNPC) {
        ...
        if (this.object["9"] != null) {
            ...
            let radius = this.Get_Object("9", "value");
            ...
            for (let enemy of enemies) {
                if (enemy.IsBossCreature()) { continue }
        ...
        if (this.object["10"] != null) {
            this.no10_count += 1;
            let value = this.Get_Object("10", "value") * this.no10_count;
