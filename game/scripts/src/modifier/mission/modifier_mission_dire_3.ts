...
export class modifier_mission_dire_3_thinker extends BaseModifier {
    ...
    OnIntervalThink(): void {
        for (const hHero of HeroList.GetAllHeroes()) {
            if (hHero.IsAlive()) {
                const sun_vect = (hHero.GetAbsOrigin() + Vector(RandomInt(-250, 250), RandomInt(-250, 250), 0)) as Vector;
...
export class modifier_mission_dire_3_sun_strike extends BaseModifier {
    ...
    OnCreated(params: any): void {
        ...
        const warning_fx = GameRules.WarningMarker.Circular(this.sun_radius, delay, this.GetParent().GetAbsOrigin(), false, Vector(255, 0, 0));
    ...
    OnDestroy(): void {
        ...
        const vParent = hParent.GetAbsOrigin();
