...
export class modifier_mission_dire_2 extends BaseModifier {
    ...
    GetModifierAura() {
        return 'modifier_mission_dire_2_aura';
    }
...
export class modifier_mission_dire_2_target extends BaseModifier {
    ...
    OnRefresh(params: any): void {
        ...
        this.effect_fx = ParticleManager.CreateParticle('particles/diy_particles/move.vpcf', ParticleAttachment.POINT_FOLLOW, this.GetParent());
        const target = params.target as EntityIndex;
        const hTarget = EntIndexToHScript(target);
        ParticleManager.SetParticleControlEnt(this.effect_fx, 1, hTarget, ParticleAttachment.POINT_FOLLOW, 'attach_hitloc', Vector(0, 0, 0), false);
