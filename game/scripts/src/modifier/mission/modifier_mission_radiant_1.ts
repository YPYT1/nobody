...
export class modifier_mission_radiant_1_football extends BaseModifier {
    ...
    OnCreated(params: any): void {
        ...
        // 添加终点辅助线
        const line_pfx = ParticleManager.CreateParticle('particles/diy_particles/move.vpcf', ParticleAttachment.POINT_FOLLOW, this.hParent);
    ...
    OnIntervalThink(): void {
        ...
        // 检查是否到达终点
        const distance = ((this.goal_vect - vect) as Vector).Length2D();
        ...
        if (this.hParent.HasModifier('modifier_knockback_lua')) {
            return;
        }

        const hHeroes = FindUnitsInRadius(
            DotaTeam.GOODGUYS,
            vect,
            null,
            100,
        ...
        if (hHeroes.length > 0) {
            const vOrigin = hHeroes[0].GetAbsOrigin();
            const vTarget = this.hParent.GetAbsOrigin();
            const direction = ((vTarget - vOrigin) as Vector).Normalized();
