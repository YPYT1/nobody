...
export class modifier_mission_radiant_8_ai extends BaseModifier {
    ...
    OnIntervalThink(): void {
        const next_point = GetRandomMoveVect(this.GetParent().GetOrigin(), RandomInt(400, 700));
    ...
    DeclareFunctions(): modifierfunction[] {
        return [ModifierFunction.MOVESPEED_ABSOLUTE_MIN, ModifierFunction.INCOMING_DAMAGE_PERCENTAGE];
    }
...
export class modifier_mission_radiant_8_speed extends BaseModifier {
    ...
    OnCreated(params: object): void {
        ...
        const next_point = GetRandomMoveVect(this.GetParent().GetOrigin(), RandomInt(300, 500));
        ExecuteOrderFromTable({
            UnitIndex: this.GetParent().entindex(),
            OrderType: UnitOrder.MOVE_TO_POSITION,
            Position: next_point,
