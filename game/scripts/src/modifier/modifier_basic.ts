...
export class modifier_basic_move extends BaseModifier {
    ...
    OnRefresh(params: any): void {
        ...
        if (params.UP) {
            this.move_up = params.UP == 1;
        }
        if (params.DOWN) {
            this.move_down = params.DOWN == 1;
        }
        if (params.LEFT) {
            this.move_left = params.LEFT == 1;
        }
        if (params.RIGHT) {
            this.move_right = params.RIGHT == 1;
        }
        if (params.SPACE == 1) {
        ...
        if (this.parent.HasModifier('modifier_debuff_chaos')) {
        ...
        const old_vect = this.parent.GetAbsOrigin();
        const origin = this.parent.GetAbsOrigin();
        if (this.move_up) {
            origin.y += this.move_distance;
        }
        if (this.move_down) {
            origin.y -= this.move_distance;
        }
        if (this.move_left) {
            origin.x -= this.move_distance;
        }
        if (this.move_right) {
            origin.x += this.move_distance;
        }
    ...
    OnIntervalThink(): void {
        if (this.parent.HasModifier('modifier_debuff_uncontroll')) {
            return;
        }
        if (this.parent.IsAlive() == false) {
            this.move_up = false;
        ...
        if (this.parent.clone_unit != null && this.parent.clone_unit.HasModifier('modifier_skywrath_5_clone_show')) {
            const clone_origin = this.parent.clone_unit.GetOrigin();
            if (this.move_up) {
                clone_origin.y += this.move_distance;
            }
            if (this.move_down) {
                clone_origin.y -= this.move_distance;
            }
            if (this.move_left) {
                clone_origin.x -= this.move_distance;
            }
            if (this.move_right) {
                clone_origin.x += this.move_distance;
            }
    ...
    OnMoveStateChange(state: boolean) {
        ...
        if (this.parent.prop_count['prop_30']) {
            if (state) {
                ...
                // 停止移动
                const ability_cd_pct = GameRules.MysticalShopSystem.GetKvOfUnit(this.parent, 'prop_30', 'ability_haste_pct');
                const cooldown_limit = GameRules.MysticalShopSystem.GetKvOfUnit(this.parent, 'prop_30', 'cooldown_limit');
                GameRules.CustomAttribute.SetAttributeInKey(this.parent, 'prop_30', {
                    AbilityCooldown: {
...
export class modifier_basic_countdown extends BaseModifier {
    ...
    OnIntervalThink(): void {
        const countdown = math.floor(this.timer);
        const cp1_x = math.floor(countdown / 10);
        const cp1_y = math.floor(countdown % 10);
