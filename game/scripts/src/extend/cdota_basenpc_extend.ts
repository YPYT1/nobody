/** @noSelf */
import type { HeroTalentObject } from '../kv_data/hero_talent_object';
import type * as RuneConfig from '../json/config/game/rune/rune_config.json';

declare global {
    interface CDOTA_BaseNPC {
        GetTalentKv<Key extends keyof typeof HeroTalentObject, T1 extends keyof (typeof HeroTalentObject)[Key]['AbilityValues']>(
            index_key: Key,
            ability_key: T1
        ): number;

        GetRuneKv<Key extends keyof typeof RuneConfig, T1 extends keyof (typeof RuneConfig)[Key]['AbilityValues']>(
            index_key: Key,
            ability_key: T1
        ): number;
