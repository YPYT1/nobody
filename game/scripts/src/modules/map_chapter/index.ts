...
export class MapChapter extends UIEventRegisterClass {
    ...
    InitChapterMap() {
        ...
        const sort_hero: { [key: number]: number } = {};
        for (const [key, RowData] of pairs(NpcHeroesCustom)) {
            if (RowData.Enable == 1) {
        ...
        for (let index = 0; index < this.player_count; index++) {
            const hero_key: MapSelectHeroData[] = [];
    ...
    DifficultySelectInit(str: string = '') {
        ...
        if (str != '') {
            ...
            for (let index = 0; index < str_list.length; index++) {
                const mid = str_list[index];
                const mid_number = tonumber(mid);
                print('mid ', mid);
                const MidData = MapInfoDifficulty[mid as keyof typeof MapInfoDifficulty];
                const unlock_difficulty_list = MidData.unlock_difficulty;
                const chapter_key = MidData.chapter_key;
                const map_key = MidData.map_key;
                //当前难度
                if (!this._map_list.hasOwnProperty(chapter_key)) {
                    const default_difficulty = ChapterInfo[chapter_key as keyof typeof ChapterInfo].default_difficulty;
                    const default_max = ChapterInfo[chapter_key as keyof typeof ChapterInfo].default_max;
                    const difficulty_max = default_difficulty + default_max - 1;
                ...
                for (const unlock_difficulty of unlock_difficulty_list) {
                    if (unlock_difficulty != 'null') {
                        const unlock_difficulty_str = tostring(unlock_difficulty);
                        const unlock_difficulty_num = tonumber(unlock_difficulty);
                        const UnlockMidData = MapInfoDifficulty[unlock_difficulty_str as keyof typeof MapInfoDifficulty];
                        const UnlockChapterKey = UnlockMidData.chapter_key;
                        if (this._map_list.hasOwnProperty(UnlockChapterKey)) {
                            ...
                            const unlock_default_difficulty = ChapterInfo[UnlockChapterKey as keyof typeof ChapterInfo].default_difficulty;
                            const unlock_default_max = ChapterInfo[UnlockChapterKey as keyof typeof ChapterInfo].default_max;
                            const unlock_difficulty_max = unlock_default_difficulty + unlock_default_max - 1;
    ...
    OnCreatedCampMap() {
        if (this.CampMapHandle == null) {
            const vLocation = Vector(GameRules.MapChapter.MAP_CAMP.x, GameRules.MapChapter.MAP_CAMP.y, 0);
            for (const hHero of HeroList.GetAllHeroes()) {
                hHero.SetOrigin(vLocation);
            }
            this.CampMapHandle = DOTA_SpawnMapAtPosition('camp', vLocation, false, this.OnCampReadyToSpawn, this.OnSpawnCampComplete, this);
        }
    }
    ...
    GetNewPlayerStatus(player_id: PlayerID, params: CGED['MapChapter']['GetNewPlayerStatus']) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MapChapter_GetNewPlayerStatus', {
            data: {
    ...
    GetDifficultyMax(player_id: PlayerID, params: CGED['MapChapter']['GetDifficultyMax']) {
        ...
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MapChapter_GetDifficultyMax', {
            data: {
    ...
    SelectDifficulty(player_id: PlayerID, params: CGED['MapChapter']['SelectDifficulty']) {
        if (this._game_select_phase == 0 && player_id == 0 && params.difficulty != '-1') {
            ...
            const MapIndex = MapInfoDifficulty[this.GameDifficulty].map_key as keyof typeof MapInfo;
            ...
            if (!this.map_list_config.hasOwnProperty(MapIndex)) {
                const ky_count = Object.keys(this.map_list_config).length;
        ...
        CustomGameEventManager.Send_ServerToAllClients('MapChapter_SelectDifficulty', {
            data: {
    ...
    GetDifficulty(player_id: PlayerID, params: CGED['MapChapter']['GetDifficulty']) {
        //获取游戏难度
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('MapChapter_GetDifficulty', {
                data: {
                    select_map: this.MapIndex,
                    select_difficulty: this.GameDifficulty,
            ...
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MapChapter_GetDifficulty', {
                data: {
                    select_map: this.MapIndex,
                    select_difficulty: this.GameDifficulty,
                    time: this.countdown_select_map_time,
    ...
    SelectDifficultyAffirm(player_id: PlayerID, params: CGED['MapChapter']['SelectDifficultyAffirm']) {
        if (this._game_select_phase == 0) {
            ...
            //发送选择英雄信息
            for (let index = 0 as PlayerID; index < GameRules.MapChapter.player_count; index++) {
                const steam_id = PlayerResource.GetSteamAccountID(index);
                //清空玩家选择状态
                if (steam_id == 0) {
                    this.player_select_hero[index].state = 1;
                } else {
    ...
    GetPlayerHeroList(player_id: PlayerID, params: CGED['MapChapter']['GetPlayerHeroList']) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MapChapter_GetPlayerHeroList', {
            data: {
    ...
    GetPlayerSelectHeroList(player_id: PlayerID, params: CGED['MapChapter']['GetPlayerSelectHeroList']) {
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('MapChapter_GetPlayerSelectHeroList', {
                data: {
                    hero_ids: this.player_select_hero,
                },
            });
        } else {
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MapChapter_GetPlayerSelectHeroList', {
                data: {
                    hero_ids: this.player_select_hero,
                },
            });
    ...
    GetGameSelectPhase(player_id: PlayerID, params: CGED['MapChapter']['GetGameSelectPhase']) {
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('MapChapter_GetGameSelectPhase', {
                data: {
                    game_select_phase: this._game_select_phase,
                },
            });
        } else {
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MapChapter_GetGameSelectPhase', {
                data: {
                    game_select_phase: this._game_select_phase,
                },
            });
    ...
    SelectHeroAffirm(player_id: PlayerID, params: CGED['MapChapter']['SelectHeroAffirm']) {
        ...
        if (this.map_list_config.hasOwnProperty(this.MapIndex)) {
            ...
            const vLocation = Vector(this.ChapterData.map_centre_x, this.ChapterData.map_centre_y, 0);
    ...
    GameStart() {
        ...
        for (let index = 0 as PlayerID; index < GameRules.MapChapter.player_count; index++) {
            ...
            hHero.SetOrigin(vLocation);
            const hname = GameRules.MapChapter.hero_list[this.player_select_hero[index].hero_id];
            PlayerResource.ReplaceHeroWith(index, hname, 0, 0);
            UTIL_Remove(hHero);
        }
    ...
    NewPlay(player_id: PlayerID, params: CGED['MapChapter']['NewPlay']) {
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('MapChapter_NewPlay', {
                data: {
            ...
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MapChapter_NewPlay', {
                data: {
                    count: this.game_count, //游戏次数
                    extend: {}, //扩展参数
                },
    ...
    OpenReopenVote(player_id: PlayerID, params: CGED['MapChapter']['OpenReopenVote']) {
        if (player_id == 0 && this._game_select_phase == 999) {
            if (this.is_reopen == false) {
                ...
                const plyaer_count = GetPlayerCount();
                ...
                GameRules.CMsg.SendCommonMsgToPlayer(-1, '主机开机投票', {});
            } else {
                GameRules.CMsg.SendErrorMsgToPlayer(player_id, '已经开启重开投票', {});
            }
        } else {
            GameRules.CMsg.SendErrorMsgToPlayer(player_id, '主机才能开启投票,或游戏未结束', {});
        }
    }
        ...
        if (this.is_reopen && this._game_select_phase == 999) {
            if (this.vote_data.playervote[player_id] == -1) {
                if (params.vote == 1) {
                    ...
                    GameRules.CMsg.SendCommonMsgToPlayer(-1, '{s:player_id} 玩家同意重开', {
                        player_id: player_id,
                    });
                    ...
                    this.vote_data.playervote[player_id] = params.vote;
                    GameRules.CMsg.SendCommonMsgToPlayer(-1, '{s:player_id} 玩家拒绝重开', {
                        player_id: player_id,
                    });
                    GameRules.MapChapter.VoteTimeLose();
                }
            } else {
                GameRules.CMsg.SendErrorMsgToPlayer(-1 as PlayerID, '你已投票....', {});
            }
        } else {
            GameRules.CMsg.SendErrorMsgToPlayer(-1 as PlayerID, '未开启投票,或游戏未结束', {});
        }
    }
    ...
    GetPlayerVoteData(player_id: PlayerID, params: CGED['MapChapter']['GetPlayerVoteData']) {
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('MapChapter_GetPlayerVoteData', {
                data: {
                    vote_data: this.vote_data,
                },
            });
        } else {
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MapChapter_GetPlayerVoteData', {
                data: {
                    vote_data: this.vote_data,
                },
            });
    ...
    ReturntoCamp() {
        ...
        if (GameRules.Spawn._game_start == false && this._game_select_phase == 999) {
            ...
            for (let index = 0 as PlayerID; index < GameRules.MapChapter.player_count; index++) {
                ...
                PlayerResource.ReplaceHeroWith(index, 'npc_dota_hero_wisp', 0, 0);
                UTIL_Remove(hHero);
            }
            //不移除地图
    ...
    GameWin() {
        ...
        for (let p_id = 0 as PlayerID; p_id < player_count; p_id++) {
            ...
            for (let i = 0; i < p_c; i++) {
                const c_p = GetCommonProbability(GameRules.Spawn._pass_card_id_pro);
                if (GameRules.Spawn._pass_card_id_list[c_p].length > 0) {
                    const c_i_l_i = RandomInt(0, GameRules.Spawn._pass_card_id_list[c_p].length - 1);
                    const item_id_str = GameRules.Spawn._pass_card_id_list[c_p][c_i_l_i];
        ...
        Timers.CreateTimer(3, () => {
            ...
            for (let index = 0 as PlayerID; index < player_count; index++) {
                ...
                const vLocation = Vector(
                    this.ChapterData.map_centre_x + this.TimersXYWIN[index].x,
                    this.ChapterData.map_centre_y + this.TimersXYWIN[index].y,
                    0
                );
        ...
        const exp_list: number[] = [];
        const cj_list: string[] = [];
        const hero_list: string[] = [];
        //通关结算
        GameRules.ArchiveService.GameOver(1, exp_list, cj_list, hero_list);
        GameRules.CMsg.SendCommonMsgToPlayer(-1 as PlayerID, '游戏胜利', {});
    ...
    GameLoser() {
        ...
        //通关结算
        GameRules.ArchiveService.GameOver(2, exp_list, cj_list, hero_list);
        //停止定时器
        GameRules.CMsg.SendCommonMsgToPlayer(-1 as PlayerID, '游戏失败', {});
    ...
    Debug(cmd: string, args: string[], player_id: PlayerID) {
        ...
        if (cmd == '-di') {
            const map_index = (args[0] ?? 'm1') as keyof typeof MapInfo;
            const difficulty = (args[0] ?? '101') as keyof typeof MapInfoDifficulty;
            this.OnLoadChapterMap(map_index, difficulty);
        }
        if (cmd == '-sh') {
            const hero_index = args[0] ?? '6';
