...
export class HeroAbilityType {
    ...
    InitTypeExp() {
        ...
        for (let i = 0; i < 6; i++) {
            let new_obj = {};
            for (let k in AbilityIdIndex) {
                let type_id = "" + AbilityIdIndex[k]
                new_obj[type_id] = 0
            }
    ...
    AddAbilityTypeExp(player_id: PlayerID, skv_type: { [key in CustomHeroAbilityTypes]: boolean }) {
        ...
        for (let k in skv_type) {
            let state = skv_type[k as keyof typeof skv_type];
            if (state) {
                let type_id = "" + AbilityIdIndex[k];
                // print("type_id",type_id);
                let curr_exp = this.skill_type_exp[player_id][type_id] ?? 0
