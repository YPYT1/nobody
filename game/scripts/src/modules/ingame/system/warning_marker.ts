...
export class WarningMarker {
     ...
     * @param color
     */
    Circular(radius: number, duration: number, vect: Vector, immediateRelease: boolean = false, color: Vector = Vector(255, 0, 0), type: number = 0) {
        ...
        const aoe_pfx2 = ParticleManager.CreateParticle(effect_name, ParticleAttachment.CUSTOMORIGIN, null);
    ...
    Line(
        ...
        const line_pfx = ParticleManager.CreateParticle('particles/diy_particles/range_finder_cone.vpcf', ParticleAttachment.POINT, caster);
        ...
        if (distance != -1) {
            ...
            const vPoint = (start_vect + direction * distance) as Vector;
     ...
     * @param end_vect
     */
    UpdateLine(line_pfx: ParticleID, start_vect: Vector, target_vect: Vector, distance: number = -1, color?: Vector) {
        if (distance == -1) {
    ...
    FixLine(caster: CDOTA_BaseNPC, line_radius: number, start_vect: Vector, target_vect: Vector, distance: number = -1, ver: LineWarningType = '1s') {
        const line_pfx = ParticleManager.CreateParticle(
    ...
    Sector(
        ...
        const end_radius = math.tan((angle * math.pi) / 360) * distance;
        // let end_radius2 = distance * 3.14 * (360 / angle)
        // print("end_radius",end_radius,end_radius2)
        // 因为特效问题,需要多偏移50的距离
        const vDir = (end_vect - start_vect) as Vector;
        const vTarget = (start_vect + vDir.Normalized() * (distance + 100)) as Vector;
        const warning_fx = ParticleManager.CreateParticle(
    ...
    CreateSectorV2(
        ...
        const line_pfx = ParticleManager.CreateParticle('particles/diy_particles/warning_sector.vpcf', ParticleAttachment.WORLDORIGIN, caster);
        ParticleManager.SetParticleControl(line_pfx, 0, caster.GetOrigin());
        ParticleManager.SetParticleControl(line_pfx, 1, caster.GetOrigin());
        ParticleManager.SetParticleControl(line_pfx, 3, Vector(end_radius, start_radius, 0));
        ParticleManager.SetParticleControl(line_pfx, 4, Vector(255, 255, 255));
        if (distance != -1) {
            const start_vect = caster.GetOrigin();
            let direction = (target_vect - start_vect) as Vector;
            direction.z = 0;
            direction = direction.Normalized();
            const vPoint = (start_vect + direction * distance) as Vector;
    ...
    DisplayRange(hParent: CDOTA_BaseNPC, fRadius: number, vColor: Vector = Vector(255, 0, 0)) {
        const effect_fx = ParticleManager.CreateParticle('particles/diy_particles/shop_range.vpcf', ParticleAttachment.ABSORIGIN_FOLLOW, hParent);
