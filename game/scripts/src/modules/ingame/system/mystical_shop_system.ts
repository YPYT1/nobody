...
export class MysticalShopSystem extends UIEventRegisterClass {
     ...
     * vip可用额外栏位
     */
    player_vip_cell_count: number = 3;
    /**
     * 满配上线
     */
    prop_ability_values: {
    ...
    constructor() {
        ...
        for (const key in MysteriousShopConfig) {
            ...
            for (let i_p = 0; i_p < this.player_count; i_p++) {
                ...
                for (let i_r = 0; i_r < 6; i_r++) {
                    //品质
                    this.item_level_probability_group[i_p][i_r].push(Info.probability[i_r]);
                }
            }
        }
        for (let index = 0; index < this.player_count; index++) {
            this.player_skill_activated.push([true, true, true, true, true]);
        ...
        //初始化
        for (const i_key in MysteriousShopConfig) {
            const data = MysteriousShopConfig[i_key as keyof typeof MysteriousShopConfig];
    ...
    InitPlayerUpgradeStatus(player_id: PlayerID) {
        ...
        this.player_skill_activated[player_id] = [true, true, true, true, true];
        ...
        const cell_count = this.player_vip_cell_count + this.cell_max;
        for (let cell_max_i = 1; cell_max_i <= cell_count; cell_max_i++) {
            ...
            } else if (cell_max_i > this.cell_max) {
                //vip锁
                this.player_shop_buy_ts_client[player_id].push({
                    item_key: '',
                    type: 0,
        ...
        const refresh_soul = GameRules.MysticalShopSystem.initial_refresh_price;
        ...
        for (const key in MysteriousShopConfig) {
            ...
            for (let i_r = 0; i_r < 6; i_r++) {
                //品质
                this.item_level_probability_group[player_id][i_r].push(Info.probability[i_r]);
            }
        }
        ...
        for (let index = 0; index < this.player_shop_field_count[player_id]; index++) {
            ...
            this.shop_field_list[player_id].list.push({
                key: 'null',
                soul: 0,
                is_discount: 0,
                discount_rate: 100,
    ...
    RefreshOneItemBySoul(player_id: PlayerID, params: CGED['MysticalShopSystem']['RefreshOneItemBySoul'], callback?: string) {
        if (this.shop_state_data[player_id].is_ready == 0) {
            const refresh_count = this.shop_field_list[player_id].refresh_count;
            const refresh_max = this.shop_field_list[player_id].refresh_max;

            if (refresh_count >= refresh_max) {
                GameRules.CMsg.SendErrorMsgToPlayer(player_id, '神秘商店 : 刷新超过上限!');
                return;
            }
            const refresh_soul = this.shop_field_list[player_id].refresh_soul;
            const player_gold_start = GameRules.ResourceSystem.ModifyResource(player_id, { Soul: -refresh_soul });
    ...
    StopShopSystem() {
        ...
        for (let index = 0 as PlayerID; index < this.player_count; index++) {
            ...
            for (let i = 0; i < this.player_shop_field_count[index]; i++) {
                ...
                if (this.shop_field_list[index].list[i].is_lock == 0) {
                    this.shop_field_list[index].list[i] = {
                        key: 'null',
                        soul: 0,
                        is_discount: 0,
                        discount_rate: 100,
            ...
            this.shop_field_list[index].refresh_count = 0;
            this.shop_field_list[index].refresh_soul = refresh_soul;
    ...
    private PlayerShopItem(player_id: PlayerID, is_refresh: boolean = false, i_index: number = -1) {
        ...
        if (is_one_refresh) {
            ...
            for (let it = 0; it < this.player_shop_field_count[player_id]; it++) {
                if (it == i_index) {
                    if (this.shop_field_list[player_id].list[i_index]) {
                        const item_data = this.shop_field_list[player_id].list[i_index];
                        shop_wp_list.push(item_data.key);
                    }
                } else {
                    const item_data = this.shop_field_list[player_id].list[it];
                    shop_wp_list.push(item_data.key);
                }
            }
        } else {
            //回归池子 全刷新
            for (const item_data of this.shop_field_list[player_id].list) {
                //锁定和卖了不会回归池子
        ...
        for (let index = 0; index < for_max; index++) {
            ...
            //获取具体物品
            const item_index = GetCommonProbability(this.item_level_probability_group[player_id][this.player_shop_level[player_id]]);
            const item_name = this.item_level_group[item_index];
            if (!is_one_refresh) {
                //保存锁定记录
            ...
            if (item_name && item_name != '') {
                //是否全局唯一
                const ItemsCustomInfo = MysteriousShopConfig[item_name as 'prop_1'];
                ...
                const buysoul = math.ceil((ItemsCustomInfo.soul * this.player_shop_discount[player_id]) / 100);
                this.shop_field_list[player_id].list[field_index].type = ItemsCustomInfo.type;
                this.shop_field_list[player_id].list[field_index].key = item_name;
                this.shop_field_list[player_id].list[field_index].soul = buysoul;
                this.shop_field_list[player_id].list[field_index].rarity = ItemsCustomInfo.rarity;
                this.shop_field_list[player_id].list[field_index].is_buy = 0;
                this.shop_field_list[player_id].list[field_index].is_lock = 0;
                if (ItemsCustomInfo.type == 2) {
                    //如果玩家没有购买过 且是成长装备
                    if (!this.player_shop_buy_ts_data[player_id].hasOwnProperty(item_name)) {
                        const ssp_index = GetCommonProbability(ItemsCustomInfo.star_start_pro);
                        const new_rarity = ssp_index + 1;
                        this.shop_field_list[player_id].list[field_index].rarity = new_rarity;
                        let count_soul = 0;
                        for (let ssp_i = 0; ssp_i <= ssp_index; ssp_i++) {
                            count_soul += ItemsCustomInfo.star_soul[ssp_i];
                        }
                        this.shop_field_list[player_id].list[field_index].soul = count_soul;
                    } else {
                        const item_count = this.player_shop_buy_ts_data[player_id][item_name];
                        this.shop_field_list[player_id].list[field_index].rarity = item_count + 1;
        ...
        if (is_refresh) {
            ...
            //刷新价格增加
            const refresh_soul = GameRules.MysticalShopSystem.GetSXPrice(this.shop_field_list[player_id].refresh_count);
    ...
    GetSXPrice(count: number): number {
        const initial_refresh_price = GameRules.MysticalShopSystem.initial_refresh_price;
    ...
    GetShopData(player_id: PlayerID, params: CGED['MysticalShopSystem']['GetShopData'], callback?: string) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MysticalShopSystem_GetShopData', {
            data: {
    ...
    GetShopState(player_id: PlayerID, params: CGED['MysticalShopSystem']['GetShopState'], callback?: string) {
        print('=========== GetShopState ===========');
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('MysticalShopSystem_GetShopState', {
                data: {
            ...
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MysticalShopSystem_GetShopState', {
                data: {
                    shop_state_data: this.shop_state_data,
                    start_buy_state: this.start_buy_state,
                    countdown_timer: this.countdown_timer,
    ...
    BuyItem(player_id: PlayerID, params: CGED['MysticalShopSystem']['BuyItem'], callback?: string) {
        if (this.shop_state_data[player_id].is_ready == 0) {
            const item_index = params.index;
            if (this.shop_field_list[player_id].list[item_index]) {
                const item_info = this.shop_field_list[player_id].list[item_index];
                if (item_info.is_buy == 0) {
                    if (item_info.type == 2) {
                        if (!this.player_shop_buy_ts_data[player_id].hasOwnProperty(item_info.key)) {
                            const length = Object.keys(this.player_shop_buy_ts_data[player_id]).length;
                    ...
                    //扣除资源
                    const ModifyResource = GameRules.ResourceSystem.ModifyResource(player_id, { Soul: -item_info.soul });
                    if (ModifyResource.status) {
                        const name = item_info.key;
                        const rarity = item_info.rarity;
                        //标记为出售
    ...
    AddPropAttribute(player_id: PlayerID, prop_name: string, rarity: number) {
        const ItemData = MysteriousShopConfig[prop_name as keyof typeof MysteriousShopConfig];
        //先判断类型再判断进入哪个背包
        if (ItemData.type == 2) {
            ...
            //成长性道具需修改概率
            const item_con_index = this.item_level_group.indexOf(prop_name);
        ...
        const prop_buff = hHero.FindModifierByName('modifier_prop_effect') as modifier_prop_effect;
        if (prop_buff) {
            const av: AbilityValuesProps = {};
            const AbilityValues = ItemData.AbilityValues;
            //处理属性加载问题
            for (const key in AbilityValues) {
                if (typeof AbilityValues[key] == 'number') {
                    ...
                    const str = tostring(AbilityValues[key]);
                    const av_list = str.split(' ');
                    let av_number = 0;
        ...
        const ObjectValues = ItemData.ObjectValues as CustomAttributeTableType;
        let attr_key = '';
        //根据类型来设置key
        if (ItemData.type == 1) {
        ...
        for (const key in ObjectValues) {
            for (const key_base in ObjectValues[key]) {
                if (ItemData.type == 2) {
                    const beilv = ItemData.star_attr_pro[rarity - 1];
        ...
        if (ItemData.type == 2 && this.player_shop_buy_ts_max[player_id] == 0) {
            ...
            if (length >= this.cell_max) {
                for (let l_i = 0; l_i < this.item_level_group.length; l_i++) {
                    const p_name = this.item_level_group[l_i];
                    const l_type = MysteriousShopConfig[p_name as keyof typeof MysteriousShopConfig].type;
                    if (l_type == 2) {
        ...
        const ret_action_string = ItemData.ret_action;
        const param = ItemData.AbilityValues;
        ...
        GameRules.ServiceInterface.PostLuaLog(
            player_id,
            '购买物品:' + prop_name + '(' + this.player_shop_buy_data[player_id][prop_name] + '/' + ItemData.buy_count_max + ')'
        );
        //发送信息
    ...
    GetPlayerShopBuyData(player_id: PlayerID, params: CGED['MysticalShopSystem']['GetPlayerShopBuyData'], callback?: string) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MysticalShopSystem_GetPlayerShopBuyData', {
            data: {
    ...
    LowerConsume(player_id: PlayerID, param: { value: string }, name: string, rarity: number) {
        ...
        const value_number = 100 - tonumber(value_list[rarity - 1]);
        GameRules.MysticalShopSystem.player_shop_discount[player_id] = value_number;
        //更新商店正在售卖物品的价格
        const length = this.shop_field_list[player_id].list.length;
        for (let index = 0; index < length; index++) {
            //没有被卖出的物品都更新价格
            if (GameRules.MysticalShopSystem.shop_field_list[player_id].list[index].is_buy == 0) {
                const ItemsCustomInfo = MysteriousShopConfig[GameRules.MysticalShopSystem.shop_field_list[player_id].list[index].key as 'prop_1'];
                const buysoul = math.ceil((ItemsCustomInfo.soul * this.player_shop_discount[player_id]) / 100);
                GameRules.MysticalShopSystem.shop_field_list[player_id].list[index].soul = buysoul;
            }
        }
        this.GetShopData(player_id, {});
    }
    ...
    prop_66(player_id: PlayerID, param: { fidamage: number; soulpro: number }, name: string, rarity: number) {
        const hHero = PlayerResource.GetSelectedHeroEntity(player_id);
        const fidamage = param.fidamage;
        hHero.prop_count['prop_66'] = 6;
        const attr_key = 'shop_prop_66';
        const ObjectValues = {
            SoulGetRate: {
                MulRegion: 0.5,
            },
    ...
    prop_69(player_id: PlayerID, param: { soulpro: number; qt_soulpro: number }, name: string, rarity: number) {
        ...
        const hHero = PlayerResource.GetSelectedHeroEntity(player_id);
        const soulpro_num = param.soulpro / 100;
        const qt_soulpro_num = param.qt_soulpro / 100;
        const attr_key = 'shop_prop_69_' + player_id;
    ...
    prop_71(player_id: PlayerID, param: { succeed: number; win: number; lose: number }, name: string, rarity: number) {
        const Soul = GameRules.ResourceSystem.player_resource[player_id]['Soul'];
    ...
    AddBuff(player_id: PlayerID, param: { [key: string]: string | number }, key: string) {
        ...
        const shopdata = MysteriousShopConfig[key as keyof typeof MysteriousShopConfig];
        if (unit) {
            // 技能
            const custom_datadriven_ability = unit.FindAbilityByName('custom_datadriven_ability');
            GameRules.CustomAttribute.AddHeroModifier(unit, custom_datadriven_ability, shopdata.BuffName, shopdata.Drive as 'Driven' | 'Script');
        }
    }
    ...
    AddBuffOfAll(player_id: PlayerID, param: { [key: string]: string | number }, key: string) {
        ...
        for (let index = 0 as PlayerID; index < playercount; index++) {
            const unit = PlayerResource.GetSelectedHeroEntity(index);
            if (unit) {
                // 技能
                const custom_datadriven_ability = unit.FindAbilityByName('custom_datadriven_ability');
                GameRules.CustomAttribute.AddHeroModifier(unit, custom_datadriven_ability, shopdata.BuffName, shopdata.Drive as 'Driven' | 'Script');
    ...
    AddAbility(player_id: PlayerID, param: { [key: string]: string | number }, key: string) {
        ...
        if (unit) {
            // 技能
            const ability = unit.AddAbility(shopdata.BuffName);
        }
    }
    ...
    SaintSword(player_id: PlayerID, param: { AttackBasePer: number }, key: string) {
        const hHero = PlayerResource.GetSelectedHeroEntity(player_id);
        const count = param.AttackBasePer * this.player_shop_buy_data[player_id]['prop_14'];
        const attr_key = 'prop_14_SaintSword';
        const ObjectValues = {
            AttackDamage: {
                BasePercent: count,
            },
    ...
    AddAttrOfAll(player_id: PlayerID, param: { ManaRegenBase: number }, key: string) {
        for (const hero of HeroList.GetAllHeroes()) {
            const TeamPropCount = this.GetTeamPropCount('prop_35');
            const count = param.ManaRegenBase * TeamPropCount;
            const attr_key = 'prop_35_aoshuzhihuan';
            const ObjectValues = {
                ManaRegen: {
                    Base: count,
                },
    ...
    GhoulsSealed(player_id: PlayerID, param: { FinalDamageMul: number }, key: string) {
        const hHero = PlayerResource.GetSelectedHeroEntity(player_id);
        const skill_index_list: number[] = [];
        for (let index = 0; index < 5; index++) {
            const Ability = hHero.GetAbilityByIndex(index);
            const null_name = 'public_null_' + (index + 1);
        ...
        if (skill_index_list.length > 0) {
            const i = RandomInt(0, skill_index_list.length - 1);
            const sk_index = skill_index_list[i];
        ...
        const attr_key = 'prop_58_' + player_id + '_' + sk_index_id;
        const ObjectValues = {
            FinalDamageMul: {
    ...
    AddAttr(player_id: PlayerID, param: { [key: string]: number }, key: string) {
        const unit = PlayerResource.GetSelectedHeroEntity(player_id);
        if (unit) {
            const attr_list: CustomAttributeTableType = {};
            for (const key in param) {
                if (key.includes('|')) {
                    const attrnamelist = key.split('|');
    ...
    PlayerReady(player_id: PlayerID, params: CGED['MysticalShopSystem']['PlayerReady'], callback?: string) {
        ...
        for (let index = 0 as PlayerID; index < player_count; index++) {
            const is_ready = this.shop_state_data[index].is_ready;
            if (is_ready == 0) {
                all_ready = false;
                break;
            }
        ...
        if (all_ready) {
            const itme = GameRules.GetDOTATime(false, false);
     ...
     */
    GetTKV<Key extends keyof typeof MysteriousShopConfig, T2 extends (typeof MysteriousShopConfig)[Key]>(
        prop_name: Key,
        ...
        const length = this.prop_ability_values[prop_name][value_key].length;
        if (length > 0) {
            if (level_index < 0) {
                return this.prop_ability_values[prop_name][value_key][0];
     ...
     * @returns
     */
    GetKvOfUnit<Key extends keyof typeof MysteriousShopConfig, T2 extends (typeof MysteriousShopConfig)[Key]>(
        hUnit: CDOTA_BaseNPC,
        prop_name: Key,
        ability_key: keyof T2['AbilityValues']
    ) {
        if (IsServer()) {
            const prop_count = hUnit.prop_count[prop_name];
            if (prop_count == null) {
                return 0;
            } else {
     ...
     * @returns
     */
    GetKvOfUnit_V2<Key extends keyof typeof MysteriousShopConfig, T2 extends (typeof MysteriousShopConfig)[Key]>(
        hUnit: CDOTA_BaseNPC,
        prop_name: Key,
        ability_key: keyof T2['AbilityValues']
    ) {
        const prop_count = hUnit.prop_count[prop_name] ?? 0;
        return this.GetTKV(prop_name, ability_key, prop_count);
    }
