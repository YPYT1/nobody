...
export class CMsg extends UIEventRegisterClass {
    ...
    SendCommonMsgToPlayer(player_id: PlayerID, message: string, data?: MessageObjectDataProps) {
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('CMsg_SendCommonMsgToPlayer', {
                data: {
                    message: message,
                    data: data,
                },
            ...
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'CMsg_SendCommonMsgToPlayer', {
                data: {
                    message: message,
                    data: data,
                },
    ...
    SendMsgToAll(event_type: CGMessageEventType) {
        CustomGameEventManager.Send_ServerToAllClients('CMsg_SendMsgToAll', {
            data: {
    ...
    SendTopCountdown(end_timer: number) {
        ...
        CustomGameEventManager.Send_ServerToAllClients('CMsg_TopCountdown', {
            data: {
    ...
    GetTopCountdown(player_id: PlayerID, params: any) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'CMsg_TopCountdown', {
            data: {
                end_timer: this.top_countdown,
            },
        });
    ...
    SendErrorMsgToPlayer(player_id: PlayerID, message: string, data?: MessageObjectDataProps) {
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('CMsg_SendErrorMsgToPlayer', {
                data: {
                    message: message,
                    data: data,
                },
            ...
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'CMsg_SendErrorMsgToPlayer', {
                data: {
                    message: message,
                    data: data,
                },
    ...
    SendServerMsgToPlayer(player_id: PlayerID, code: number, message: string, type: number = 0, data?: any) {
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('CMsg_SendServerMsgToPlayer', {
                code,
                message,
                type,
                data,
            ...
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'CMsg_SendServerMsgToPlayer', {
                code,
                message,
                type,
                data,
    ...
    SetBossHealthBar(hUnit: CDOTA_BaseNPC, barTypes: TopHealthBarTyps = 'Boss') {
        const unit_entity = hUnit.GetEntityIndex();
    ...
    RemoveBossHealthBar(hUnit: CDOTA_BaseNPC) {
        if (hUnit == null || IsValid(hUnit)) {
            return;
        }
        const unit_entity = hUnit.GetEntityIndex();
        const index = this.boss_list.indexOf(unit_entity);
    ...
    RemoveAllHealthBar() {
        for (const entity of this.boss_list) {
            const hUnit = EntIndexToHScript(entity);
            UTIL_Remove(hUnit);
        }
        this.boss_list = [];
        this.GetEntityListHealthBar(-1, {});
    ...
    GetEntityListHealthBar(player_id: PlayerID, params: any) {
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('CMsg_GetEntityListHealthBar', {
                data: {
                    boss_list: this.boss_list,
                },
            });
        } else {
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'CMsg_GetEntityListHealthBar', {
                data: {
                    boss_list: this.boss_list,
                },
            });
    ...
    Popups(target: CDOTA_BaseNPC, popup_type: PopupsType, amount: number, player?: CDOTAPlayerController) {
        if (player) {
            CustomGameEventManager.Send_ServerToPlayer(player, 'CMsg_PopupUnitState', {
                data: {
            ...
            CustomGameEventManager.Send_ServerToAllClients('CMsg_PopupUnitState', {
                data: {
                    unit: target.entindex(),
                    popup_type: popup_type,
                    amount: amount,
    ...
    GetDamageRecord(...arg: any) {
        CustomGameEventManager.Send_ServerToAllClients('CMsg_GetDamageRecord', {
            data: {
    ...
    BossCastWarning(show: boolean, message?: string, data?: MessageObjectDataProps) {
        CustomGameEventManager.Send_ServerToAllClients('CMsg_BossCastWarning', {
            data: {
                show: show ? 1 : 0,
                message: message,
                data: data,
    ...
    AbilityChannel(hCaster: CDOTA_BaseNPC, hMdf: CDOTA_Buff, state: number) {
        if (hMdf.GetAbility() == null) {
            return;
        }
        if ((hMdf.is_clone ?? 0) == 1) {
            return;
        }
        ...
        CustomGameEventManager.Send_ServerToPlayer(hCaster.GetPlayerOwner(), 'CMsg_AbilityChannel', {
            data: {
                state: state,
    ...
    Debug(cmd: string, args: string[], player_id: PlayerID): void {
        ...
        if (cmd == '-event') {
            const event_id = tonumber(args[0] ?? '101');
