import type { modifier_skywrath_2b_shield } from '../../../abilities/hero/skywrath/skywrath_2/skywrath_2b';
...
export class DamageSystem {
    ...
    DelReduction(hUnit: CDOTA_BaseNPC, key: string) {
        if (hUnit == null || hUnit.ReductionPct == null) {
            return;
        }
        key = 'reduction_' + key;
    ...
    UpdateReduction(hUnit: CDOTA_BaseNPC) {
        hUnit.SetContextThink(
            ...
            () => {
                let base_value = 1;
                for (const key in hUnit.ReductionPct) {
                    const value = hUnit.ReductionPct[key];
    ...
    GetReduction(hUnit: CDOTA_BaseNPC, damage: number, damage_type: DamageTypes, element_type?: ElementTypes) {
        const custom_attribute = hUnit.custom_attribute_value;
        if (element_type == ElementTypes.NONE) {
            // 物理护甲 碰撞伤害
            const PhyicalArmor = custom_attribute.PhyicalArmor;
            const armor_dmg_reduction = PhyicalArmor / (100 + math.abs(PhyicalArmor));
    ...
    ApplyDamage(params: ApplyCustomDamageOptions) {
        ...
        if (hAttacker == null || IsValid(hAttacker)) {
            return 0;
        }
        if (hTarget.HasModifier('modifier_state_boss_invincible') || hTarget.HasModifier('modifier_state_boss_invincible_channel')) {
        ...
        const hAbility = params.ability;
        let element_type = params.element_type ?? ElementTypes.NONE;
        const is_primary = params.is_primary ?? false;
        let is_crit = 0;
        const critical_flag = params.critical_flag ?? 0;
        ...
        const CriticalDamage = hAttacker.custom_attribute_value.CriticalDamage;
        // 乘区计算
        // let BasicAbilityDmg = (params.BasicAbilityDmg ?? 100) * 0.01
        // print("params.SelfAbilityMul",params.SelfAbilityMul)
        const SelfAbilityMul = (params.SelfAbilityMul ?? 100) * 0.01;
        let DamageBonusMul = (params.DamageBonusMul ?? 0) + hAttacker.custom_attribute_value.DamageBonusMul;
        const AbilityImproved = (params.AbilityImproved ?? 0) + hAttacker.custom_attribute_value.AbilityImproved;
        let ElementDmgMul = (params.ElementDmgMul ?? 0) + hAttacker.custom_attribute_value.AllElementDamageBonus;
        const FinalDamageMul = (params.FinalDamageMul ?? 0) + hAttacker.custom_attribute_value.FinalDamageMul;
        const DmgReductionPct = (100 - (params.victim.enemy_attribute_value.DmgReductionPct ?? 0)) * 0.01;
        ...
        // 游侠天赋击破效果
        const drow_13_stack_buff = hTarget.FindModifierByName('modifier_drow_2a_a_debuff');
        if (drow_13_stack_buff) {
            const stack = drow_13_stack_buff.GetStackCount();
            const stack_income = GameRules.HeroTalentSystem.GetTalentKvOfUnit(drow_13_stack_buff.GetCaster(), '13', 'value');
        ...
        if (params.damage_type == DamageTypes.PHYSICAL) {
            ...
            if (element_type == ElementTypes.FIRE) {
                ...
                const EPent = params.attacker.custom_attribute_value.FirePent ?? 0;
                const EResist = params.victim.enemy_attribute_value.FireResist ?? 0;
                ...
                if (params.attacker.prop_count['prop_3']) {
                    const FireResist = GameRules.MysticalShopSystem.GetKvOfUnit(params.attacker, 'prop_3', 'FireResist');
                    const duration = GameRules.MysticalShopSystem.GetKvOfUnit(params.attacker, 'prop_3', 'duration');
                ...
                const EPent = params.attacker.custom_attribute_value.ThunderPent ?? 0;
                const EResist = params.victim.enemy_attribute_value.ThunderResist ?? 0;
                ElementResist += math.min(0, EPent - EResist);
                if (is_primary) {
                ...
                const EPent = params.attacker.custom_attribute_value.WindPent ?? 0;
                const EResist = params.victim.enemy_attribute_value.WindResist ?? 0;
                ElementResist += math.min(0, EPent - EResist);
        ...
        const ElementResistMul = (100 - ElementResist) * 0.01;
        // print("SelfAbilityMul", SelfAbilityMul)
        let increased_injury =
            1 *
            SelfAbilityMul *
        ...
        if (params.victim.IsBossCreature()) {
            const CreatureDmgLeader = (params.attacker.custom_attribute_value.CreatureDmgLeader ?? 0) * 0.01;
            increased_injury *= 1 + CreatureDmgLeader;
        } else {
            const CreatureDmgNormal = (params.attacker.custom_attribute_value.CreatureDmgNormal ?? 0) * 0.01;
        ...
        const talent_mdf = hAttacker.FindModifierByName('modifier_talent_effect') as modifier_talent_effect;
        if (talent_mdf) {
        ...
        // 特殊机制
        const bonus_dmg_pct = this.AboutSpecialMechanism(params);
        ...
        // 后续
        const picture_mdf = hAttacker.FindModifierByName('modifier_picture_abilities') as modifier_picture_abilities;
        if (picture_mdf) {
            picture_mdf._OnTakeDamage(params);
            if (is_crit == 1) {
                picture_mdf._CritEvent(params);
            }
        }
    ...
    ApplyDamageForBadTeam(params: ApplyCustomDamageOptions) {
        // 无敌
        if (params.victim.HasModifier('modifier_altar_effect_6') || params.victim.HasModifier('modifier_public_blink')) {
            return 0;
        }
        // 雷电屏障
        if (params.miss_flag != 1 && params.victim.HasModifier('modifier_skywrath_2b_shield')) {
            const skywrath_2b = params.victim.FindModifierByName('modifier_skywrath_2b_shield') as modifier_skywrath_2b_shield;
            skywrath_2b._OnHit(params.attacker);
            return 0;
        }

        params.damage_type = DamageTypes.PURE;
        const custom_attribute_value = params.victim.custom_attribute_value;
        if (custom_attribute_value == null) {
            return ApplyDamage(params);
        }

        const EvasionProb = custom_attribute_value ? custom_attribute_value.EvasionProb : 0;
        if (params.miss_flag != 1 && RollPercentage(EvasionProb)) {
            // 闪避
            const talent_mdf = params.victim.FindModifierByName('modifier_talent_effect') as modifier_talent_effect;
            if (talent_mdf) {
        ...
        params.damage = this.GetReduction(params.victim, params.damage, params.damage_type, params.element_type);

        params.damage = params.damage * (100 - custom_attribute_value.DmgReductionPct) * 0.01;

        const picture_buff = params.victim.FindModifierByName('modifier_picture_abilities') as modifier_picture_abilities;
        if (picture_buff) {
            if (picture_buff.OnBeInjured(params)) {
                return 0;
            }
        }
        const rune_buff = params.victim.FindModifierByName('modifier_rune_effect') as modifier_rune_effect;
        if (rune_buff) {
        ...
        const prop_buff = params.victim.FindModifierByName('modifier_prop_effect') as modifier_prop_effect;
        if (prop_buff) {
    ...
    GetBonusDamageFromProp(params: ApplyCustomDamageOptions) {
        let bonus = 0;
        const distance = ((params.attacker.GetAbsOrigin() - params.victim.GetAbsOrigin()) as Vector).Length2D();
        // prop_8	【否定信仰】	队伍对有控制状态效果的人额外造成15%伤害
        const prop_8_count = GameRules.MysticalShopSystem.GetTeamPropCount('prop_8');
        if (prop_8_count > 0) {
            if (params.victim.IsStunned()) {
                const prop_8_value = GameRules.MysticalShopSystem.GetTKV('prop_8', 'value', 0);
        ...
        if (params.attacker.prop_count['prop_18']) {
            const more_than_pct = GameRules.MysticalShopSystem.GetKvOfUnit(params.attacker, 'prop_18', 'more_than_pct');
            const less_than_pct = GameRules.MysticalShopSystem.GetKvOfUnit(params.attacker, 'prop_18', 'less_than_pct');
            const health_pct = params.victim.GetHealthPercent();
        ...
        const prop_24_count = GameRules.MysticalShopSystem.GetTeamPropCount('prop_24');
        if (prop_24_count > 0) {
            const mdf_list = params.victim.FindAllModifiers();
            let has_debuff = false;
            for (const mdf of mdf_list) {
                if (mdf.IsDebuff()) {
                    has_debuff = true;
                    break;
                }
