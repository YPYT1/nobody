...
export class CustomItem extends UIEventRegisterClass {
    ...
    Drop(item_key: ItemListKey, vPos: Vector, time: number = -1) {
        const unit_name = 'npc_item_' + item_key;
        const ItemUnit = CreateUnitByName(unit_name, vPos, false, null, null, DotaTeam.NEUTRALS);
        if (ItemUnit) {
    ...
    useItemEffect(item_name: string, hUnit: CDOTA_BaseNPC) {
        if (item_name == 'npc_item_hp') {
            ...
            // 50%最大蓝量
            for (const hHero of HeroList.GetAllHeroes()) {
                const effect_fx = ParticleManager.CreateParticle('particles/items3_fx/mango_active.vpcf', ParticleAttachment.POINT_FOLLOW, hHero);
                ParticleManager.ReleaseParticleIndex(effect_fx);
    ...
    Debug(cmd: string, args: string[], player_id: PlayerID): void {
        ...
        if (cmd == '-drop') {
            const item = (args[0] ?? 'hp') as ItemListKey;
