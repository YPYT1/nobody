...
export class InvestSystem extends UIEventRegisterClass {
    ...
    constructor() {
        ...
        for (let player_id = 0 as PlayerID; player_id < this.player_count; player_id++) {
            ...
            const level = this.PlayerInvestLevelList[player_id] + this.PlayerExtraInvestLevelList[player_id];
            const InvestGetResource = this.EqK(player_id, level);
    ...
    GetPlayerInvestData(player_id: PlayerID, params: CGED['InvestSystem']['GetPlayerInvestData'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'InvestSystem_GetPlayerInvestData', {
            data: {
    ...
    PostInvestUp(player_id: PlayerID, Level: number) {
        ...
        if (hero.rune_level_index['rune_2']) {
            if (this.PlayerInvestLevelList[player_id] + this.PlayerExtraInvestLevelList[player_id] > 0) {
                const value = GameRules.RuneSystem.GetKvOfUnit(hero, 'rune_2', 'value');
                const attr = value * (this.PlayerInvestLevelList[player_id] + this.PlayerExtraInvestLevelList[player_id]) - 1;
    ...
    EqK(player_id: PlayerID, level: number): number {
        ...
        const Efficiency = this.InvestGetResourceEfficiencyList[player_id];
        //重新获取资源计算规则
    ...
    Debug(cmd: string, args: string[], player_id: PlayerID) {
        if (cmd == '-syks') {
            ...
            //投资等级升级
            const up_level = args[0] ? tonumber(args[0]) : 1;
