...
export class RuneSystem extends UIEventRegisterClass {
    ...
    constructor() {
        ...
        for (const i_key in RuneConfig) {
            const data = RuneConfig[i_key as keyof typeof RuneConfig];
        ...
        for (const i_key in RuneAttrConfig) {
    ...
    InitPlayerUpgradeStatus(player_id: PlayerID, hHero: CDOTA_BaseNPC = null) {
        ...
        //公共物品物品
        const drop_info: { key: string[]; pro: number[] } = {
            key: [],
            pro: [],
        };
        const tianhui_task_drop_list: { key: string[]; pro: number[] } = {
            key: [],
            pro: [],
        };
        const nightmare_task_drop_list: { key: string[]; pro: number[] } = {
            key: [],
            pro: [],
        };

        const low_drop_list: { key: string[]; pro: number[] } = {
            key: [],
            pro: [],
        };

        for (const key in RuneConfig) {
    ...
    GetRuneSelectToPlayer(player_id: PlayerID, type: number = 0) {
        ...
        const fate_dota: CGEDPlayerRuneSelectServerData = {
            is_check: false,
    ...
    GetRuneSelectData(player_id: PlayerID, params: CGED['RuneSystem']['GetRuneSelectData'], callback?) {
        //商店组成 1未刷新 2未挑战
        const data: CGEDPlayerRuneSelectDataList = {
            item_list: {},
        ...
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'RuneSystem_GetRuneSelectData', {
            data,
        });
    }
    ...
    GetPlayerRuneData(player_id: PlayerID, params: CGED['RuneSystem']['GetPlayerRuneData'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'RuneSystem_GetPlayerRuneData', {
    ...
    RefreshShopList(player_id: PlayerID, is_sx = false) {
        const fate_data_info = this.player_fate_data[player_id][this.player_fate_data_index[player_id]];
        ...
        if (fate_data_info.is_refresh == false) {
            const _hero = PlayerResource.GetSelectedHeroEntity(player_id);
            const hero_level = _hero.GetLevel();
            //修改已刷新状态
            fate_data_info.is_refresh = true;
            //最多几样物品
            const amount = this.player_select_amount[player_id];
            const ret_data: { [key: string]: CGEDPlayerRuneSelectData } = {};
            const shop_wp_list: string[] = [];
            for (let index = 1; index <= amount; index++) {
                // 45 - 44
                const rune_sub = this.player_select_rune_max[player_id] - this.check_rune_name[player_id].length;
                if (index > rune_sub) {
                    //补全
                ...
                if (fate_data_info.type == 1) {
                    const key_list = this.tianhui_task_drop_list[player_id].key;
                    const pro_list = this.tianhui_task_drop_list[player_id].pro;
                    item_name = key_list[GetCommonProbability(pro_list)];
                } else if (fate_data_info.type == 2) {
                    const key_list = this.nightmare_task_drop_list[player_id].key;
                    const pro_list = this.nightmare_task_drop_list[player_id].pro;
                    item_name = key_list[GetCommonProbability(pro_list)];
                } else {
                    if (hero_level <= GameRules.MapChapter.LowRuneDropLevel) {
                        const key_list = this.low_drop_list[player_id].key;
                        const pro_list = this.low_drop_list[player_id].pro;
                        item_name = key_list[GetCommonProbability(pro_list)];
                    } else {
                        const key_list = this.drop_list[player_id].key;
                        const pro_list = this.drop_list[player_id].pro;
                        item_name = key_list[GetCommonProbability(pro_list)];
                    }
                }
                if (item_name == 'null') {
                ...
                let attr_count = 1;
                const ran_number = RandomInt(1, 100);
                if (ran_number > 98) {
                    // 99  100
                    attr_count = 3;
                ...
                if (!_hero.prop_count['prop_72']) {
                    for (let a_i = 0; a_i < attr_count; a_i++) {
                        const attr_id_index = RandomInt(0, this.rune_attr_list.length - 1);
                        const attr_id = this.rune_attr_list[attr_id_index];
                        if (attr_list.hasOwnProperty(attr_id)) {
                            a_i--;
                        } else {
                            const attr_id_data = RuneAttrConfig[attr_id as keyof typeof RuneAttrConfig];
                            const attr_id_number = this.ZoomNumber(attr_id_data.AttrSection, attr_id_data.Decimal);
            ...
            if (is_sx == false) {
                ...
                if (_hero.prop_count['prop_67']) {
                    ...
                    const count_down_time = GameRules.GetDOTATime(false, false) + this.count_down_time;
                    ...
                    hero.SetContextThink(
                        'REFRESH_SHOP_LIST' + '_' + player_id + '_' + this.player_fate_data_index[player_id],
                        () => {
                            GameRules.RuneSystem.TimeSelectRune(player_id);
                            return null;
                        },
    ...
    PostSelectRune(player_id: PlayerID, params: CGED['RuneSystem']['PostSelectRune']) {
        ...
        if (GameRules.RuneSystem.player_fate_data[player_id].length > GameRules.RuneSystem.player_fate_data_index[player_id]) {
            const fate_data_info = GameRules.RuneSystem.player_fate_data[player_id][GameRules.RuneSystem.player_fate_data_index[player_id]];
            ...
            const item_name = fate_data_info.item_list[index].name;
            level_index = fate_data_info.item_list[index].level_index;
            const attr_list = fate_data_info.item_list[index].attr_list;
    ...
    UpdateRuneMdf(name: string, hHero: CDOTA_BaseNPC) {
        ...
        if (rune_mdf) {
            const row_rune_data = RuneConfig[name as keyof typeof RuneConfig];
            ...
            for (const k in AbilityValues) {
                ...
                const value = this.GetKvOfUnit(hHero, name as 'rune_2', k as 'value');
    ...
    GetRune(
        ...
        const item_name = params.item_name as keyof typeof RuneConfig;
        let rune_index = 0;
        ...
        const RuneData = RuneConfig[item_name as keyof typeof RuneConfig];
        const is_more_level = RuneData.is_item_level == 1 ? true : false;
        const is_level_up = RuneData.is_level_up == 1 ? true : false;
        const is_all = RuneData.is_all;
        const rune_level = RuneData.item_level_section[level_index];
        const hHero = PlayerResource.GetSelectedHeroEntity(player_id);

        const item_level_section_length =
            GameRules.RuneSystem.rune_keyvalue[item_name as keyof typeof GameRules.RuneSystem.rune_keyvalue].item_level_section.length;
        //品质升级
        ...
        GameRules.CMsg.SendCommonMsgToPlayer(-1 as PlayerID, '#custom_text_player_rune_selected', {
            player_id: player_id,
            rune_name: item_name,
        ...
        //特殊处理符文变化时
        if (GameRules.RuneSystem.check_rune_name[player_id].includes('rune_102')) {
            // 聚少成多
            const kv_value = GameRules.RuneSystem.GetKvOfUnit_V2(hHero, 'rune_102', 'value');
            //更新数值
            const rune_count = GameRules.RuneSystem.check_rune_name[player_id].length;
            const value = rune_count * kv_value;
        ...
        const ret_action_string = RuneData.ret_action;
        const param = RuneData.AbilityValues;
    ...
    GetRuneValues(player_id: PlayerID, rune_name: string, level_index: number, is_all: number) {
        const hHero = PlayerResource.GetSelectedHeroEntity(player_id);
        const ObjectValues = RuneConfig[rune_name as keyof typeof RuneConfig].ObjectValues;
        const attr_count: CustomAttributeTableType = {};
        if (is_all == 1) {
            for (const hero of HeroList.GetAllHeroes()) {
                const my_player_id = hero.GetPlayerID();
                for (const Attr in ObjectValues) {
                    ...
                    for (const AttrType in ObjectValues[Attr]) {
                        if (typeof ObjectValues[Attr][AttrType] == 'number') {
                            ...
                            let value = 0;
                            if (Str_List.length <= level_index + 1) {
                                // 2  2
    ...
    GetSelectRuneAttr(
        ...
        if (Object.keys(attr_list).length > 0) {
            ...
            for (const key in attr_list) {
                const count = attr_list[key];
                const RuneData = RuneAttrConfig[key as keyof typeof RuneAttrConfig];
                const AttrName = RuneData.AttrName;
                const AttrClass = RuneData.AttrClass;
     ...
     */
    GetTKV<Key extends keyof typeof RuneConfig, T2 extends (typeof RuneConfig)[Key]>(
        rune_name: Key,
        key: keyof T2['AbilityValues'],
        level_index: number = 0
    ) {
        const value_key = key as string;
        const length = this.rune_ability_values[rune_name][value_key].length;
        if (length > 0) {
            if (level_index < 0) {
     ...
     * @returns
     */
    GetKvOfUnit<Key extends keyof typeof RuneConfig, T2 extends (typeof RuneConfig)[Key]>(
        hUnit: CDOTA_BaseNPC,
        rune_name: Key,
        ability_key: keyof T2['AbilityValues']
    ) {
        if (IsServer()) {
            const level_index = hUnit.rune_level_index[rune_name];
            if (level_index == null) {
                return 0;
            } else {
     ...
     * @returns
     */
    GetKvOfUnit_V2<Key extends keyof typeof RuneConfig, T2 extends (typeof RuneConfig)[Key]>(
        hUnit: CDOTA_BaseNPC,
        rune_name: Key,
        ability_key: keyof T2['AbilityValues']
    ) {
        const level_index = hUnit.rune_level_index[rune_name] ?? 0;
        return this.GetTKV(rune_name, ability_key, level_index);
    }
    ...
    UpgradeRune(player_id: PlayerID, rune_index: string = '', is_max: boolean = false) {
        if (rune_index == '') {
            const RuneKeys: string[] = [];
            for (const key in this.player_rune_list[player_id]) {
                if (
                    this.player_rune_list[player_id][key].is_level_up &&
            ...
            if (RuneKeys.length < 1) {
                GameRules.CMsg.SendCommonMsgToPlayer(player_id as PlayerID, '没有可升级的符文');
                return;
            }
            const random_index = RandomInt(0, RuneKeys.length - 1);
            rune_index = RuneKeys[random_index];
        }
        const hHero = PlayerResource.GetSelectedHeroEntity(player_id);
        if (this.player_rune_list[player_id][rune_index].is_more_level && this.player_rune_list[player_id][rune_index].is_delete == false) {
            const PlayerRune = this.player_rune_list[player_id][rune_index];
            const _name = PlayerRune.name;
            const item_level_section_length =
                GameRules.RuneSystem.rune_keyvalue[_name as keyof typeof GameRules.RuneSystem.rune_keyvalue].item_level_section.length;
            if (item_level_section_length > PlayerRune.level_index + 1) {
                GameRules.CMsg.SendCommonMsgToPlayer(player_id as PlayerID, '#custom_text_player_rune_up_1', {
                    player_id: player_id, //玩家id
                    rune_name: _name, //符文名字
                });
                if (is_max) {
                    this.player_rune_list[player_id][rune_index].level =
                        GameRules.RuneSystem.rune_keyvalue[_name as keyof typeof GameRules.RuneSystem.rune_keyvalue].item_level_section[
                    ...
                    //重算属性
                    GameRules.RuneSystem.UpgradeRuneValues(player_id, _name, item_level_section_length - 1, PlayerRune.level_index);
                } else {
                    this.player_rune_list[player_id][rune_index].level++;
                ...
                //重新发送可升级属性
                GameRules.CMsg.SendCommonMsgToPlayer(player_id as PlayerID, '#custom_text_player_rune_up', {
                    player_id: player_id,
                    rune_name: _name,
                });
                this.GetPlayerRuneData(player_id, {});
            } else {
                GameRules.CMsg.SendCommonMsgToPlayer(player_id as PlayerID, '#custom_text_player_rune_up_2', {
                    player_id: player_id,
                    rune_name: _name,
                });
            }
    ...
    RuneGetATK(player_id: PlayerID, param: { value: number }, key: string) {
        ...
        const rune_count = GameRules.RuneSystem.check_rune_name[player_id].length;
        const hHero = PlayerResource.GetSelectedHeroEntity(player_id);
        const value = rune_count * param.value;
        const attr_count: CustomAttributeTableType = {
            AttackDamage: {
                BasePercent: value,
            },
    ...
    GetExpPro(player_id: PlayerID, param: { value: number }, key: string) {
        const selfhHero = PlayerResource.GetSelectedHeroEntity(player_id);
    ...
    GetExpProALL(player_id: PlayerID, param: { self: number; else: number }, key: string) {
        //更新数值
        const paramself = param.self;
        const paramelse = param.else;
        //更新数值
    ...
    Rune118(player_id: PlayerID, param: {}, key: string) {
        ...
        if (hHero.rune_level_index['rune_113']) {
            const kv_value = GameRules.RuneSystem.GetKvOfUnit_V2(hHero, 'rune_113', 'move_speed');
        ...
        if (hHero.rune_level_index['rune_111']) {
            const buff = hHero.FindModifierByName('modifier_rune_effect_111');
            if (buff) {
                buff.ForceRefresh();
            }
        }
    ...
    Debug(cmd: string, args: string[], player_id: PlayerID): void {
        if (cmd == '-tjfw') {
            //添加一个符文
            const select = tonumber(args[0]) ?? 1;
            const level_index = tonumber(args[1]) ?? 0;
            const playerid = (tonumber(args[2]) as PlayerID) ?? -1;
            GameRules.RuneSystem.GetRune(
                playerid != -1 ? playerid : player_id,
                {
