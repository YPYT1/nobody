...
type MisslieHandleType_Dire =
    ...
    | Mission_Dire_7
    | Mission_Dire_8;
type MisslieHandleType_Radiant =
    | Mission_Radiant_1
    | Mission_Radiant_2
    | Mission_Radiant_3
    | Mission_Radiant_4
...
export class MissionSystem extends UIEventRegisterClass {
    ...
    StartRadiantMissionLine() {
        ...
        this.RadiantOrder += 1;
        const mission_name = this.RadiantMissionList[this.RadiantOrder];
    ...
    StartDireMissionLine() {
        ...
        const vStart = (this.vMapCenter + RandomVector(RandomInt(1500, 2500))) as Vector;
        this.DireOrder += 1;
        const mission_name = this.DireMissionList[this.DireOrder];
    ...
    StartEventOfName(event_name: RadiantMissleNameList | DireMissleNameList, is_test: boolean = false) {
        const ChapterData = GameRules.MapChapter.ChapterData;
        this.vMapCenter = Vector(ChapterData.map_centre_x, ChapterData.map_centre_y, 128);
        const start = (this.vMapCenter + RandomVector(RandomInt(1500, 2500))) as Vector;
    ...
    SendMissionTips(mission_type: number, mission_name: string) {
        ...
        CustomGameEventManager.Send_ServerToAllClients('MissionSystem_SendMissionTips', {
            data: {
    ...
    GetCurrentMission(player_id: PlayerID, params?: any) {
        if (player_id == -1) {
            CustomGameEventManager.Send_ServerToAllClients('MissionSystem_GetCurrentMission', {
                data: this.mission_object,
            });
        } else {
            CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'MissionSystem_GetCurrentMission', {
                data: this.mission_object,
            });
        }
    }
    ...
    MissionCompleteSend(mission_type: number) {
        CustomGameEventManager.Send_ServerToAllClients('MissionSystem_MissionComplete', {
            data: {
                mission_type: mission_type,
            },
        });
    ...
    Debug(cmd: string, args: string[], player_id: PlayerID): void {
        ...
        if (cmd == '-rwks') {
            const name = (args[0] ?? 'd_2') as MissleNameList;
