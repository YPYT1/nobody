...
export class Mission_Dire_4 extends MissionModule {
    ...
    ExecuteLogic(start: Vector): void {
        ...
        const place_point = (start + ((vMapCenter - start) as Vector).Normalized() * -200) as Vector;
        const dummy = CreateUnitByName('npc_mission_dummy', place_point, false, null, null, DotaTeam.BADGUYS);
    ...
    CreateMission(vPos: Vector, vMapCenter: Vector, is_test: boolean) {
        const angle = this.angle_list[RandomInt(0, 3)];
        const line_pos = (vMapCenter + Vector(3200, 0, 0)) as Vector;
