...
export class MissionModule {
    ...
    GetToNextPoints(vStart: Vector, fDistance: number) {
        ...
        const next_pos = (vStart + direction * fDistance * -1) as Vector;
    ...
    StartTheMission(start: Vector) {
        ...
        const end_time = GameRules.GetDOTATime(false, false) + (this.limit_time ?? 0);
    ...
    EndOfMission(success: boolean) {
        ...
        if (this.is_stop == false) {
            GameRules.CMsg.SendCommonMsgToPlayer(-1, '{s:mission_name} 任务 {s:success}', {
                mission_name: this.mission_name,
    ...
    SendMissionProgress() {
        GameRules.CMsg.SendCommonMsgToPlayer(-1, '{s:mission_name} 任务进度{d:progress_value} / {d:progress_max}', {
            mission_name: this.mission_name,
