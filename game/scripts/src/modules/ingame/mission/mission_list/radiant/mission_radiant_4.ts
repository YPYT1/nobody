...
export class Mission_Radiant_4 extends MissionModule {
    ...
    ExecuteLogic(start: Vector): void {
        // 该任务的初始点必然在地图中心的四个角落
        const angle_list = [0, 45, 135, 225, 315];
        const angle_value = angle_list[0];
        const aux_points = (this.vMapCenter + Vector(2000, 0, 0)) as Vector;
        const vStart = RotatePosition(this.vMapCenter, QAngle(0, angle_value, 0), aux_points);
        this.progress_max = 1;
    ...
    _CreateRelayPoint(vOrign: Vector) {
        const unit = CreateModifierThinker(
            null,
            null,
