...
export class BasicRules extends UIEventRegisterClass {
    ...
    CreateRoundAcceleration() {
        ...
        const x = GameRules.MapChapter.ChapterData.map_centre_x;
        const y = GameRules.MapChapter.ChapterData.map_centre_y;
        const vOrigin = Vector(x, y, 128);
    ...
    RemoveRoundAcceleration() {
        ...
        for (const acc_thinker of this.last_acc_thinker) {
    ...
    BossChestDrop(vPos: Vector, boss_wave: number) {
        const chestUnit = CreateUnitByName('npc_public_treasure_chest', vPos, false, null, null, DotaTeam.GOODGUYS);
    ...
    BossChestReward(boss_wave: number) {
        ...
        const gold = boss_wave * 500;
        const exp = boss_wave * 500;
