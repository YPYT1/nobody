...
export class creature_boss_19 extends BaseCreatureAbility {
    ...
    OnSpellStart(): void {
        ...
        const players = PlayerResource.GetPlayerCountForTeam(DotaTeam.GOODGUYS);
        let note: number[] = [];
        if (players > 1) {
            // 保底黄
            note = [3];
            const other_note = [1, 2, 4];
        ...
        for (let i = 0; i < note.length; i++) {
            const hHero = PlayerResource.GetSelectedHeroEntity(i as PlayerID);
            if (hHero) {
...
export class modifier_creature_boss_19 extends BaseModifier {
    ...
    GetModifierIncomingDamage_Percentage(event: ModifierAttackEvent): number {
        if (event.attacker.HasModifier('modifier_creature_boss_19_note1')) {
            ...
            // 黑色音符玩家攻击boss会将伤害转移给平均其他玩家身上。
            if (this.hero_counts <= 1) {
                return 0;
            }
            const damage = (0.25 * event.damage) / (this.hero_counts - 1);
            for (const hHero of HeroList.GetAllHeroes()) {
                // print(hHero , event.attacker,hHero == event.attacker)
                if (hHero == event.attacker) {
                    continue;
                }
                ApplyCustomDamage({
                    victim: hHero,
