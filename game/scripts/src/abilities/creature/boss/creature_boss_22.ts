...
export class creature_boss_22 extends BaseCreatureAbility {
    OnAbilityPhaseStart(): boolean {
        ...
        this.nPreviewFX = GameRules.WarningMarker.Circular(this._radius, this._cast_point, this.vOrigin, false, Vector(255, 255, 0), 1);
    ...
    OnSpellStart(): void {
        ...
        if (enemies.length > 0) {
            ...
            const max_hp_pct = (base_dmg_pct / enemies.length) * 0.01;
            for (const enemy of enemies) {
                const damage = enemy.GetMaxHealth() * max_hp_pct;
                ApplyCustomDamage({
                    victim: enemy,
