...
export class creature_boss_10 extends BaseCreatureAbility {
    ...
    CreateGarbage(vPos: Vector, type: number, vTarget: Vector) {
        let garbage = CreateUnitByName(
            "npc_public_garbage_" + type,
            vPos,
            false,
            null,
            null,
...
export class modifier_creature_boss_10_stack extends BaseModifier {
    ...
    OnDestroy(): void {
        if (!IsServer()) { return }
        let defen = this.GetStackCount() - 1;
        let score_label = "c";
        if (defen < 3) {
            // 分类3个垃圾以下，造成高额伤害。（伤害为玩家最大生命值75%）
            for (let hHero of HeroList.GetAllHeroes()) {
                let damage = hHero.GetMaxHealth() * this.dmg_max_hp
                ApplyCustomDamage({
                    victim: hHero,
...
export class modifier_creature_boss_9_tag_0 extends BaseModifier {
    ...
    OnIntervalThink(): void {
        ...
        let distance = (origin - this.target as Vector).Length2D();
        if (distance < this.radius) {
            // 加分
            let hCaster = this.GetCaster();
            let stack_buff = hCaster.FindModifierByName("modifier_creature_boss_10_stack");
            if (stack_buff) {
...
export class modifier_creature_boss_9_garbag_search extends BaseModifier {
    ...
    OnIntervalThink(): void {
        ...
        if (units.length > 0) {
            ...
            let dir = (this.origin - vTarget as Vector).Normalized()
            let cast_angle = VectorToAngles(dir).y
