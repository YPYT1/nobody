...
export class creature_boss_25 extends BaseCreatureAbility {
    ...
    OnSpellStart(): void {
        ...
        for (let i = 0; i < PlayerResource.GetPlayerCountForTeam(DotaTeam.GOODGUYS); i++) {
            const place_vect = (this.vOrigin + RandomVector(RandomInt(800, 1200))) as Vector;
...
export class modifier_creature_boss_25_channel extends BaseModifier {
    ...
    PlayEffect() {
        for (let i = 0; i < this.count; i++) {
            const pos = RotatePosition(this.origin, QAngle(0, RandomInt(0, 359), 0), this.origin2);
    ...
    SendProjectile(pos: Vector) {
        const vDirection = ((this.origin - pos) as Vector).Normalized();
        vDirection.z = 0;
        ProjectileManager.CreateLinearProjectile({
...
export class modifier_creature_boss_25_spear extends BaseModifier {
    ...
    OnCreated(params: any): void {
        ...
        //
        const effect_fx = ParticleManager.CreateParticle('particles/diy_particles/move_glow.vpcf', ParticleAttachment.POINT_FOLLOW, this.GetParent());
        ParticleManager.SetParticleControlEnt(effect_fx, 0, this.parent, ParticleAttachment.POINT_FOLLOW, 'attach_hitloc', Vector(0, 0, 0), true);
    ...
    OnIntervalThink(): void {
        ...
        if (enemies.length > 0) {
            this.StartIntervalThink(-1);
            const vDirection = ((this.vCaster - this.origin) as Vector).Normalized();
            vDirection.z = 0;
            ProjectileManager.CreateLinearProjectile({
                Ability: this.GetAbility(),
    ...
    DeclareFunctions(): modifierfunction[] {
        return [ModifierFunction.MODEL_CHANGE, ModifierFunction.MODEL_SCALE];
    }
