//change_07.ts
/**
 * name: Windranger（风行者的强力击·三箭）
 * Description: 主动使用：对点指方向同时射出3支强力箭矢，对路径上的敌人造成物理伤害。
 * 三支箭共享同一特效，伤害按“命中顺序”逐支递减：
 *  - 第1个命中的箭：造成 base_Powershot_damage 伤害
 *  - 第2个命中的箭：造成 base_Powershot_damage × (1 - base_damage_coefficient) 伤害
 *  - 第3个命中的箭：造成 base_Powershot_damage × (1 - base_damage_coefficient × 2) 伤害
 * 说明：仅伤害衰减（速度不衰减），并且只在命中时结算伤害（以 OnProjectileHit 为准）。
 * hero: 风行者
 * 配置（AbilityValues）：
 *  - base_Powershot_damage 800 （基础伤害）
 *  - base_damage_coefficient 0.25 （命中顺序伤害递减系数）
 *  - cast_range 3000 （射程）
 *
 ...
 * 特效：
 *  - 飞行体：particles/units/heroes/hero_windrunner/windrunner_spell_powershot.vpcf
 *  - 命中：particles/units/heroes/hero_windrunner/windrunner_spell_powershot_destruction.vpcf
 */
import { registerAbility } from '../../../utils/dota_ts_adapter';
import { BaseCreatureAbility } from '../base_creature';

@registerAbility()
export class change_07 extends BaseCreatureAbility {
