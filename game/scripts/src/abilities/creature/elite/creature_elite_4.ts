...
export class creature_elite_4 extends BaseCreatureAbility {
    ...
    OnAbilityPhaseStart(): boolean {
        ...
        ParticleManager.SetParticleControlEnt(effect_fx, 0, this.GetCaster(), ParticleAttachment.ABSORIGIN_FOLLOW, '', Vector(0, 0, 0), true);
        ParticleManager.SetParticleControlEnt(effect_fx, 1, this.GetCaster(), ParticleAttachment.ABSORIGIN_FOLLOW, '', Vector(0, 0, 0), true);
        this.nPreviewFX_2 = effect_fx;

        const effect_fx2 = ParticleManager.CreateParticle(
    ...
    OnSpellStart(): void {
        ...
        const friendly = FindUnitsInRadius(
        ...
        for (const unit of friendly) {
            const vect = (this.vPoint + Vector(RandomInt(-hero_distance, hero_distance), RandomInt(-hero_distance, hero_distance), 0)) as Vector;
            unit.SetAbsOrigin(vect);
            const effect_fx = ParticleManager.CreateParticle('particles/custom/creature/elite/teleport_start.vpcf', ParticleAttachment.POINT, unit);
            ParticleManager.SetParticleControl(effect_fx, 2, color);
            ParticleManager.ReleaseParticleIndex(effect_fx);

            const effect_fx_end = ParticleManager.CreateParticle(
