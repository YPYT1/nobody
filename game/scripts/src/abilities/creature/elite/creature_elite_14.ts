...
export class modifier_creature_elite_14 extends modifier_generic_arc_lua {
    ...
    GetModifierAura() {
        return 'modifier_creature_elite_14_aura';
    }
    ...
    _OnCreated(kv: any): void {
        ...
        ParticleManager.SetParticleControlEnt(effect_fx, 1, this.GetParent(), ParticleAttachment.POINT_FOLLOW, 'attach_head', Vector(0, 0, 0), true);
        this.AddParticle(effect_fx, false, false, -1, false, false);
    }

    DeclareFunctions(): ModifierFunction[] {
        return [ModifierFunction.DISABLE_TURNING];
    }
...
export class modifier_creature_elite_14_aura extends BaseModifier {
    ...
    OnCreated(params: object): void {
        ...
        const damage = hParent.GetMaxHealth() * 0.4;
        const hAbility = this.GetAbility() as creature_elite_14;
        const final_vect = hAbility.final_vect;
        const speed = hAbility.line_distance;
        const direction = hAbility.direction;
        const distance = ((vCaster - final_vect) as Vector).Length2D();
        const vTarget = (hParent.GetAbsOrigin() + hCaster.GetForwardVector() * distance) as Vector;
