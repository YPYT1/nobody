...
export class creature_elite_7 extends BaseCreatureAbility {
    ...
    OnAbilityPhaseStart(): boolean {
        ...
        const line_width = this.GetSpecialValueFor('line_width');
        const line_distance = this.GetSpecialValueFor('line_distance');
        ...
        for (let i = 0; i < count; i++) {
            ...
            const line_fx = GameRules.WarningMarker.Line(this.hCaster, line_width, this.vOrigin, line_pos, -1, this._cast_point);
    ...
    OnSpellStart(): void {
        ...
        const line_distance = this.GetSpecialValueFor('line_distance');
        for (let i = 0; i < count; i++) {
            const vDirection = ((line_pos - vCaster) as Vector).Normalized();
    ...
    OnProjectileHit(target: CDOTA_BaseNPC | undefined, location: Vector): boolean | void {
        if (target) {
            const damage = target.GetMaxHealth() * 0.2;
            ApplyCustomDamage({
                victim: target,
