...
export class modifier_arms_2 extends BaseModifier {
    ...
    OnCreated(params: object): void {
        ...
        this.extend_a1 = false;
        this.extend_a2 = false;
    }
    ...
    OnIntervalThink(): void {
        ...
        if (this.GetAbility().IsCooldownReady()) {
            ...
            this.caster.AddNewModifier(this.caster, this.ability, "modifier_arms_2_ring_circle", {
                duration: duration,
            })
            // this.caster.AddNewModifier(this.caster, this.ability, "modifier_arms_2_ring_outer", {
            //     duration: duration,
            // })
        }
    }
    ...
    OnIntervalThink(): void {
        ...
        if (this.timer % (this.interval * 100) == 1) {
            ...
            for (let hUnit of enemies) {
                ...
                if ((hUnit.CDResp["arms_2_ring_paralysis"] ?? 0) < gameTime) {
                    hUnit.CDResp["arms_2_ring_paralysis"] = gameTime + 2
...
@registerModifier()
export class modifier_arms_2_ring_circle extends BaseModifier {

    base_raidus: number;
    aura_radius: number;
    ring_buff: modifier_arms_2_ring;
    ...
    GetModifierAura() { return "modifier_arms_2_ring_circle_effect"; }
    ...
    OnCreated(params: object): void {
        ...
        if (!IsServer()) { return }
        this.ring_buff = this.GetCaster().FindModifierByName("modifier_arms_2_ring") as modifier_arms_2_ring;
        this.OnIntervalThink()
        this.StartIntervalThink(0.01)
    }
...
@registerModifier()
export class modifier_arms_2_ring_circle_effect extends BaseModifier {
    ...
    OnCreated(params: object): void {
        if (!IsServer()) { return }
        let buff = this.GetCaster().FindModifierByName("modifier_arms_2_ring");
        if (buff) {
    ...
    OnDestroy(): void {
        if (!IsServer()) { return }
        // 出圈 且施法者得有
        if (this.GetCaster().HasModifier("modifier_arms_2_ring")) {
            ApplyCustomDamage({
                victim: this.GetParent(),
                attacker: this.GetCaster(),
                damage: this.ability_damage,
