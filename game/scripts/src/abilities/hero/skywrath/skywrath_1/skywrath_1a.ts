...
export class skywrath_1a extends skywrath_1 {
    ...
    OnAttackLandAoe(vPos: Vector, extraData: ProjectileExtraData) {
        ...
        for (const enemy of enemies) {
            const is_crit = RollPercentage(this.caster.custom_attribute_value.CriticalChance);
            if (is_crit) {
                bm_crit = 1;
            }
            ApplyCustomDamage({
        ...
        const base_bonus_mul = GameRules.HeroTalentSystem.GetTalentKvOfUnit(this.caster, '60', 'base_bonus_mul');
        if (bm_crit == 1 && base_bonus_mul > 0) {
            ...
            const attack_damage2 = attack_damage * base_bonus_mul * 0.01;
            for (const enemy of enemies) {
                ApplyCustomDamage({
                    victim: enemy,
                    attacker: this.caster,
...
export class modifier_skywrath_1a_yinlei extends BaseModifier {
    ...
    GetModifierAura() {
        return 'modifier_skywrath_1a_yinlei_aura';
    }
    ...
    OnCreated(params: any): void {
        ...
        const cloud_pos = (origin + Vector(0, 0, -500)) as Vector;
...
export class modifier_skywrath_1a_yinlei_aura extends BaseModifier {
    ...
    OnCreated(params: object): void {
        ...
        const yl_thunder_bonus = GameRules.HeroTalentSystem.GetTalentKvOfUnit(this.GetCaster(), '61', 'yl_thunder_bonus');
