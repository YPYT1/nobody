    ...
    OnProjectileHit_ExtraData(target: CDOTA_BaseNPC | undefined, location: Vector, extraData: ProjectileExtraData): boolean | void {
        if (target) {
            ...
            if (this.ylong_max_stack > 0 && target.HasModifier('modifier_skywrath_1c_lystack')) {
                const stack = target.GetModifierStackCount('modifier_skywrath_1c_lystack', this.caster);
            ...
            if (this.ylong_max_stack) {
                const longyin_buff = target.FindModifierByNameAndCaster('modifier_skywrath_1c_lystack', this.caster);
    ...
    PlayPerformAttack2(vDirection: Vector, attack_damage: number) {
        ...
        if (this.caster.clone_unit && this.caster.clone_unit.HasModifier('modifier_skywrath_5_clone_show')) {
            ...
            if (enemies.length > 0) {
                ...
                const vDirection2 = ((clone_target - clone_origin) as Vector).Normalized();
                vDirection2.z = 0;
                ProjectileManager.CreateLinearProjectile({
                    Ability: this.GetAbility(),
...
export class modifier_skywrath_1c_lx_channel extends BaseModifier {
    ...
    OnCreated(params: object): void {
        ...
        const lx_key = this.caster.GetRuneKv('rune_58', 'lx_key');
        if (lx_key == 1) {
    ...
    OnDestroy(): void {
        ...
        const skywrath_1c = this.caster.FindModifierByName('modifier_skywrath_1c') as modifier_skywrath_1c;
