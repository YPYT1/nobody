...
import type { modifier_element_effect_fire } from '../../../../modifier/modifier_element';
...
export class modifier_skywrath_3a_a extends modifier_skywrath_3a {
    ...
    PlayEffect(params: PlayEffectProps) {
        ...
        if (this.CheckClone()) {
            const clone_pos = this.caster.clone_unit.GetOrigin();
...
export class modifier_skywrath_3a_a_channel extends BaseModifier {
    ...
    OnDestroy(): void {
        ...
        if (this.least_time <= GameRules.GetDOTATime(false, false)) {
            // 成功吟唱
            const mdf = this.caster.FindModifierByName('modifier_skywrath_3a_a') as modifier_skywrath_3a_a;
            if (mdf) {
...
export class modifier_skywrath_3a_a_meteor extends BaseModifier {
    ...
    OnIntervalThink(): void {
        // effect
        const effect_fx = ParticleManager.CreateParticle('particles/custom/hero/skywrath3a/sun_strike.vpcf', ParticleAttachment.CUSTOMORIGIN, null);
        ...
        const rune_72 = this.caster.GetRuneKv('rune_72', 'value');
        ...
        for (const enemy of enemies) {
            if (rune_72 > 0 && enemy.HasModifier('modifier_element_effect_fire')) {
                const fire_mdf = enemy.FindModifierByName('modifier_element_effect_fire') as modifier_element_effect_fire;
                if (fire_mdf) {
            ...
            if (enemy.HasModifier('modifier_skywrath_3a_a_fentian')) {
                const stack = enemy.GetModifierStackCount('modifier_skywrath_3a_a_fentian', this.caster);
        ...
        // 89
        const explosion_radius = GameRules.HeroTalentSystem.GetTalentKvOfUnit(this.caster, '89', 'explosion_radius');
        if (explosion_radius > 0) {
            // 引爆火种
            const tinder_damage = GameRules.HeroTalentSystem.GetTalentKvOfUnit(this.caster, '89', 'tinder_damage');
            ...
            const tinder_list = FindUnitsInRadius(
                this.team,
                this.parent_origin,
                null,
                this.radius + 25,
            ...
            for (const tinder of tinder_list) {
                const unitname = tinder.GetUnitName();
                if (unitname == 'npc_summoned_tinder') {
                    const tinder_origin = tinder.GetAbsOrigin();
                    UTIL_Remove(tinder);
        ...
        // 火种
        const tinder_duration = GameRules.HeroTalentSystem.GetTalentKvOfUnit(this.caster, '88', 'tinder_duration');
    ...
    PlayMultiCast() {
        const value = 2;
        const sound_cast = 'Hero_OgreMagi.Fireblast.x1';
    ...
    CreateTinder(tinder_duration: number) {
        const tinder = CreateUnitByName('npc_summoned_tinder', this.parent_origin, false, null, null, this.team);
