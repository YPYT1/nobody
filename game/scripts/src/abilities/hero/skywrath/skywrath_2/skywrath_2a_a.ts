...
export class skywrath_2a_a extends skywrath_2a {
    ...
    UpdataSpecialValue(): void {
        const aoe_radius = this.caster.GetTalentKv('69', 'bz_radius');
    ...
    TriggerActive(params: PlayEffectProps): void {
        ...
        const surround_d_final = this.GetTypesAffixValue(0, 'Surround', 'skv_surround_d_final');
        ...
        const cast_fx = ParticleManager.CreateParticle('particles/custom/hero/skywrath2a/aoe_explosion.vpcf', ParticleAttachment.WORLDORIGIN, null);
...
export class modifier_skywrath_2a_a extends modifier_skywrath_2a {
    ...
    UpdataSpecialValue(): void {
        ...
        const rune60_value = this.caster.GetRuneKv('rune_60', 'value');
        if (rune60_value > 0) {
    ...
    OnIntervalThink() {
        if (this.CastingConditions() && this.count < this.surround_limit) {
            ...
            const total_count = math.min(max_count, this.surround_count);
            // 1个 面向 2个对角 3
            const pre_angle = 360 / total_count;
            const surround_speed = 300; // this.ability.GetTypesAffixValue(300, "Surround", "skv_surround_speed");
            const surround_distance = this.surround_radius; //this.ability.GetTypesAffixValue(, "Surround", "skv_surround_distance")
            for (let i = 0; i < total_count; i++) {
                ...
                if (this.CheckClone()) {
                    const hSpirit2 = GameRules.SummonedSystem.CreatedUnit(
                        'npc_summoned_dummy',
    ...
    DeclareFunctions(): modifierfunction[] {
        return [ModifierFunction.COOLDOWN_REDUCTION_CONSTANT];
    }
...
export class modifier_skywrath_2a_a_surround extends modifier_skywrath_2a_surround {
    ...
    C_OnCreated(params: any): void {
        ...
        const effect_name = 'particles/custom/hero/skywrath2a/surround_orb_fire_2.vpcf';
    ...
    OnDestroy(): void {
        ...
        if (IsValid(this.caster) == false) {
            const main_mdf = this.caster.FindModifierByName('modifier_skywrath_2a_a') as modifier_skywrath_2a_a;
...
export class modifier_skywrath_2a_a_surround_collision extends modifier_skywrath_2a_surround_collision {
    OnCreated_Extends() {
        ...
        if ((this.GetAuraOwner().summoned_damage ?? 0) < GameRules.GetDOTATime(false, false)) {
            ...
            const aoe_multiple = this.GetAbility().GetTypesAffixValue(0, 'Aoe', 'skv_aoe_chance');
            if (RollPercentage(aoe_multiple)) {
                this.GetAbility().MultiCastAoe(vPos);
            }
            const talent71 = GameRules.HeroTalentSystem.GetTalentKvOfUnit(this.caster, '71', 'fb_duration_bonus');
            if (this.GetAuraOwner()) {
                if (talent71 == 0) {
                    const surround_mdf = this.GetAuraOwner().FindModifierByName('modifier_skywrath_2a_a_surround');
