...
export class drow_3a_b extends drow_3a {
    ...
    UpdataSpecialValue(): void {
        const bx_radius = this.caster.GetTalentKv('33', 'radius');
    ...
    TriggerActive(params: PlayEffectProps): void {
        ...
        const ability_damage = params.damage;
...
export class modifier_drow_3a_b extends modifier_drow_3a {
    ...
    ExtraEffect(): void {
        const bx_state = (this.caster.hero_talent['33'] ?? 0) > 0;
        if (bx_state) {
...
export class modifier_drow_3a_b_summoned extends modifier_drow_3a_summoned {
    ...
    C_OnCreated(params: any): void {
        const cast_fx = ParticleManager.CreateParticle('particles/dev/tornado/tornado_4.vpcf', ParticleAttachment.POINT_FOLLOW, this.GetParent());
...
export class modifier_drow_3a_b_summoned_collision extends modifier_drow_3a_summoned_collision {
    ...
    OnIntervalThink(): void {
        const real_damage = ApplyCustomDamage({
...
export class modifier_drow_3a_b_storage extends BaseModifier {
    ...
    OnDestroy(): void {
        ...
        if (hCaster == null || hAbility == null) {
            return;
        }

        const damage = math.min(hCaster.GetAverageTrueAttackDamage(null) * this.bx_limit_pct * 0.01, this.bx_record_dmg);
        const vPos = hCaster.GetAbsOrigin();
        hAbility.TriggerActive({ vPos: vPos, damage: damage });
        const aoe_multiple = hAbility.GetTypesAffixValue(0, 'Aoe', 'skv_aoe_chance');
...
export class modifier_drow_3a_b_cowlofice extends BaseModifier {
    ...
    OnCreated(params: any): void {
        ...
        const bx_radius = params.bx_radius;
