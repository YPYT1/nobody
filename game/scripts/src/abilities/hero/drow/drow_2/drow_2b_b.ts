...
export class drow_2b_b extends drow_2b {
    ...
    OnProjectileHit_ExtraData(target: CDOTA_BaseNPC | undefined, location: Vector, extraData: ProjectileExtraData): boolean | void {
        if (target) {
            ...
            if (this.zc_value > 0) {
                const vOrigin = Vector(extraData.x, extraData.y, 0);
                const vTarget = target.GetAbsOrigin();
                const distance = math.max(this.closest_distance, ((vOrigin - vTarget) as Vector).Length2D());
                // 散射对距离越近的单位造成伤害越高。最近25码
                const dmg_bonus = (1 - (distance - this.closest_distance) / 1000) * this.zc_value;
