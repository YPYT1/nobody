...
export class modifier_drow_4a extends BaseHeroModifier {
    ...
    UpdataAbilityValue(): void {
        const recover_mana = this.ability.GetSpecialValueFor('recover_mana');
        ...
        const recover_hp_pct = GameRules.HeroTalentSystem.GetTalentKvOfUnit(this.caster, '47', 'recover_hp_pct');
    ...
    OnIntervalThink(): void {
        if (this.CastingConditions()) {
            ...
            if (this.recover_hp_pct > 0) {
                // 回血
                const heal_amount = this.caster.GetMaxHealth() * this.recover_hp_pct * 0.01;
            ...
            if (this.caster.rune_level_index.hasOwnProperty('rune_48')) {
                const dmg_reduction = GameRules.RuneSystem.GetKvOfUnit(this.caster, 'rune_48', 'dmg_reduction');
                const duration = GameRules.RuneSystem.GetKvOfUnit(this.caster, 'rune_48', 'duration');
            ...
            if (this.caster.rune_level_index.hasOwnProperty('rune_49')) {
                const bonus_ingame = GameRules.RuneSystem.GetKvOfUnit(this.caster, 'rune_49', 'bonus_ingame');
                const duration = GameRules.RuneSystem.GetKvOfUnit(this.caster, 'rune_49', 'duration');
...
export class modifier_drow_4a_recover_mana extends BaseModifier {
    ...
    OnCreated(params: object): void {
        ...
        // 持续回蓝
        const cast_fx = ParticleManager.CreateParticle('particles/items_fx/healing_clarity.vpcf', ParticleAttachment.ABSORIGIN_FOLLOW, this.caster);
