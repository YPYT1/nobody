    ...
    PlayAttackStart(params: PlayEffectProps): void {
        ...
        if (this.missile_count > 1) {
            for (let i = 0; i < this.missile_count - 1; i++) {
                const vTarget2 = RotatePosition(vCaster, QAngle(0, RandomInt(-30, 30), 0), vTarget);
                const vDirection2 = ((vTarget2 - vCaster) as Vector).Normalized();
                vDirection2.z = 0;
                const vVelocity2 = (vDirection2 * this.proj_speed) as Vector;
