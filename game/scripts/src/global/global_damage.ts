...
function PlayElementHitEffect(hUnit: CDOTA_BaseNPC, element_type?: ElementTypes) {
    ...
    if (element_type == ElementTypes.THUNDER) {
        const hit_fx = ParticleManager.CreateParticle('particles/diy/element_impact_thunder.vpcf', ParticleAttachment.POINT_FOLLOW, hUnit);
...
function ElementSpecialEffect_Fire(hTarget: CDOTA_BaseNPC, hCaster: CDOTA_BaseNPC) {
    const hAbility = hCaster.FindAbilityByName('public_arms');
    const iPlayerID = hCaster.GetPlayerOwnerID();
    let dot_buff = hTarget.FindModifierByNameAndCaster('modifier_element_bond_fire', hCaster);
    const bond_count = GameRules.NewArmsEvolution.ElementBondDateCount[iPlayerID].Element['1'];
    const dot_duration = bond_count >= 6 ? 6 : 3;
    if (dot_buff == null) {
...
function PopupDamageNumber(
    ...
    if (attacker.GetPlayerOwner()) {
        CustomGameEventManager.Send_ServerToPlayer(attacker.GetPlayerOwner(), 'Popup_DamageNumberToClients', {
            data: {
