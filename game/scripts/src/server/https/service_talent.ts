...
export class ServiceTalent extends UIEventRegisterClass {
    ...
    constructor() {
        ...
        for (const key in NpcHeroesCustom) {
            ...
            for (const Tkey in ServerTalentData) {
                const TalentData = ServerTalentData[Tkey as keyof typeof ServerTalentData];
            ...
            if (this.player_server_talent_list[player_id].hasOwnProperty(hero_id_str)) {
                ...
                for (let x = 0; x < this.index_count; x++) {
                    if (this.player_server_talent_list[player_id][hero_id_str][x]) {
                        const c = this.player_talent_list[player_id][hero_id_str][x].u + this.player_talent_list[player_id][hero_id_str][x].y;
                        if (map_level > c) {
    ...
    GetPlayerServerTalent(player_id: PlayerID, params: CGED['ServiceTalent']['GetPlayerServerTalent'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceTalent_GetPlayerServerTalent', {
            data: {
    ...
    GetPlayerServerTalentByHero(player_id: PlayerID, params: CGED['ServiceTalent']['GetPlayerServerTalentByHero'], callback?) {
        ...
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceTalent_GetPlayerServerTalentByHero', {
            data: {
    ...
    LoadPlayerServerTalent(player_id: PlayerID, Data: { [hero_id: number]: string }) {
        const player_map_level = GameRules.ServiceInterface.player_map_level[player_id].level;
        for (const key in Data) {
            const hero_id: number = tonumber(key);
            if (Data[hero_id] != '') {
    ...
    ClickTalent(player_id: PlayerID, params: CGED['ServiceTalent']['ClickTalent'], callback?) {
        ...
        const ti = params.index;
        const TalentData = ServerTalentData[key as keyof typeof ServerTalentData];
        const hero_id = tostring(TalentData.hero_id);
        const tier = TalentData.tier_number;
        const tier_str = tostring(TalentData.tier_number);
        const parent_node = TalentData.parent_node;
        const max = TalentData.max_number; //最多点几点
                    ...
                    if (this.player_talent_list[player_id][hero_id][ti].i[tier_str].k[key].uc < max) {
                        ...
                        if (tier == 0) {
                            ...
                            if (this.player_talent_list[player_id][hero_id][ti].i[tier_str].k[key].uc >= this.basic_input) {
                                const letcount = Object.keys(this.player_talent_node_config[hero_id]).length;
                                for (let key_tier_number = 0; key_tier_number < letcount; key_tier_number++) {
                                    const key_tier_str = tostring(key_tier_number);
                                    if (key_tier_number != 0) {
                                        const us_key = this.player_talent_node_config[hero_id][key_tier_number][1];
                        ...
                        } else if (tier > 0 && parent_node > 0 && parent_node < 8) {
                            //中间层 下层解锁
                            if (this.player_talent_list[player_id][hero_id][ti].i[tier_str].k[key].uc == this.or_input) {
                                const us_key = this.player_talent_node_config[hero_id][tier][parent_node + 1];
                        ...
                        if (this.player_talent_list[player_id][hero_id][ti].i[tier_str].c == this.count_input) {
                            const us_key = this.player_talent_node_config[hero_id][tier][9];
                            if (!this.player_talent_list[player_id][hero_id][ti].i[tier_str].k.hasOwnProperty(us_key)) {
                                this.player_talent_list[player_id][hero_id][ti].i[tier_str].k[us_key] = {
                                    uc: 0,
                                };
    ...
    SaveTalentConfig(player_id: PlayerID, params: CGED['ServiceTalent']['SaveTalentConfig'], callback?) {
        ...
        const bcdata = JSON.encode(this.player_talent_list[player_id]);
    ...
    RestoreTalentConfig(player_id: PlayerID, params: CGED['ServiceTalent']['RestoreTalentConfig'], callback?) {
        const hero_id = tostring(params.hero_id);
        const ti = params.index;
        if (!this.player_talent_list[player_id][hero_id]) {
            GameRules.CMsg.SendErrorMsgToPlayer(player_id, '还原天赋:未找到英雄...');
        } else {
            this.player_talent_list[player_id][hero_id][ti] = CustomDeepCopy(
                this.player_server_talent_list[player_id][hero_id][ti]
            ) as CGEDGetTalentListInfo;
        }
    ...
    EmptyTalentOfPlayer(player_id: PlayerID, index: number, hero_id: number) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceTalent_EmptyTalentOfPlayer', {
            data: {
                index: index,
                hero_id: hero_id,
            },
    ...
    GetSelectTalentIndex(player_id: PlayerID, params: CGED['ServiceTalent']['GetSelectTalentIndex'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceTalent_GetSelectTalentIndex', {
            data: {
