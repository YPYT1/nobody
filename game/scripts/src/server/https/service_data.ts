...
export class ServiceData extends UIEventRegisterClass {
    ...
    constructor() {
        ...
        for (let index = 0; index < 6; index++) {
            ...
            for (let i_n = 1; i_n <= 5; i_n++) {
                //初始化空数据
                const item_id = 1000 + i_n;
                const item_id_str = tostring(item_id);
        ...
        for (const key in PictuerCardData) {
            const CardData = PictuerCardData[key as keyof typeof PictuerCardData];
    ...
    GetMonsterPackageIndexAndCount(player_id: PlayerID, item_id: number): { count: number; index: number } {
        const ret: { count: number; index: number } = {
            count: 0,
            index: -1,
        };
        //检查卡片是否充足
        const monster_package = GameRules.ServiceData.server_monster_package_list[player_id];
    ...
    AddPackageMonsterItem(player_id: PlayerID, id: string, item_id: number, customs: string, count: number): boolean {
        const check = this.GetMonsterPackageIndexAndCount(player_id, item_id);
        if (check.index != -1) {
    ...
    DeletePackageMonsterItemServer(player_id: PlayerID, item_id: number, count: number, id?: string): boolean {
        let c_index = -1;
        const item_package = GameRules.ServiceData.server_monster_package_list[player_id];
    ...
    AddPackageItem(player_id: PlayerID, id: string, item_id: number, customs: string, count: number): boolean {
        const item_id_string = tostring(item_id);
        const merge = ServerItemList[item_id_string as keyof typeof ServerItemList].merge;
        if (merge != 1) {
            const check = this.VerifyPackageItem(player_id, item_id);
    ...
    VerifyPackageItem(player_id: PlayerID, item_id: number, count?: number, id?: string): { is_verify: boolean; index: number } {
        //检查卡片是否充足
        const ret: { is_verify: boolean; index: number } = {
            is_verify: false,
            index: -1,
        };
        const item_package = GameRules.ServiceData.server_package_list[player_id];
        const item_id_string = tostring(item_id);
        const merge = ServerItemList[item_id_string as keyof typeof ServerItemList].merge;
        if (merge != 1) {
            for (let index = 0; index < item_package.length; index++) {
    ...
    GoldVerifyPackageItem(player_id: PlayerID, item_id: number, count?: number, id?: string): { is_verify: boolean; index: number } {
        ...
        const item_package = GameRules.ServiceData.server_gold_package_list[player_id];
        const item_id_string = tostring(item_id);
    ...
    GoldPackageUpData(player_id: PlayerID, item_id: number, count: number): boolean {
        ...
        if (item_id == 1004) {
            //更新经验相关的数据 必须优先更新
            const new_exp = GameRules.ServiceData.server_gold_package_list[player_id][item_id_string].number + count;
    ...
    LoadPlayerServerAttr(player_id: PlayerID) {
        ...
        const hero_id = selfhHero.GetHeroID();
        const hero_id_str = tostring(hero_id);
        //加载天赋属性
        const select_talent_index = GameRules.ServiceTalent.select_talent_index;
        const talent_data = GameRules.ServiceTalent.player_server_talent_list[player_id][hero_id_str][select_talent_index].i;
        for (let index = 0; index < 6; index++) {
            const index_str = tostring(index);
            if (talent_data.hasOwnProperty(index_str)) {
                for (const key in talent_data[index_str].k) {
                    if (talent_data[index_str].k[key].uc > 0) {
                        const attr = ServerTalentData[key as keyof typeof ServerTalentData].ObjectValues as CustomAttributeTableType;
        ...
        //加载图鉴属性
        const pictuer_count = GameRules.ServiceData.server_player_config_pictuer_fetter[player_id][0];
        for (const count_id of pictuer_count) {
            const length = GameRules.ServiceData.server_pictuer_fetter_list[player_id][count_id].length;
            //属性值
            const ListValues = PictuerFetterConfig[count_id as keyof typeof PictuerFetterConfig].ListValues as {
                [key: string]: CustomAttributeTableType;
            };
            //消耗
            const consume = PictuerFetterConfig[count_id as keyof typeof PictuerFetterConfig].consume;
            if (length == consume) {
                const ability_id_list = PictuerFetterConfig[count_id as keyof typeof PictuerFetterConfig].ability_id;
                for (const element of ability_id_list) {
                    if (element != 'null') {
                        const le = element.split('_');
                        const ab_key = tonumber(le[0]);
                        const ab_level = tonumber(le[1]);
            ...
            for (let index = 1; index <= length; index++) {
                const attr = ListValues[index.toString()];
                for (const key1 in attr) {
                    for (const key2 in attr[key1]) {
                        if (!attr_count.hasOwnProperty(key1)) {
                            attr_count[key1] = {};
        ...
        //加载魂石属性
        const SoulList = GameRules.ServiceSoul.soul_list[player_id].i;
        for (const s_key in SoulList) {
            for (const d_data of SoulList[s_key].d) {
                const soul_key = d_data.k;
                const soul_value = d_data.v;
                const MainProperty = ServerSoulAttr[soul_key as keyof typeof ServerSoulAttr].MainProperty;
                const TypeProperty = ServerSoulAttr[soul_key as keyof typeof ServerSoulAttr].TypeProperty;
    ...
    Debug(cmd: string, args: string[], player_id: PlayerID): void {
        ...
        if (cmd == '!!dg') {
            const CardDataList = Object.keys(PictuerCardData);
            const count = tonumber(args[0] ?? '50');
            for (let index = 1; index <= count; index++) {
                const i = RandomInt(0, CardDataList.length - 1);
                let tos = 0;
                if (args[1]) {
                    tos = tonumber(args[1]);
                } else {
