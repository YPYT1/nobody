...
import * as ServerEquipDeputy from "../../json/config/server/equip/server_equip_deputy.json";
...
export class ServiceEquipment extends UIEventRegisterClass {
    ...
    //装备配置
    player_equi_config : CGEDEquipConfigInfo[] = [];
    //server装备配置
    server_player_equi_config: CGEDEquipConfigInfo[] = [];
    //装备生成唯一标识
    ...
    AddEquipByDifficulty(player_id: PlayerID , difficulty_index: string ) {
        let add_equip_list: ServerEquip[] = [];
        let count = 0;
        let DropCount = ServerEquipDropCount[difficulty_index as keyof typeof ServerEquipDropCount];
        ...
        if(count > 0){
            for (let index = 0; index < count; index++) {
                ...
                //tobo 品质提升物品在这里处理
                let equip_data = ServerEquipInfo[equip_key as keyof typeof ServerEquipInfo];
                let main_attr_count = this.AttrCountFunc(equip_r , equip_data.main_attr_random_initial , equip_data.main_attr_random_initial_pro );
                let main_attr_list: { //主属性词条信息
                    k: string, //键
                    v: number, //值
    ...
    AttrCountFunc(equip_r : number, attr_random_initial : number[], attr_random_initial_pro : number[]) : number{
        ...
        if (typeof attr_random_initial[equip_r] == "number") {
            ...
            let main_count_string = attr_random_initial[equip_r].toString();
            if (main_count_string.includes(",")) {
                let main_count_list = main_count_string.split(",");
                let main_count_main_list: number[] = [];
                if (attr_random_initial_pro[equip_r] == 0) {
                    main_attr_count = tonumber(main_count_list[RandomInt(0, main_count_list.length - 1)]);
                } else {
                    let main_count_p_string = attr_random_initial_pro[equip_r].toString();
                    for (let v of main_count_p_string.split(",")) {
     ...
     */
    MainAttrNumber(p_count : number , main_attr_random_key : string ) : {
        k: string, //键
        v: number, //值
    }[]{
        let p_ct_list: { //主属性词条
            ...
            if (NEW_Main_Attr_SEAEKEY.length > 0) {
                ...
                if (p_count >= NEW_Main_Attr_SEAEKEY.length) {
                    for (let p_i = 0; p_i < NEW_Main_Attr_SEAEKEY.length; p_i++) {
                        ...
                        let attr_key = NEW_Main_Attr_SEAEKEY[p_i];
                        if (wp_list.includes(attr_key)) {
                            //跳过本次 
                            p_i--;
                            continue;
                        }
                        let SEAE_data = ServerEquipMainAttr[attr_key as keyof typeof ServerEquipMainAttr];
                    ...
                    for (let p_i = 0; p_i < p_count; p_i++) {
                        ...
                        let attr_key_index = GetCommonProbability(NEW_Main_Attr_SEAEP);
                        let attr_key = NEW_Main_Attr_SEAEKEY[attr_key_index];
                        if (wp_list.includes(attr_key)) {
                            //跳过本次 
                            p_i--;
                            continue;
     ...
     * @returns 
     */
    PuzzleAttrNumber(p_count : number , puzzle_attr_random_key : string )  : {
        k : string, //键
        v : number, //值
        l : number, //拼图等级
    }[]{
        let p_ct_list: { //拼图属性词条
            k: string, //键
            v: number, //值
            l: number, //拼图等级
        }[] = [];
            ...
            if (Attr_SEAEKEY.length > 0) {
                ...
                if (p_count >= Attr_SEAEKEY.length) {
                    ...
                    for (let p_i = 0; p_i < p_count; p_i++) {
                        ...
                        let attr_key_index = GetCommonProbability(Attr_SEAEP);
                        let attr_key = Attr_SEAEKEY[attr_key_index];
                        if (wp_list.includes(attr_key)) {
                            //跳过本次 
                            p_i--;
                            continue;
    ...
    // //获取玩家装备配置
    // GetEquipConfig(player_id: PlayerID, params: GEFPD["ServiceEquipment"]["GetEquipConfig"], callback?) {
    //     CustomGameEventManager.Send_ServerToPlayer(
    //         PlayerResource.GetPlayer(player_id),
    //         "ServiceEquipment_GetEquipConfig",
    //         {
    //             data: {
    //                 server: this.server_player_equi_config[player_id],
    //                 local: this.player_equi_config[player_id]
    //             }
    //         }
    //     );
    // }
    // //获取玩家背包装备列表
    // GetEquipList(player_id: PlayerID, params: GEFPD["ServiceEquipment"]["GetEquipList"], callback?) {
    //     CustomGameEventManager.Send_ServerToPlayer(
    //         PlayerResource.GetPlayer(player_id),
    //         "ServiceEquipment_GetEquipList",
    //         {
    //             data: {
    //                 list: this.player_equi_list[player_id]
    //             }
    //         }
    //     );
    // }
    ...
    // //装备强化
    // EquipIntensify(player_id: PlayerID, params: GEFPD["ServiceEquipment"]["EquipIntensify"], callback?) {
    //     let eq_id = params.id;
    //     let equi_data = this.player_equi_list[player_id][eq_id];
    //     let server_log = "QH_"
    //     if (equi_data.id != eq_id) {
    //         GameRules.CMsg.SendErrorMsgToPlayer(player_id, "装备强化:装备信息错误...");
    //         GameRules.ServiceEquipment.GetEquipList(player_id, {});
    //     } else {
    //         if (equi_data.zl <= equi_data.l) {
    //             GameRules.CMsg.SendErrorMsgToPlayer(player_id, "装备强化:强化已到上限...");
    //             GameRules.ServiceEquipment.GetEquipList(player_id, {});
    //         } else {
    //             let EquipIntensifyConfig = ServerNewEquipIntensify[(equi_data.l + 1).toString() as keyof typeof ServerNewEquipIntensify];
    //             let e_d = ServerEquipInfo[equi_data.n as keyof typeof ServerEquipInfo];

    //             let need_materials = EquipIntensifyConfig.class_level
    //             // if (e_d.class_level == 5) {
    //             //     need_materials = EquipIntensifyConfig["class_level_5"];
    //             // } else if (e_d.class_level == 10) {
    //             //     need_materials = EquipIntensifyConfig["class_level_10"];
    ...
    //                     if(item_id == 8 ){ //箭魂特殊处理
    //                         if (GameRules.ArchiveService.player_currency[player_id].jian_hun < need_list[item_id]) {
    //                             GameRules.CMsg.SendErrorMsgToPlayer(player_id, "装备强化:箭魂不足");
    //                             GameRules.ServiceEquipment.GetEquipList(player_id, {});
    //                             return;
    //                         }
    //                         continue
    ...
    //                     if (record_index != -1) {
    //                         if (GameRules.ArchiveService.server_package_list[player_id][record_index].number < need_list[item_id]) {
    //                             GameRules.CMsg.SendErrorMsgToPlayer(player_id, "装备强化:强化材料不足");
    //                             GameRules.ServiceEquipment.GetEquipList(player_id, {});
    //                             return;
    //                         }
    //                     } else {
    //                         GameRules.CMsg.SendErrorMsgToPlayer(player_id, "装备强化:强化材料无");
    //                         GameRules.ServiceEquipment.GetEquipList(player_id, {});
    //                         return;
    //                     }
    //                 }
    ...
    //             server_log += "CL:" + consume + ";";
    //             let i_type: number = 0;
    //             if ((EquipIntensifyConfig.success_rate + new_equi_data.ei) >= 100 || RollPercentage(EquipIntensifyConfig.success_rate + new_equi_data.ei)) {
    //                 new_equi_data.l++;
    //                 new_equi_data.ei = 0;
    //                 i_type = 1;
    //             } else {
    //                 new_equi_data.ei++;
    //                 if (EquipIntensifyConfig.demotion_rate > 0) {
    //                     if (RollPercentage(EquipIntensifyConfig.demotion_rate)) {
    //                         new_equi_data.l--;
    //                         i_type = 2;
    //                     }
    //                 }
    ...
    // }
    // //安装装备
    // InstallEquip(player_id: PlayerID, params: GEFPD["ServiceEquipment"]["InstallEquip"], callback?) {
    //     let eq_id = params.id;
    //     let equi_data = this.player_equi_list[player_id][eq_id];
    //     if (equi_data.id != eq_id) {
    //         GameRules.CMsg.SendErrorMsgToPlayer(player_id, "穿戴装备:装备信息错误...");
    //     } else {
    //         if (params.type == 1) { //通用穿戴
    //             let e_d = ServerEquipInfo[equi_data.n as keyof typeof ServerEquipInfo];
    //             this.player_equi_config[player_id].public[params.t - 1][e_d.equi_type] = equi_data.id;

    //         } else if (params.type == 2) { //专用穿戴
    //             if (!params.hero_name) {
    //                 GameRules.CMsg.SendErrorMsgToPlayer(player_id, "穿戴装备:角色错误....");
    //             } else {
    //                 if (!this.player_equi_config[player_id].hero[params.hero_name]) {
    //                     GameRules.CMsg.SendErrorMsgToPlayer(player_id, "穿戴装备:未找到角色...");
    //                 } else {
    //                     let e_d = ServerEquipInfo[equi_data.n as keyof typeof ServerEquipInfo];
    //                     this.player_equi_config[player_id].hero[params.hero_name][params.t - 1][e_d.equi_type] = equi_data.id;
    //                 }
    //             }
    //         } else {
    //             GameRules.CMsg.SendErrorMsgToPlayer(player_id, "穿戴装备:配置选项错误...");
    //         }
    //     }
    //     this.GetEquipConfig(player_id, {});
    // }
    // //保存装备
    // SaveEquipConfig(player_id: PlayerID, params: GEFPD["ServiceEquipment"]["SaveEquipConfig"], callback?) {
    //     GameRules.ArchiveService.EquipCfgModify(player_id, this.player_equi_config[player_id]);
    // }
    // //还原装备配置装备
    // RestoreEquipConfig(player_id: PlayerID, params: GEFPD["ServiceEquipment"]["RestoreEquipConfig"], callback?) {
    //     if (params.type == 1) { //通用穿戴
    //         this.player_equi_config[player_id].public[params.t - 1] =
    //             CustomDeepCopy(this.server_player_equi_config[player_id].public[params.t - 1]) as string[];
    //     } else if (params.type == 2) { //专用穿戴
    //         if (!params.hero_name) {
    //             GameRules.CMsg.SendErrorMsgToPlayer(player_id, "还原配装:角色错误....");
    //         } else {
    //             if (!this.player_equi_config[player_id].hero[params.hero_name]) {
    //                 GameRules.CMsg.SendErrorMsgToPlayer(player_id, "还原配装:未找到角色...");
    //             } else {
    //                 this.player_equi_config[player_id].hero[params.hero_name][params.t - 1] =
    //                     CustomDeepCopy(this.server_player_equi_config[player_id].hero[params.hero_name][params.t - 1]) as string[];
    //             }
    //         }
    //     } else {
    //         GameRules.CMsg.SendErrorMsgToPlayer(player_id, "还原配装:配置选项错误...");
    //     }
    //     this.GetEquipConfig(player_id, {});
    // }
    // //解除装备
    // UninstallEquip(player_id: PlayerID, params: GEFPD["ServiceEquipment"]["UninstallEquip"], callback?) {
    //     if (params.type == 1) { //通用穿戴
    //         this.player_equi_config[player_id].public[params.t - 1][params.equip_type] = "";
    //     } else if (params.type == 2) { //专用穿戴
    //         if (!params.hero_name) {
    //             GameRules.CMsg.SendErrorMsgToPlayer(player_id, "解除装备:角色错误....");
    //         } else {
    //             if (!this.player_equi_config[player_id].hero[params.hero_name]) {
    //                 GameRules.CMsg.SendErrorMsgToPlayer(player_id, "解除装备:未找到角色...");
    //             } else {
    //                 this.player_equi_config[player_id].hero[params.hero_name][params.t - 1][params.equip_type] = "";
    //             }
    //         }
    //     } else {
    //         GameRules.CMsg.SendErrorMsgToPlayer(player_id, "解除装备:配置选项错误...");
    //     }
    //     this.GetEquipConfig(player_id, {});
    // }
    // //分解数量
    // GetResolveEquipItems( player_id :PlayerID , l_list : {
    //     [index : string] : string
    // }) : {
    ...
    //         let n = this.player_equi_list[player_id][l_list[equip_key]].n;
    //         let l = this.player_equi_list[player_id][l_list[equip_key]].l;
    //         let r = this.player_equi_list[player_id][l_list[equip_key]].r;
    //         let zl = this.player_equi_list[player_id][l_list[equip_key]].zl;
    //         //副职
    //         let t = this.player_equi_list[player_id][l_list[equip_key]].t;
    //         //技能
    //         let w = this.player_equi_list[player_id][l_list[equip_key]].w;
    //         //套装
    //         let wt = this.player_equi_list[player_id][l_list[equip_key]].wt;
    //         let em22 = this.player_equi_list[player_id][l_list[equip_key]].em22;
    //         let em23 = this.player_equi_list[player_id][l_list[equip_key]].em23;
    //         let em24 = this.player_equi_list[player_id][l_list[equip_key]].em24;
    ...
    //         //     //     }
    //         //     // }
    //         //     let EquipIntensifyConfig = SEIntensifyLevelConfig[zl.toString() as keyof typeof SEIntensifyLevelConfig]
    //         //     let lv_fj_item_list = EquipIntensifyConfig.level_materials as string[];
    //         //     for (const lv_item_str of lv_fj_item_list) {
    //         //         if (lv_item_str.includes("_")) {
    //         //             let item_list = lv_item_str.split("_");
    ...
    //         //         if (ret_data.add_item_list.hasOwnProperty(key)) {
    //         //             ret_data.add_item_list[key] += str_item_list[key] * 0.1;
    //         //         } else {
    //         //             ret_data.add_item_list[key] = str_item_list[key] * 0.1;
    //         //         }
    //         //     }
    ...
    //         //     for (const key in str_item_list) {
    //         //         if (ret_data.add_item_list.hasOwnProperty(key)) {
    //         //             ret_data.add_item_list[key] += str_item_list[key] * 0.5;
    //         //         } else {
    //         //             ret_data.add_item_list[key] = str_item_list[key] * 0.5;
    //         //         }
    //         //     }
    ...
    //                     let intensify_count = 2;
    //                     if(intensify_count > 0){
    //                         let item_id = ServerEquipDemonSuitContras[wt[0].k as keyof typeof  ServerEquipDemonSuitContras].item_id + 1
    //                         if (str_item_list.hasOwnProperty(item_id)) {
    //                             str_item_list[item_id] += intensify_count;
    //                         } else {
    //                             str_item_list[item_id] = intensify_count;
    ...
    //                     let intensify_count = 2;
    //                     if(intensify_count > 0){
    //                         let item_id = ServerEquipDemon[w[0].k as keyof typeof  ServerEquipDemon].item_id + 1
    //                         if (str_item_list.hasOwnProperty(item_id)) {
    //                             str_item_list[item_id] += intensify_count;
    //                         } else {
    //                             str_item_list[item_id] = intensify_count;
    ...
    //         }
    //         if(em23 > 0 ){
    //             let item_18 = math.ceil(em23 * 0.5)
    ...
    //         }
    //         if(em24 > 0 ){
    //             let item_19 = math.ceil(em24 * 0.5)
    ...
    //  * @param callback 
    //  */
    // GetResolveEquipData(player_id: PlayerID, params: GEFPD["ServiceEquipment"]["GetResolveEquipData"], callback?){
    //     let id_list = params.id_list as any;
    //     let id_list_obj = id_list as {
    //         [index : string] : string
    //     }
    //     let ret_data = this.GetResolveEquipItems(player_id , id_list_obj);
    //     let add_item_list = ret_data.add_item_list;
    //     CustomGameEventManager.Send_ServerToPlayer(
    //         PlayerResource.GetPlayer(player_id),
    ...
    //  * @param callback 
    //  */
    // ResolveEquip(player_id: PlayerID, params: GEFPD["ServiceEquipment"]["ResolveEquip"], callback?) {
    //     let id_list = params.id_list as any;
    //     let id_list_obj = id_list as {
    //         [index : string] : string
    //     }
    ...
    //     let add_list_string = "";
    //     let server_log = "FJ_"
    //     let ResolveEquipData = this.GetResolveEquipItems(player_id , id_list_obj)
    //     let add_list_count = ResolveEquipData.add_list_count;
    //     let add_item_list = ResolveEquipData.add_item_list;
    //     let del_list_id = ResolveEquipData.del_list_id;
    //     if (del_list_id != "") {
    //         for (const key in add_item_list) {
    //             add_list_string += add_list_string == "" ? key + "_" + math.floor(add_item_list[key]) : "," + key + "_" + math.floor(add_item_list[key]);
    //         }
