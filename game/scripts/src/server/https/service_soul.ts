...
export class ServiceSoul extends UIEventRegisterClass {
    ...
    constructor() {
        ...
        for (const key in ServerSoulAttr) {
            const spa_data = ServerSoulAttr[key as keyof typeof ServerSoulAttr];
        ...
        for (const key in ServerSoulConfig) {
            const data = ServerSoulConfig[key as keyof typeof ServerSoulConfig];
            if (data.type == 1) {
    ...
    SoulAddOfField(player_id: PlayerID, params: CGED['ServiceSoul']['SoulAddOfField']) {
        const box_type = params.box_type;
        const key = params.key;
        if (box_type > 0 && box_type <= this.player_box_type_count) {
            if (this.soul_list[player_id].i.hasOwnProperty(box_type)) {
                const r_data = CustomDeepCopy(this.soul_list[player_id].i[box_type].d) as CGEDGetSoulListData[];
                const r_data_key_list = Object.keys(r_data);
                const max = math.min(math.floor(GameRules.ServiceInterface.player_map_level[player_id].level / 10), this.player_xq_count);
                if (r_data_key_list.length < max) {
                    ...
                    if (is_meiyou) {
                        //获取数据...
                        const ret = this.SoulDataUp(key, 0, 0);
                        if (ret.code == true) {
                            const Sj_config_key = this.level_u_d_config.up[0];
                            const need_item = this.GetUpItem(Sj_config_key, key, 1);
                            let red_item_str = '';
                            for (const need_item_key in need_item) {
                                const need_item_id = need_item_key;
                                const need_item_count = need_item[need_item_id];
                                if (red_item_str == '') {
                                ...
                                const ret_ver = GameRules.ServiceData.SelectVerifyPackageItem(player_id, tonumber(need_item_id), need_item_count);
                            ...
                            const new_soul_list = CustomDeepCopy(this.soul_list[player_id]) as CGEDGetSoulList;
                            new_soul_list.i[box_type].d.push(ret.data);
                            new_soul_list.i[box_type].l++;
                            new_soul_list.i[box_type].z++;
                            const new_soul_list_str = JSON.encode(new_soul_list);
                            GameRules.ArchiveService.PlayerSoulStoneSave(player_id, new_soul_list_str, red_item_str);
                        } else {
    ...
    SoulIntensify(player_id: PlayerID, params: CGED['ServiceSoul']['SoulIntensify']) {
        ...
        const ext_item = params.ext_item;
        if (box_type > 0 && box_type <= this.player_box_type_count) {
            if (this.soul_list[player_id].i.hasOwnProperty(box_type)) {
                if (this.soul_list[player_id].i[box_type].d[index]) {
                    const r_data = CustomDeepCopy(this.soul_list[player_id].i[box_type].d[index]) as CGEDGetSoulListData;
                    const level = r_data.l;
                    const value = r_data.v;
                    const key = r_data.k;
                    const l = this.soul_list[player_id].i[box_type].l;
                    if (type == 1) {
                        if (level >= 5) {
                    ...
                    if (ret.code == true) {
                        ...
                        const need_item = this.GetUpItem(Sj_config_key, key, type);
                        if (ext_item) {
                            if (ext_item == 1283) {
                                // 低级魂石升级保护卷
                                need_item[ext_item] = 1;
                                ext_pro = 5;
                            } else if (ext_item == 1284) {
                                // 中级魂石升级保护卷
                                need_item[ext_item] = 1;
                                ext_pro = 10;
                            } else if (ext_item == 1285) {
                                // 高级魂石升级保护卷
                                need_item[ext_item] = 1;
                                ext_pro = 15;
                            }
                        ...
                        let red_item_str = '';
                        for (const need_item_key in need_item) {
                            const need_item_id = tonumber(need_item_key);
                            const need_item_count = need_item[need_item_key];
                            if (red_item_str == '') {
                                red_item_str = need_item_key + '_' + need_item_count;
                            } else {
                                red_item_str += ',' + need_item_key + '_' + need_item_count;
                            ...
                            if (type == 1) {
                                ...
                                if (set_item_id >= 10000 && set_item_id < 20000) {
                                    const ServerSoulConfigDataConsume = ServerSoulConfig[Sj_config_key as keyof typeof ServerSoulConfig].consume;
                                    const con_data = ServerSoulConfigDataConsume.split('_');
                                    if (con_data[0] == '1') {
                                        set_item_id = 1287;
                            ...
                            if (ret.data.c.hasOwnProperty(need_item_key)) {
                                ret.data.c[need_item_key] += need_item_count;
                            } else {
                                ret.data.c[need_item_key] = need_item_count;
                            }
                            const ret_ver = GameRules.ServiceData.SelectVerifyPackageItem(player_id, need_item_id, need_item_count);
                            if (!ret_ver.is_verify) {
                                GameRules.CMsg.SendErrorMsgToPlayer(player_id, '魂石功能:材料不足...');
                                return;
                            }
                        ...
                        //计算概率
                        const pro = ServerSoulConfig[Sj_config_key as keyof typeof ServerSoulConfig].pro + ext_pro;
                        const new_soul_list = CustomDeepCopy(this.soul_list[player_id]) as CGEDGetSoulList;
                        if (RollPercentage(pro)) {
                            new_soul_list.i[box_type].d[index] = ret.data;
                            if (type == 1) {
                                new_soul_list.i[box_type].z++;
                                if (new_soul_list.i[box_type].z > new_soul_list.i[box_type].l) {
                                    new_soul_list.i[box_type].l = new_soul_list.i[box_type].z;
                                }
                            } else {
                                new_soul_list.i[box_type].z--;
                            }
                        ...
                        GameRules.ArchiveService.PlayerSoulStoneSave(player_id, new_soul_list_str, red_item_str, post_type, '', ret.value);
                    } else {
                        GameRules.CMsg.SendErrorMsgToPlayer(player_id, '魂石功能:升级失败...');
                    }
                } else {
    ...
    SoulDataUp(key: string, level: number = 0, value: number = 0): { code: boolean; data: CGEDGetSoulListData; value: number } {
        const ret: { code: boolean; data: CGEDGetSoulListData; value: number } = {
            code: true,
            data: {
                k: key,
                l: 0,
        ...
        const SoulAttr_data = ServerSoulAttr[key as keyof typeof ServerSoulAttr];
        const float = SoulAttr_data.float;
        const up_value = SoulAttr_data.up_value;
        let value_max = SoulAttr_data.value_max_5;
        const level_max = SoulAttr_data.level_max;
        ...
        if (level > 0) {
            // 1-4
            const value_per = SoulAttr_data.value_per_1_5;
        ...
        if (level > 4) {
            //5-9
            const value_per = SoulAttr_data.value_per_6_10;
            value_max = SoulAttr_data.value_max_10;
            if (level >= 9) {
                per += value_per * 5;
            } else {
                const cz_level = level - 4;
        ...
        if (level > 9) {
            // 10-14
            const value_per = SoulAttr_data.value_per_11_15;
            value_max = SoulAttr_data.value_max_15;
            if (per >= 14) {
                per += value_per * 5;
            } else {
                const cz_level = level - 9;
                per += value_per * cz_level;
            }
        }
        ...
        if (level > 14) {
            // 15-19
            const value_per = SoulAttr_data.value_per_16_20;
            value_max = SoulAttr_data.value_max_20;
            if (per >= 19) {
                per += value_per * 5;
            } else {
                const cz_level = level - 14;
                per += value_per * cz_level;
            }
        }

        const newSection = this.SectionPer(up_value, float, per);
        const add_value = this.ZoomNumber(newSection, float);
    ...
    SoulDataDrop(key: string, level: number = 0, value: number = 0): { code: boolean; data: CGEDGetSoulListData; value: number } {
        ...
        const SoulAttr_data = ServerSoulAttr[key as keyof typeof ServerSoulAttr];
        const float = SoulAttr_data.float;
        const drop_value = SoulAttr_data.drop_value;
        const value_min = 0;
        if (level - 1 < 0) {
            return ret;
        }
        ...
        if (level > 15) {
            ...
            if (per >= 20) {
                ...
                const cz_level = level - 15;
                per += value_per * cz_level;
            }
        }

        const newSection = this.SectionPer(drop_value, float, per);
        const sub_value = this.ZoomNumber(newSection, float);
    ...
    GetUpItem(
        ...
        const SoulUpData = ServerSoulConfig[key as keyof typeof ServerSoulConfig];
        const consume = SoulUpData.consume;
        const items = SoulUpData.items;

        if (consume != 'null') {
            const con_data = consume.split('_');
            if (type == 1) {
                const item_id_list = ServerSoulAttr[attr_key as keyof typeof ServerSoulAttr].item_id;
                const section = tonumber(con_data[0]) - 1;
                const get_item_id = tostring(item_id_list[section]);
        ...
        for (const element of items) {
            if (element != 'null') {
                const item_data = element.split('_');
                const item_id = item_data[0];
                const item_count = tonumber(item_data[1]);
    ...
    GetPlayerServerSoulData(player_id: PlayerID, params: CGED['ServiceSoul']['GetPlayerServerSoulData']) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceSoul_GetPlayerServerSoulData', {
            data: {
    ...
    DeforehandSoulDelete(player_id: PlayerID, params: CGED['ServiceSoul']['DeforehandSoulDelete']) {
        ...
        if (this.soul_list[player_id].i.hasOwnProperty(box_type)) {
            if (this.soul_list[player_id].i[box_type].d[index]) {
                const items = this.soul_list[player_id].i[box_type].d[index].c;
                const new_items = this.GetDeleteItems(items);
                CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceSoul_DeforehandSoulDelete', {
                    data: {
                        itemsdata: new_items,
                        box_type: box_type,
                        index: index,
        ...
        if (this.soul_list[player_id].i.hasOwnProperty(box_type)) {
            if (this.soul_list[player_id].i[box_type].d[index]) {
                ...
                for (const key in new_items.list) {
                    const count = new_items.list[key];
                    if (add_item_str == '') {
                        add_item_str = key + '_' + count;
                    } else {
                        add_item_str += ',' + key + '_' + count;
                    }
                }
                const new_soul_list = CustomDeepCopy(this.soul_list[player_id]) as CGEDGetSoulList;
                const level = new_soul_list.i[box_type].d[index].l;
                //删除当前等级
                new_soul_list.i[box_type].z -= level;
                //删除符文数据
                new_soul_list.i[box_type].d.splice(index, 1);
                const new_soul_list_str = JSON.encode(new_soul_list);
                GameRules.ArchiveService.PlayerSoulStoneSave(player_id, new_soul_list_str, '', 4, add_item_str);
            } else {
                GameRules.CMsg.SendErrorMsgToPlayer(player_id, '魂石功能:没有此项..');
            }
        } else {
    ...
    GetDeleteItems(
        ...
        for (const key in items) {
            const item_id = tonumber(key);
            const count = math.floor((items[key] / 100) * pro);
            if (count > 0) {
