...
export class ServiceInterface extends UIEventRegisterClass {
    ...
    constructor() {
        ...
        //初始化技能数据
        for (const i_key in PictuerFetterAbility) {
            const data = PictuerFetterAbility[i_key as keyof typeof PictuerFetterAbility];
            this.fetter_ability_values[i_key] = {};
            //技能数组
            for (const A_key in data.AbilityValues) {
                const str = tostring(data.AbilityValues[A_key]);
    ...
    GetGameActivate(player_id: PlayerID, params: CGED['ServiceInterface']['GetGameActivate'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetGameActivate', {
            data: {
    ...
    GetServerMapLevel(exp: number): AM2_Server_Exp_Data {
        ...
        for (let index = 1; index < max_level; index++) {
            ...
            const use_exp = LFUN.eval(exp_equation, param);
            if (yyexp >= use_exp) {
                level++;
                yyexp -= use_exp;
            } else {
    ...
    MapExpUpdate(player_id: PlayerID, exp: number) {
        if (exp != GameRules.ServiceData.server_gold_package_list[player_id]['1004'].number) {
            const new_player_map_level = GameRules.ServiceInterface.GetServerMapLevel(exp);
            ...
            if (is_up_soul_and_talent) {
                const map_level = GameRules.ServiceInterface.player_map_level[player_id].level;
                print('map_level : ', map_level);
                //重新发送魂石信息
                GameRules.ServiceSoul.GetPlayerServerSoulData(player_id, {});
                //重新修改天赋点
                for (const key in NpcHeroesCustom) {
                    const hero = NpcHeroesCustom[key as keyof typeof NpcHeroesCustom];
                    const hero_id_str = tostring(hero.HeroID);
                    if (GameRules.ServiceTalent.player_server_talent_list[player_id].hasOwnProperty(hero_id_str)) {
                        ...
                        for (let x = 0; x < GameRules.ServiceTalent.index_count; x++) {
                            if (GameRules.ServiceTalent.player_server_talent_list[player_id][hero_id_str][x]) {
                                const c =
                                    GameRules.ServiceTalent.player_talent_list[player_id][hero_id_str][x].u +
                                    GameRules.ServiceTalent.player_talent_list[player_id][hero_id_str][x].y;
                                if (map_level > c) {
                                    const chazhi = map_level - c;
    ...
    GetServerSkillfulLevel(key: string, exp: number): { level: number; cur_exp: number; level_exp: number; is_max: number } {
        const exp_equation = ServerSkillExp[key as keyof typeof ServerSkillExp].exp;
        const max_level = ServerSkillExp[key as keyof typeof ServerSkillExp].max_level;
        let yyexp = exp;
        let level = 0;
        for (let index = 1; index < max_level; index++) {
            const param = {
    ...
    LoadSkillfulLevelInit(player_id: PlayerID) {
        const sse_length = Object.keys(ServerSkillExp);
    ...
    LoadSkillfulLevel(player_id: PlayerID) {
        ...
        const obj_exp = this.PlayerServerSkillLevelExp[player_id];
        //加载主技能经验等级
    ...
    GenerateSkillLevel(player_id: PlayerID, key: string, skill_exp: number) {
        const is_advanced = ServerSkillExp[key as keyof typeof ServerSkillExp].is_advanced;
        if (is_advanced == 0) {
            //不是高级
            const lvdata = this.GetServerSkillfulLevel(key, skill_exp);
            const need_number = lvdata.level_exp - lvdata.cur_exp;
            const need_number_list: { [item_id: number]: number } = {
                1293: need_number,
            };
    ...
    GenerateSkillTypeLevel(player_id: PlayerID, key: string) {
        ...
        for (const key in ServerSkillful) {
            const ServerSkillfulData = ServerSkillful[key as keyof typeof ServerSkillful];
            //初始化
            if (ServerSkillfulData.type == PlayerServerSkillLevelCount.type) {
                //计算等级
                if (ServerSkillfulData.is_lock == 1) {
                    //处理特殊解锁
                    ...
                    if (PlayerServerSkillLevelCount.lv >= ServerSkillfulData.max_level) {
                        ...
                        const skill_level = PlayerServerSkillLevelCount.lv - (ServerSkillfulData.min_level - 1);
                        if (skill_level > 0) {
                            this.PlayerServerSkillTypeLevel[player_id][key] = {
                                lv: skill_level,
                            };
    ...
    ServerSkillUp(player_id: PlayerID, params: CGED['ServiceInterface']['ServerSkillUp']) {
        const skill_key = params.key;
        if (this.PlayerServerSkillLevelCount[player_id].level.hasOwnProperty(skill_key)) {
            const data = this.PlayerServerSkillLevelCount[player_id].level[skill_key];
            const max_level = ServerSkillExp[skill_key as keyof typeof ServerSkillExp].max_level;
            const is_advanced = ServerSkillExp[skill_key as keyof typeof ServerSkillExp].is_advanced;
            if (data.lv < max_level) {
                ...
                for (const key in data.need_item) {
                    const item_id_number = tonumber(key);
                    const number = data.need_item[key];
                    ...
                    const item_ret = GameRules.ServiceData.VerifyPackageItem(player_id, item_id_number, number);
                ...
                const skill_data = CustomDeepCopy(GameRules.ServiceInterface.PlayerServerSkillLevelExp[player_id]) as {
                    [skill_key: string]: number;
                };
                skill_data[skill_key] += up_exp;
                const skill_key_str = JSON.encode(skill_data);
                const zz_exp = this.PlayerServerSkillLevelCount[player_id].level[skill_key].exp + up_exp;
    ...
    GetPlayerServerSkillData(player_id: PlayerID, params: CGED['ServiceInterface']['GetPlayerServerSkillData'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerServerSkillData', {
            data: {
    ...
    GetServerTime(player_id: PlayerID, params: CGED['ServiceInterface']['GetServerTime'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetServerTime', {
            data: {
    ...
    PlayerConsumeCard(player_id: PlayerID, params: CGED['ServiceInterface']['PlayerConsumeCard'], callback?) {
        const suit_id = params.suit_id;
        const card_id = tonumber(params.card_id);
        const PictuerFetterData = PictuerFetterConfig[suit_id as keyof typeof PictuerFetterConfig];
        if (PictuerFetterData.card_ids.includes(card_id)) {
            //物品id
            const item_id = PictuerCardData[params.card_id as keyof typeof PictuerCardData].item_id;
            const check_data = GameRules.ServiceData.GetMonsterPackageIndexAndCount(player_id, item_id);
            if (check_data.index >= 0 && check_data.count > 0) {
                //copy一份数据
                const server_pictuer_fetter_copy = CustomDeepCopy(
                    GameRules.ServiceData.server_pictuer_fetter_list[player_id]
                ) as ServerPlayerConfigPictuerFetter;
                ...
                const pictuer_data = json.encode(server_pictuer_fetter_copy);
                const red_item_str = item_id + '_1';
    ...
    ConfigPictuerFetter(player_id: PlayerID, params: CGED['ServiceInterface']['ConfigPictuerFetter'], callback?) {
        ...
        for (let index = 0; index < GameRules.ServiceData.locality_player_config_pictuer_fetter[player_id][i].length; index++) {
            const u_suit_id = GameRules.ServiceData.locality_player_config_pictuer_fetter[player_id][i][index];
    ...
    UninstallPictuerFetter(player_id: PlayerID, params: CGED['ServiceInterface']['UninstallPictuerFetter'], callback?) {
        ...
        const suit_id = params.suit_id;
        if (GameRules.ServiceData.locality_player_config_pictuer_fetter[player_id][i].includes(suit_id)) {
            const index = GameRules.ServiceData.locality_player_config_pictuer_fetter[player_id][i].indexOf(suit_id);
    ...
    SavePictuerFetter(player_id: PlayerID, params: CGED['ServiceInterface']['SavePictuerFetter'], callback?) {
        const i = params.index;
        //保存数据
        const pictuer_config = json.encode(GameRules.ServiceData.locality_player_config_pictuer_fetter[player_id]);
    ...
    GetConfigPictuerFetter(player_id: PlayerID, params: CGED['ServiceInterface']['GetConfigPictuerFetter']) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetConfigPictuerFetter', {
            data: {
    ...
    PictuerLoadClose(player_id: PlayerID) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_PictuerLoadClose', {
            data: {},
        });
    }
    ...
    PulbicLoadClose(player_id: PlayerID) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_PulbicLoadClose', {
            data: {},
        });
    }
    ...
    CompoundCard(player_id: PlayerID, params: CGED['ServiceInterface']['CompoundCard']) {
        const list = params.list as any;
        const type = params.type;
        const list_obj = list as {
            [index: string]: {
                [index: string]: string;
            };
        };
        if (Object.keys(list_obj).length <= 8) {
            ...
            for (const list_key in list_obj) {
                ...
                for (const d_key in list_obj[list_key]) {
                    //品质检测
                    const card_id = list_obj[list_key][d_key];
                    const rarity = PictuerCardData[card_id as keyof typeof PictuerCardData].rarity;
                    if (r == -1) {
                        r = rarity;
                    } else {
                        if (r != rarity) {
                    ...
                    //记录数量
                    const item_id = PictuerCardData[card_id as keyof typeof PictuerCardData].item_id;
            ...
            for (const itemid in consume) {
                ...
                const need_item_count = consume[itemid].c;
                if (red_item_str == '') {
            ...
            for (const list_key in list_obj) {
                ...
                for (const spcs_key in GameRules.ServiceData.server_pictuer_card_special) {
                    const ts_list = GameRules.ServiceData.server_pictuer_card_special[spcs_key];
                    for (const ts_key in list_obj[list_key]) {
                        const cid_num = tonumber(list_obj[list_key][ts_key]);
                ...
                //是否每个都满足
                if (cz_ts) {
                    const ts_list = GameRules.ServiceData.server_pictuer_card_special[is_pictuer_id];
                    for (const ts_key in list_obj[list_key]) {
                        const cid_num = tonumber(list_obj[list_key][ts_key]);
                        if (!ts_list.includes(cid_num)) {
                            is_ts = false;
                ...
                if (is_ts) {
                    if (RollPercentage(13)) {
                        //进阶为特殊卡片
                        const get_c_id = is_pictuer_id;
                        const get_item_id = PictuerCardData[get_c_id as keyof typeof PictuerCardData].item_id;
                        ...
                        //不进阶
                        const length = GameRules.ServiceData.server_pictuer_card_special[is_pictuer_id].length;
                        const RInt = RandomInt(0, length - 1);
                        const get_c_id = tostring(GameRules.ServiceData.server_pictuer_card_special[is_pictuer_id][RInt]);
                        const get_item_id = PictuerCardData[get_c_id as keyof typeof PictuerCardData].item_id;
                        if (add_list.hasOwnProperty(tostring(get_item_id))) {
                            add_list[tostring(get_item_id)]++;
                        } else {
                    ...
                    if (list_obj[list_key].hasOwnProperty('0')) {
                        ...
                        //普通卡片处理
                        const length = GameRules.ServiceData.server_pictuer_card_rarity[rarity].length;
                        const RInt = RandomInt(0, length - 1);
                        const get_c_id = GameRules.ServiceData.server_pictuer_card_rarity[rarity][RInt];
                        const get_item_id = PictuerCardData[get_c_id as keyof typeof PictuerCardData].item_id;
                        if (add_list.hasOwnProperty(tostring(get_item_id))) {
                            add_list[tostring(get_item_id)]++;
                        } else {
    ...
    GetPlayerCardList(player_id: PlayerID, params: CGED['ServiceInterface']['GetPlayerCardList']) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerCardList', {
            data: {
    ...
    GetCompoundCardList(player_id: PlayerID, cardlist: string[], type: number = 1) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetCompoundCardList', {
            data: {
                card: cardlist, //卡片id
    ...
    GetPlayerServerPackageData(player_id: PlayerID, params: CGED['ServiceInterface']['GetPlayerServerPackageData'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerServerPackageData', {
    ...
    GetPlayerServerGoldPackageData(player_id: PlayerID, params: CGED['ServiceInterface']['GetPlayerServerPackageData'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerServerGoldPackageData', {
    ...
    GetPlayerMapLevel(player_id: PlayerID, params: CGED['ServiceInterface']['GetPlayerMapLevel'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerMapLevel', {
    ...
    GetServerItemPopUp(player_id: PlayerID, add_items: AM2_Server_Backpack[], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetServerItemPopUp', {
            data: {
                items: add_items,
            },
        });
    ...
    RechargeOrderData(player_id: PlayerID, pay_order: string, pay_m: string) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_RechargeOrderData', {
            data: {
    ...
    GetPlayerServerDrawLottery(player_id: PlayerID, lottery_data: AM2_Draw_Lottery_Data[]) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerServerDrawLottery', {
            data: lottery_data,
        });
    }
    ...
    GetPlayerServerDrawLotteryDrawRecord(player_id: PlayerID, params: CGED['ServiceInterface']['GetPlayerServerDrawLotteryDrawRecord'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerServerDrawLotteryDrawRecord', {
    ...
    GetPlayerServerPassRecord(player_id: PlayerID, params: CGED['ServiceInterface']['GetPlayerServerDrawLotteryDrawRecord'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerServerPassRecord', {
    ...
    GetPlayerShoppingLimit(player_id: PlayerID, params: CGED['ServiceInterface']['GetPlayerShoppingLimit'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerShoppingLimit', {
    ...
    GetPlayerVipData(player_id: PlayerID, params: CGED['ServiceInterface']['GetPlayerVipData'], callback?) {
        ...
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_GetPlayerVipData', {
            data: d,
        });
    }
    ...
    PackageDataUpdate(player_id: PlayerID, ServerBackpackUpdate: AM2_Server_Backpack_Update[]) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ServiceInterface_PackageDataUpdate', {
    ...
    PostLuaLog(player_id: PlayerID, msg: string) {
        const gametime = math.floor(GameRules.GetDOTATime(false, false) - GameRules.GameInformation.play_game_time);
        let gametimegsh = '';
        let hour_str = '00';
        let minute_str = '00';
        let second_str = '00';
        ...
        if (gametime > 3600) {
            const gametime_int = math.floor(gametime / 3600);
            if (gametime_int < 10) {
        ...
        if (gametime > 60) {
            const minute_int = math.floor((gametime % 3600) / 60);
            if (minute_int < 10) {
    ...
    SendLuaLog(player_id: PlayerID) {
        const send_obj: { [sid: string]: string } = {};
        if (player_id == -1) {
            const player_count = GetPlayerCount();
            for (let index = 0 as PlayerID; index < player_count; index++) {
                if (this.player_log_data[index] != '') {
                    ...
                    const steam_id = PlayerResource.GetSteamAccountID(index).toString();
                    const hero_level = Hero.GetLevel();
                    send_obj[steam_id] =
                        '第' +
                        GameRules.MapChapter.game_count +
                        '次游戏:' +
                        ';当前波数:' +
            ...
            if (this.player_log_data[player_id] != '') {
                ...
                const steam_id = PlayerResource.GetSteamAccountID(player_id).toString();
                const hero_level = Hero.GetLevel();
                send_obj[steam_id] =
                    '第' +
                    GameRules.MapChapter.game_count +
    ...
    ShoppingBuy(player_id: PlayerID, params: CGED['ServiceInterface']['ShoppingBuy'], callback?) {
        const shop_id = tonumber(params.shop_id);
    ...
    UseItem(player_id: PlayerID, params: CGED['ServiceInterface']['UseItem'], callback?) {
        const use_item_id = params.use_item_id;
        const count = params.count;
    ...
    GetServerPass(player_id: PlayerID, params: CGED['ServiceInterface']['GetServerPass'], callback?) {
        const paramstype = params.type;
        const count = params.count;
        const get_type = params.get_type;
    ...
    GetOrderItem(player_id: PlayerID, params: CGED['ServiceInterface']['GetOrderItem'], callback?) {
        const pay_order = params.pay_order;
     ...
     */
    GetTKVOfFa<Key extends keyof typeof PictuerFetterAbility, T2 extends (typeof PictuerFetterAbility)[Key]>(
        prop_name: Key,
        key: keyof T2['AbilityValues'],
        level_index: number = 0
    ) {
        const value_key = key as string;
        //因为只有 1级 所以全部返回 0 的下标
        // return this.prop_ability_values[prop_name][value_key][0];
        const length = this.fetter_ability_values[prop_name][value_key].length;
        if (length > 0) {
            if (level_index < 0) {
     ...
     * @returns
     */
    GetKvOfUnit<Key extends keyof typeof PictuerFetterAbility, T2 extends (typeof PictuerFetterAbility)[Key]>(
        hUnit: CDOTA_BaseNPC,
        prop_name: Key,
        ability_key: keyof T2['AbilityValues']
    ) {
        if (IsServer()) {
            const prop_count = hUnit.pictuer_ability_name[prop_name];
            if (prop_count == null) {
                return 0;
            } else {
    ...
    Debug(cmd: string, args: string[], player_id: PlayerID) {
        ...
        if (cmd == '!CC') {
            const list = [[args[0], args[1], args[2]]];
