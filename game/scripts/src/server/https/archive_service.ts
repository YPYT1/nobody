...
export class ArchiveService extends UIEventRegisterClass {
    ...
    //使用的steam_id
    debug_steam_id = [859509413, 999992, 999993, 999994, 999995, 999997];
    ...
    GameOver(state: number, exp: number[] = [], cj: string[] = [], hero: string[] = [], is_endless: number = -1, is_nianshou: number = -1) {
        print('==============游戏结束================');
        //技能经验获取
        const exp_list = GameRules.HeroAbilityType.GetAbilityTypeExp();

        const host_steam_id = PlayerResource.GetSteamAccountID(0);
        const param_data = <GameOverParam>{
            state: state,
        ...
        for (let p_id = 0 as PlayerID; p_id < player_count; p_id++) {
            //掉落卡片
            const player_steam_id = tostring(PlayerResource.GetSteamAccountID(p_id));
            param_data.ext_items[player_steam_id] = GameRules.Spawn.player_card_drop[p_id];
            //技能经验
            const skill_exp = CustomDeepCopy(GameRules.ServiceInterface.PlayerServerSkillLevelExp[p_id]) as {
                [skill_key: string]: number;
            };
            ...
            (data: GameOverReturn) => {
                ...
                if (data.code == 200) {
                    ...
                    for (let index = 0 as PlayerID; index < player_count; index++) {
                        ...
                        const steam_id_string = tostring(steam_id);
                        const PlayerPassItem: CGEDPlayerPassItem[] = [];
                        let player_exp: number = 0;
                        const CGEDPlayerSkillExp: CGEDPlayerSkillExp = {};
                        //获取掉落
                        if (data.data.list.hasOwnProperty(steam_id_string)) {
                            const add_items = data.data.list[steam_id_string].add_items;
                            for (const add_item of add_items) {
                                const item_id = tostring(add_item.item_id);
                                if (item_id == '1004') {
                                    player_exp = add_item.number;
                                } else {
                                    const quality = ServerItemList[item_id as keyof typeof ServerItemList].quality;
                                    PlayerPassItem.push({
                            ...
                            for (const e_l_k in exp_list[index]) {
                                if (exp_list[index][e_l_k] > 0) {
                                    const skill_old_exp = GameRules.ServiceInterface.PlayerServerSkillLevelExp[index][e_l_k];
                                    const skill_exp = exp_list[index][e_l_k];
                            ...
                            //经验经验
                            GameRules.ServiceInterface.PlayerServerSkillLevelExp[index] = JSON.decode(data.data.list[steam_id_string].skill_exp) as {
                                [skill_key: string]: number;
                            };
                            GameRules.ServiceInterface.LoadSkillfulLevel(index);
    ...
    GetPlayerGameOverData(player_id: PlayerID, params: CGED['ArchiveService']['GetPlayerGameOverData'], callback?) {
        CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player_id), 'ArchiveService_GetPlayerGameOverData', {
    ...
    AddEquip(player_id: PlayerID, equipdata: ServerEquip[]) {
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <AddEquipParam>{
            sid: steam_id.toString(),
            equipdata: equipdata,
        };
        HttpRequest.AM2Post(
    ...
    GetEquip(player_id: PlayerID) {
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <GetEquipParam>{
            sid: steam_id.toString(),
            limit: 50,
        };
        HttpRequest.AM2Post(
    ...
    UpdateEquip(player_id: PlayerID, equipdatalist: { [equip_id: string]: ServerEquip }, red_list: string = '') {
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <UpdateEquipParam>{
            sid: steam_id.toString(),
    ...
    ShoppingBuy(player_id: PlayerID, shop_id: number, buy_count: number, buy_types: number = 1) {
        //只验证主机
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <ShoppingBuyParam>{
        ...
        HttpRequest.AM2Post(
            ...
            (data: ShoppingBuyReturn) => {
                if (data.code == 200) {
                    //先删除再添加
                    const red_item = data.data.red_item;
                    const add_item = data.data.add_item;
                    const gold_data = data.data.base;
                    //Vip信息
    ...
    GetCustomBackpack(player_id: PlayerID, aff_class: string) {
        //只验证主机
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <GetCustomBackpackParam>{
            sid: tostring(steam_id), //steamid
            aff_class: aff_class,
        };
        HttpRequest.AM2Post(
            ACTION_GET_CUSTOM_BACKPACK,
            {
                param: param_data,
            },
    ...
    DrawLottery(player_id: PlayerID, type: number = 1, count: number = 1) {
        //只验证主机
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <DrawLotteryParam>{
            sid: tostring(steam_id), //steamid
            types: type,
            number: count,
        };
    ...
    GetServerDrawAcc(player_id: PlayerID, type: number = 1, count: number = 1) {
        //只验证主机
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <GetServerDrawAccParam>{
            sid: tostring(steam_id), //steamid
            type: type,
            count: count,
        };
        HttpRequest.AM2Post(
            ACTION_GET_SERVER_DRAW_ACC,
            {
                param: param_data,
            },
    ...
    GetServerPass(player_id: PlayerID, type: number = 1, count: number = 1, get_type: number = 1) {
        ...
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <GetServerPassParam>{
            sid: tostring(steam_id), //steamid
            type: type,
            count: count,
            get_type: get_type,
    ...
    GetPictuerDataParam(player_id: PlayerID) {
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <SkillDataUpParam>{
            sid: tostring(steam_id), //steamid
        };
        HttpRequest.AM2Post(
            ACTION_GET_PICTUER_DATA,
            {
                param: param_data,
            },
            (data: GetPictuerDataReturn) => {
                if (data.code == 200) {
                    ...
                    if (data.data.pictuer.pictuer_data && data.data.pictuer.pictuer_data != '') {
                        GameRules.ServiceData.server_pictuer_fetter_list[player_id] = JSON.decode(
                            data.data.pictuer.pictuer_data
                    ...
                    if (data.data.pictuer.pictuer_config && data.data.pictuer.pictuer_config != '') {
                        GameRules.ServiceData.server_player_config_pictuer_fetter[player_id] = JSON.decode(
                            data.data.pictuer.pictuer_config
                        ) as string[][];
                        GameRules.ServiceData.locality_player_config_pictuer_fetter[player_id] = JSON.decode(
                            data.data.pictuer.pictuer_config
                        ) as string[][];
                    }
    ...
    PictuerSave(
        ...
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <PictuerSaveParam>{
            sid: tostring(steam_id), //steamid
        ...
        HttpRequest.AM2Post(
            ...
            (data: PictuerSaveReturn) => {
                if (data.code == 200) {
                    ...
                    if (data.data.pictuer.pictuer_data && data.data.pictuer.pictuer_data != '') {
                        const d: ServerPlayerConfigPictuerFetter = JSON.decode(data.data.pictuer.pictuer_data) as any;
        ...
        HttpRequest.AM2Post(
            ...
            (data: SkillDataUpReturn) => {
                if (data.code == 200) {
                    ...
                    //更新存档技能数据
                    GameRules.ServiceInterface.PlayerServerSkillLevelExp[player_id] = JSON.decode(data.data.skill_data) as {
                        [skill_key: string]: number;
                    };
     ...
     * @param type
     */
    PlayerSoulStoneSave(player_id: PlayerID, pa_data: string, red_item_str: string, type: number = 1, add_item_str?: string, up_num: number = 0) {
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <PlayerSoulStoneSaveParam>{
            sid: tostring(steam_id), //steamid
            pa: pa_data,
            red_item_str: red_item_str,
     ...
     * @param talentdata
     */
    PlayerTalentSave(player_id: PlayerID, talentdata: string) {
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <PlayerTalentSaveParam>{
            sid: tostring(steam_id), //steamid
            talentdata: talentdata,
        };
        HttpRequest.AM2Post(
            ACTION_PLAYER_TALENT_SAVE,
            {
                param: param_data,
            },
    ...
    RechargeOrder(player_id: PlayerID, from: number, count: number, shop_id: number = -1) {
        //只验证主机  AM2_Draw_Pass_Record
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <RechargeOrderParam>{
            sid: tostring(steam_id), //steamid
            from: from,
            shop_id: shop_id,
            count: count,
        ...
        HttpRequest.AM2Post(
            ...
            (data: RechargeOrderReturn) => {
                if (data.code == 200) {
                    // let red_item = data.data.red_item;
                    const pay_m = data.data.pay_m;
                    const pay_order = data.data.pay_order;
    ...
    GetOrderItem(player_id: PlayerID, pay_order: string) {
        //只验证主机  AM2_Draw_Pass_Record
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <GetOrderItemParam>{
            sid: tostring(steam_id), //steamid
            pay_order: pay_order,
        };
        HttpRequest.AM2Post(
            ACTION_GET_ORDER_ITEM,
            {
                param: param_data,
            },
    ...
    Debug(cmd: string, args: string[], player_id: PlayerID) {
        ...
        if (cmd == '!gm') {
            const shop_id = args[0] ?? '1';
            this.ShoppingBuy(player_id, tonumber(shop_id), 1);
        }
        if (cmd == '!gp') {
            const aff_class = args[0] ?? '21,2,22';
    ...
    PulbicItemAddDel(player_id: PlayerID, red_item_str: string, add_item_str: string, type: number = 1, cardtype: number = 1) {
        //只验证主机
        const steam_id = PlayerResource.GetSteamAccountID(player_id);
        const param_data = <PulbicItemAddDelParam>{
            sid: tostring(steam_id), //steamid
            red_item_str: red_item_str,
            add_item_str: add_item_str,
        };
        HttpRequest.AM2Post(
            ACTION_PULBIC_ITEM_ADD_DEL,
            {
                param: param_data,
            },
