...
const GetCompoundCardList = (event: NetworkedData<CustomGameEventDeclarations['ServiceInterface_GetCompoundCardList']>) => {
    ...
    const list = Object.values(data.card);
    for (const card_id of list) {
        const cardPanel = $.CreatePanel('Panel', CompoundCardList, card_id);
...
const CompoundCard_OnDragStart = (panel: Panel, dragCallbacks: Panel) => {
    panel.visible = false;
    const displayPanel = $.CreatePanel('Panel', $.GetContextPanel(), 'drag_CardImage');
