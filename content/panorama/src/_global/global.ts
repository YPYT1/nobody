...
import type { DASHBOARD_NAVBAR } from '../dashboard/components';
...
declare global {
    interface CustomUIConfig {
        ...
        SendCustomEvent: <T1 extends keyof CGED, T2 extends keyof CGED[T1], T3 extends CGED[T1][T2]>(
            pEventName: T1,
            event_name: T2,
            params: T3
        ) => void;
        ...
        DashboardRoute<Key extends keyof typeof DASHBOARD_NAVBAR, T2 extends (typeof DASHBOARD_NAVBAR)[Key]>(
            dashboard_id: Key,
            nav: keyof T2['Sub']
        ): void;
...
GameUI.CustomUIConfig().CreateServerItem = function (item_id: string, item_count: number, parent: Panel) {
    const ServerItemPanel = $.CreatePanel('Panel', parent, '');
...
GameUI.CustomUIConfig().ConverAttrAndValueLabel = (attr: string, value: number, decimal: number = 0) => {
    const is_pct = AttributeConst[attr as keyof typeof AttributeConst].is_pct == 1;
    let res_label = '0';
    if (is_pct) {
...
GameUI.CustomUIConfig().SetHotKey = function (key: string, down_func: Function, up_func?: Function) {
    const command_string = `On${key}${Date.now()}`;
...
GameUI.CustomUIConfig().SendCustomEvent = function <T1 extends keyof CGED, T2 extends keyof CGED[T1], T3 extends CGED[T1][T2]>(
    pEventName: T1,
    event_name: T2,
    params: T3
) {
...
GameUI.CustomUIConfig().ConvertServerItemToArray = function (item_input: string) {
    const data_array = [];
    const item_arr = item_input.split(',');
    for (const row_item of item_arr) {
        const row_data = row_item.split('_');
        const item_id = row_data[0];
        const item_count = parseInt(row_data[1]);
...
GameUI.CustomUIConfig().ConvertServerItemToObject = function (item_input: string) {
    const data_object: { [item_id: string]: number } = {};
    const item_arr = item_input.split(',');
    for (const row_item of item_arr) {
        const row_data = row_item.split('_');
        const item_id = row_data[0];
...
const HeroIdTable: { [heroid: number]: string } = {};
