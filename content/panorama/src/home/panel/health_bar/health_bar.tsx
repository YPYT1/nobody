...
const overhead_panel_boss: { [key: EntityIndex]: Panel } = {};
...
const CMsg_GetEntityListHealthBar = (params: NetworkedData<CustomGameEventDeclarations['CMsg_GetEntityListHealthBar']>) => {
    ...
    for (let i = 0; i < TopHealthBarContent.GetChildCount(); i++) {
        const row_healthbar = TopHealthBarContent.GetChild(i);
        if (row_healthbar) {
            const entity = row_healthbar.Data<HealtrBarPanelData>().entitiy;
...
function UpdateTopPanelBoss() {
    overhead_boss_list.forEach(entity => {
        ...
        if (pPanel === null || pPanel === undefined) {
            ...
            const unit_hp_max = Entities.GetMaxHealth(entity);
            const unit_hp_current = Entities.GetHealth(entity);
            ...
            const ever_hp = unit_hp_max * EVER_HP_PCT * 0.01;
            const layer_num = Math.ceil(unit_hp_current / ever_hp); // 还有几根血条
            ...
            const boss_attack = Entities.GetDamageMax(entity);
        ...
        const max_layer = Math.ceil(100 / EVER_HP_PCT); // 最大根数
        ...
        const is_inv = Entities.IsInvulnerable(entity);
        if (is_inv) {
            // 如果是无敌状态,在这个阶段逐渐变为满血
            const buff_id = Entities_FindBuffID(entity, 'modifier_custom_appearance_underground');
            if (buff_id) {
                const ElapsedTime = Buffs.GetElapsedTime(entity, buff_id);
                const duration = Buffs.GetDuration(entity, buff_id);
                const healthWidth = (100 * ElapsedTime) / duration;
                // $.Msg(["healthWidth", healthWidth])
                EvenBossHealthBar.style.width = `${healthWidth}%`;
            }
            return;
        }
        const curr_hp = Entities.GetHealth(entity);
        const max_hp = Entities.GetMaxHealth(entity);
        const ever_hp = max_hp * EVER_HP_PCT * 0.01;
        /** 当前帧扣除的生命 */
        const deduct_hp = health_bar_object[`${entity}`].curr_hp - curr_hp;
        ...
        if (deduct_hp <= 0) {
            ...
            const layer_num = Math.ceil(curr_hp / ever_hp); // 还有几根血条
            pPanel.Data<HealtrBarPanelData>().layer_num = layer_num;
            pPanel.SetDialogVariableInt('hp_count', layer_num);
            const color_index = (max_layer - layer_num) % TOTAL_COLOR_COUNT;
            const next_color_index = (max_layer - layer_num + 1) % TOTAL_COLOR_COUNT;

            /** 当前层数为偶数? */
            const is_even = layer_num % 2 == 0;
            ...
            const healthWidth = GetNumDecimals(((curr_hp - ever_hp * (layer_num - 1)) / ever_hp) * 100, 1);
            const currHudHeathBar = CurrentHealthBar.FindChildTraverse('HudHeathBar')!;
            const currBossHealthBar = CurrentHealthBar.FindChildTraverse('BossHealthBar')!;
            ...
            // 下次血条
            const nextHudHeathBar = NextHealthBar.FindChildTraverse('HudHeathBar')!;
            const nextBossHealthBar = NextHealthBar.FindChildTraverse('BossHealthBar')!;
        ...
        // 当前血量是第几层
        const layer_num = pPanel.Data<HealtrBarPanelData>().layer_num;
        pPanel.SetDialogVariableInt('hp_count', layer_num);
        const color_index = (max_layer - layer_num) % TOTAL_COLOR_COUNT;
        const next_color_index = (max_layer - layer_num + 1) % TOTAL_COLOR_COUNT;
        ...
        // 当前层数血量百分比
        const curr_layer_hp_pct = Math.max(0, ((curr_hp - ever_hp * (layer_num - 1)) / ever_hp) * 100);
        const currHudHeathBar = CurrentHealthBar.FindChildTraverse('HudHeathBar')!;
        const currBossHealthBar = CurrentHealthBar.FindChildTraverse('BossHealthBar')!;
...
function UpdateOverheadThinker() {
    const entities = Entities.GetAllEntitiesByName('npc_dota_creature');
    entities.forEach(entity => {
        ...
        const healthWidth = GetNumDecimals((Entities.GetHealth(entity) / Entities.GetMaxHealth(entity)) * 100, 1);
