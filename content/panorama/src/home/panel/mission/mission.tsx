...
export const Init = () => {
    ...
    $('#resource').BLoadLayout('file://{resources}/layout/custom_game/home/panel/mission/resource/resource.xml', false, false);
...
const RemoveMissionTipsClass = (panel: Panel) => {
    ...
    for (const key of mission_key_list) {
...
const MisstionTimerUpdate = () => {
    ...
    for (const pPanel of [RadiantMissionContainer, DireMissionContainer]) {
        const end_time = pPanel.Data<PanelDataObject>().end_time as number;
        const max_time = (pPanel.Data<PanelDataObject>().max_time as number) ?? 1;
        if (max_time == 1) {
            continue;
        }
        const diff_time = Math.ceil(end_time - dota_time);
...
const CustomSubscribe = () => {
    GameEvents.Subscribe('MissionSystem_SendMissionTips', event => {
        ...
        const mission_type = data.mission_type;
        if (mission_type == 1) {
    ...
    GameEvents.Subscribe('MissionSystem_MissionComplete', event => {
        const data = event.data;
        const m_type = data.mission_type;
