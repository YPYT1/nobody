...
const HeroIdTable: { [key: number]: string } = {};

let local_select_id = 6;
let select_talent_index = 0;
function InitKvTable() {
    for (const k in NpcHeroesCustom) {
        const hero_data = NpcHeroesCustom[k as keyof typeof NpcHeroesCustom];
        const heroid = hero_data.HeroID;
        HeroIdTable[heroid] = hero_data.override_hero;
    }
}

GameEvents.Subscribe('MapChapter_GetPlayerHeroList', event => {
    const heroes = event.data.hero_id;
...
GameEvents.Subscribe('MapChapter_GetPlayerSelectHeroList', event => {
    ...
    const hero_ids = Object.values(data.hero_ids);
    for (let player_id = 0 as PlayerID; player_id < hero_ids.length; player_id++) {
        const PlayerState = PlayerStateContainer.GetChild(player_id)!;
        const player_data = hero_ids[player_id];
        const hero_id = player_data.hero_id;
        const hero_name = HeroIdTable[hero_id];

        const hero_data = NpcHeroesCustom[hero_name as keyof typeof NpcHeroesCustom];
        const PortraitImage = hero_data.PortraitImage;
        // $.Msg(["hero_name",hero_name]);

        const HeroIcon = PlayerState.FindChildTraverse('HeroIcon') as ImagePanel;
        ...
        const is_enable = hero_data.Enable == 1;

        if (player_id == Game.GetLocalPlayerID()) {
            // $.Msg(["hero_id", hero_id, local_select_id])
            for (let i = 0; i < HeroBackground.GetChildCount(); i++) {
                const row_panel = HeroBackground.GetChild(i) as ScenePanel;
                const row_id = row_panel.id;
            ...
            const is_ready = player_data.state == 1;
            HeroConfirm.enabled = !is_ready;

            for (let i = 0; i < HeroesList.GetChildCount(); i++) {
                const HeroCardButton = HeroesList.GetChild(i) as RadioButton;
                const card_hero_id = HeroCardButton.id;
...
function StartLoopThinker() {
    ...
    const start_time = MainPanel.Data<PanelDataObject>().start_time;
    const TimeLabel = FormatNumberToTime(Math.floor(start_time - dotaTime));
...
function CreatePlayerStatePanel() {
    ...
    const AllPlayerId: PlayerID[] = Game.GetAllPlayerIDs();
    const heroname = 'npc_dota_hero_drow_ranger';

    for (const pid of AllPlayerId) {
        const is_local = pid == Players.GetLocalPlayer();
        const playerInfo = Game.GetPlayerInfo(pid);
        const PlayerState = $.CreatePanel('Panel', PlayerStateContainer, '');
    ...
    for (const key in NpcHeroesCustom) {
        const hero_data = NpcHeroesCustom[key as keyof typeof NpcHeroesCustom];
        const hero_id = hero_data.HeroID;
        const hero_name = hero_data.override_hero;
        const Enable = hero_data.Enable == 1;
        const HeroCardButton = $.CreatePanel('RadioButton', HeroesList, `${hero_id}`);
        ...
        HeroCardButton.SetPanelEvent('onactivate', () => {
            ...
            for (let i = 0; i < HeroesList.GetChildCount(); i++) {
                const row_panel = HeroesList.GetChild(i) as RadioButton;
...
export const Init = () => {
    ...
    GameEvents.Subscribe('MapChapter_GetGameSelectPhase', event => {
        const game_select_phase = event.data.game_select_phase;
...
function UpdateTalentDataInTips() {
    ...
    for (const main_key in attr_object) {
        for (const sub_key in attr_object[main_key]) {
            ...
            const _Panel = $.CreatePanel('Panel', TalentAttributeList, '');
