...
const UpdateTopInfoTime = () => {
    const GameSelectPhase = MainPanel.Data<PanelDataObject>().GameSelectPhase as number;
    ...
    const RoundGameTime = DotaGameTime - RoundGameStartTime;
    const TimeLabel = FormatNumberToTime(RoundGameTime);
    MainPanel.SetDialogVariable('time_label', TimeLabel.join(':'));

    const RoundBossTime = Math.max(0, Math.floor(BossCountdownTimer - DotaGameTime));
