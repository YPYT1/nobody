...
const UpdateScoreBoard = () => {
    ...
    for (let i = 0; i < PlayerList.GetChildCount(); i++) {
        const PlayerScoreBoard = PlayerList.GetChild(i);
        if (PlayerScoreBoard) {
            const revive_time = PlayerScoreBoard.Data<PanelDataObject>().revive_time ?? 0;
            const last_time = Math.floor(revive_time - dota_time);
            if (last_time > 0) {
...
const Create_Scoreboard = (count: number) => {
    ...
    const player_count = Game.GetAllPlayerIDs();
    for (const player_id of player_count) {
        const playerInfo = Game.GetPlayerInfo(player_id);
        const PlayerScoreBoard = $.CreatePanel('Panel', PlayerList, `${player_id}`);
        PlayerScoreBoard.BLoadLayoutSnippet('PlayerScoreBoard');
        const AvatarImage = PlayerScoreBoard.FindChildTraverse('AvatarImage') as AvatarImage;
        ...
        const player_color = PlayerIdToARGB(Players.GetPlayerColor(player_id));
        // DamageRecordBar.style.washColor = "#" + player_color;
        const connect = playerInfo.player_connection_state;
    }
};
...
const CustomSubscribe = () => {
    ...
    GameEvents.Subscribe('GameInformation_GetPlayerDieData', event => {
        ...
        const time_list = Object.values(data.time);
    ...
    GameEvents.Subscribe('MapChapter_GetPlayerVoteData', event => {
        ...
        const state = event.data.vote_data.state;
        PlayerList.SetHasClass('ShowState', state == 1);
        if (state == 1) {
            const playervote = Object.values(event.data.vote_data.playervote);
    ...
    GameEvents.Subscribe('CMsg_GetDamageRecord', event => {
        const dmg_record = Object.values(event.data.dmg_record);
        // $.Msg(["dmg_record",dmg_record])
        const max_damage = Math.max(...dmg_record);
        for (let i = 0; i < PlayerList.GetChildCount(); i++) {
            const PlayerScoreBoard = PlayerList.GetChild(i);
            if (PlayerScoreBoard) {
                const damage_label = FormatIntToString(dmg_record[i]);
