...
const MysticalShop = $('#MysticalShop');
const ShopItemList = $('#ShopItemList');
let start_buy_state = 0;
const loop_state = false;
const PopupModal = $('#PopupModal');
const PurchaseConfirm = $('#PurchaseConfirm');
const BtnConfirm = $('#BtnConfirm') as Button;
...
const GameEventsSubscribeInit = () => {
    GameEvents.Subscribe('MysticalShopSystem_GetShopState', event => {
        ...
        // 更新玩家状态
        const ReadyList = Object.values(data.shop_state_data);
        const is_ready = ReadyList[localPlayer].is_ready == 1;
        // $.Msg(["ready_state",ready_state])
        const ReadyBtn = $('#ReadyBtn') as Button;
    ...
    GameEvents.Subscribe('MysticalShopSystem_GetShopData', event => {
        ...
        const data = event.data;
        // 刷新次数和刷新价格
        const shop_info = data.shop_field_list;
        ...
        const local_vip = data.player_vip_status;
        const has_limit_item: string[] = [];
        // 成长道具
        const list_data = Object.values(event.data.player_shop_buy_ts_data);
        for (let i = 0; i < list_data.length; i++) {
            // $.Msg(ItemPanel)
            const data = list_data[i];
            const item_key = data.item_key;
            const type = data.type;
            const is_vip = data.is_vip == 1;
            const ItemPanel = ExtremePropsList.GetChild(i);
            const star = data.count;
            if (ItemPanel) {
                ...
                const ShopItemComponent = ItemPanel.FindChildTraverse('ShopItemComponent') as GameComponent_PropItem;
                ...
                if (type == 1) {
                    ...
                    // Star
                    const ItemStar = ItemPanel.FindChildTraverse('ItemStar') as GameComponent_PropItemStar;
            ...
            const NavItemPanel = LocalExtremePropsList.GetChild(i);
            if (NavItemPanel) {
                NavItemPanel.SetHasClass('IsLock', type == 0);
                NavItemPanel.SetHasClass('Null', type != 1);
                NavItemPanel.SetHasClass('IsVip', is_vip);
                //
                const LimitItem = NavItemPanel.FindChildTraverse('LimitItem') as GameComponent_PropItem;
                if (type == 1) {
                    // ItemPanel.SetDialogVariable("item_name", $.Localize(`#custom_shopitem_${data.item_key}`));
        ...
        const shop_list = data.shop_field_list.list;
        for (const k in shop_list) {
            const index = parseInt(k) - 1;
            const row_data = shop_list[k];
            const shop_key = row_data.key;
            // $.Msg(["shop_key",shop_key])
            if (shop_key == 'null') {
                continue;
            }
            // shop_sell_item_list.push(shop_key)
            const item_label = $.Localize(`#custom_shopitem_${row_data.key}`);
            const ShopItem = ShopItemList.GetChild(index)!;
            const is_vip = local_vip < row_data.is_vip;
            const IsLimit = row_data.type == 2;
            ...
            const show_level = IsLimit ? data_r : 1;
            let object_percent = 100;
            if (IsLimit) {
                object_percent = ShopItemJson.star_attr_pro[data_r - 1];
            }

            const item_desc = SetLabelDescriptionExtra(
            ...
            // 如果有相同的极限道具
            const same_index = has_limit_item.indexOf(row_data.key);
...
export const ClosedPopups = () => {
    ...
    for (let i = 0; i < PopupModal.GetChildCount(); i++) {
        const RowPopups = PopupModal.GetChild(i)!;
...
export const UpdataCountdownTimer = () => {
    ...
    const countdown_timer = MysticalShop.Data<PanelDataObject>().countdown_timer as number;
    const diff_timer = Math.floor(countdown_timer - dotatime);
...
export const CreatePanel = () => {
    ...
    for (let i = 0; i < 6; i++) {
        const extremePropsRows = $.CreatePanel('Panel', ExtremePropsList, '');
        extremePropsRows.BLoadLayoutSnippet('ExtremePropsRows');
        const ShopItemComponent = extremePropsRows.FindChildTraverse('ShopItemComponent')!;
        const PropItem = LoadGameComponent(ShopItemComponent, 'prop_item');
        PropItem._SetConfig({});
        const ItemStar = extremePropsRows.FindChildTraverse('ItemStar')!;
        const PropItemStar = LoadGameComponent(ItemStar, 'prop_item_star');
    ...
    for (let i = 0; i < 6; i++) {
        const ExtremeProps = $.CreatePanel('Panel', LocalExtremePropsList, '');
        ExtremeProps.BLoadLayoutSnippet('NavLimitItem');
        const LimitItem = ExtremeProps.FindChildTraverse('LimitItem')!;
        const PropItem = LoadGameComponent(LimitItem, 'prop_item');
    ...
    for (let i = 0; i < SHOP_ITEM_COUNT; i++) {
        const ShopItem = $.CreatePanel('Button', ShopItemList, '');
        ...
        ShopItemCard.SetPanelEvent('onactivate', () => {
            const item_key = ShopItemCard.Data<PanelDataObject>().item_key;
    ...
    // 弹窗按钮
    const BtnCancel = $('#BtnCancel') as Button;
