...
export const bar_panel_list: { [key: string]: Panel } = {};

export const CreatePanel_HpBar = () => {
    const HpBarPanel = $('#HpBar');
    const BarContent = HpBarPanel.FindChildTraverse('BarContent');
    if (BarContent == null) {
        return;
    }
    BarContent.RemoveAndDeleteChildren();
    for (const bar_type of BarTypeList) {
        const row_bar_panel = $.CreatePanel('Panel', BarContent, '');
