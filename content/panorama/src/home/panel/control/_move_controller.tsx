...
export function SetArrowKey(key: string, down_func: Function, up_func: Function) {
    const command_string = `On${key}_${Date.now()}`;
    Game.CreateCustomKeyBind(key, `+${command_string}`);
    Game.AddCommand(
        `+${command_string}`,
        () => {
...
const LoopCamera_Edgemove = () => {
    const ScreenPos = GameUI.GetCameraPosition();
    if (ScreenPos) {
        ...
        const CursorY = CursorPosition[1];
        if (ScreenX > 10000) {
            if (CursorX > 1400) {
