...
export function Init() {
    ...
    MainPanel.SetPanelEvent('ontooltiploaded', () => {
        ...
        const propData = MysteriousShopConfig[name as keyof typeof MysteriousShopConfig];

        const PropImage = $('#PropImage') as ImagePanel;
        const textrue = propData.AbilityTextureName;
        PropImage.SetImage(GetTextureSrc(textrue));

        const rarity = propData.rarity;
        for (let r = 1; r <= 7; r++) {
            MainPanel.SetHasClass('rare_' + r, rarity == r);
        }
        MainPanel.SetDialogVariable('prop_title', $.Localize(`#custom_shopitem_${name}`));

        const prop_desc = SetLabelDescriptionExtra(
