...
const ServerItemPanel = LoadCustomComponent($('#ItemBorder'), 'server_item');
...
const SetTooltipView = (item_id: string, count: number, show_count: number) => {
    ...
    const rarity = item_data.quality;
...
export function Init() {
    ...
    MainPanel.SetPanelEvent('ontooltiploaded', () => {
        const count = $.GetContextPanel().GetAttributeInt('count', 0);
        const item_id = $.GetContextPanel().GetAttributeString('item_id', '');
        const show_count = $.GetContextPanel().GetAttributeInt('show_count', 0);
