...
export const Init = () => {
    // $.Msg(["card/register"])
    const picture_count = Object.keys(PictuerFetterConfig).length;
...
const InitAllPictuerList = () => {
    ...
    for (const index in PictuerFetterConfig) {
        const suit_id = index;
        const row_data = PictuerFetterConfig[index as keyof typeof PictuerFetterConfig];
        const PictuerGroupRow = $.CreatePanel('Panel', AllPictuerList, '' + index);
        ...
        FooterToggle.SetPanelEvent('onactivate', () => {
            PictuerGroupRow.ToggleClass('ShowExtends');
        });
        const List = PictuerGroupRow.FindChildTraverse('List')!;
        // $.Msg(row_data.card_ids)
        for (const _card_id of row_data.card_ids) {
            const card_id = `${_card_id}`;
            const card_data = GetPictureCardData(`${card_id}`);
            const CardPanel = $.CreatePanel('Button', List, '' + card_id);
            // CardPanel.BLoadLayoutSnippet("Card");
            const _CardPanel = LoadCustomComponent(CardPanel, 'card_item');
...
const UpdatePictureFliter = () => {
    for (let i = 0; i < AllPictuerList.GetChildCount(); i++) {
        const PictuerGroupRow = AllPictuerList.GetChild(i)!;
        const loc_name = PictuerGroupRow.Data<PanelDataObject>().name as string;
        const text_res = loc_name.search(pic_fliter_text);
...
const GetPlayerCardList = (params: NetworkedData<CustomGameEventDeclarations['ServiceInterface_GetPlayerCardList']>) => {
    ...
    $.Schedule(0, () => {
        const card_list = Object.values(params.data.card);
        pictuer_list = params.data.pictuer_list;
        const card_object: { [card_id: string]: AM2_Server_Backpack } = {};

        for (const _card_data of card_list) {
            const card_id = '' + _card_data.item_id;
        ...
        for (let i = 0; i < AllPictuerList.GetChildCount(); i++) {
            ...
            const max_count = row_data.consume;
            let act_count = 0;
            for (let order = 0; order < List.GetChildCount(); order++) {
                ...
                if (pictuer_card_list.length > 0) {
                    const in_index = pictuer_card_list.indexOf(iCardId);
                    if (in_index != -1) {
...
const GetConfigPictuerFetter = (params: NetworkedData<CustomGameEventDeclarations['ServiceInterface_GetConfigPictuerFetter']>) => {
    ...
    const config_loc = params.data.locality['1'];
    const config_serve = params.data.server['1'];
    EquipPictureList.RemoveAndDeleteChildren();
    ...
    for (let i = 10; i < 15; i++) {
        const starPanel = StarCostInfo.GetChild(i)!;
    ...
    for (const k in config_loc) {
        ...
        if (pictuer_equip_info != null) {
            const act_count = Object.values(pictuer_equip_info).length;
    ...
    // 配置对比
    const is_same = JSON.stringify(config_loc) == JSON.stringify(config_serve);
...
const UpdatePictureAttributeList = (FooterAttributeList: Panel, act_count: number, max_count: number) => {
    for (let i = 0; i < FooterAttributeList.GetChildCount(); i++) {
        const FooterAttribute = FooterAttributeList.GetChild(i)!;
...
const SetPictureAttributeList = (FooterAttributeList: Panel, row_data: (typeof PictuerFetterConfig)[keyof typeof PictuerFetterConfig]) => {
    ...
    for (const key in ListValues) {
        const ObjectValues = ListValues[key];
        for (const attr in ObjectValues) {
            // $.Msg(["attr",attr])
            let attr_key = '' + attr;
            let attr_value = 0;
            const row_attr = ObjectValues[attr as keyof typeof ObjectValues];
            for (const sub_key in row_attr) {
                attr_key += '.' + sub_key;
                attr_value = row_attr[sub_key as keyof typeof row_attr] ?? 0;
                break;
            }
            const FooterAttribute = $.CreatePanel('Panel', FooterAttributeList, '');
    ...
    const id_list = row_data.ability_id;
    for (const aid of id_list) {
        if (aid == 'null') {
            continue;
        }
        const id_arr = aid.split('_');
        const ability_id = id_arr[0];
        const ability_level = parseInt(id_arr[1]);

        const SpecialFooterAttribute = $.CreatePanel('Panel', FooterAttributeList, '');
        ...
        const picture_data = PictuerFetterAbility[ability_id as keyof typeof PictuerFetterAbility];
