...
const SkillPopups = $('#SkillPopups');
const UppderSkillBtn = $('#UppderSkillBtn') as Button;
const PopupsButtonCancel = $('#PopupsButtonCancel') as Button;
const PopupsButtonConfirm = $('#PopupsButtonConfirm') as Button;
...
export const Init = () => {
    const BackpackCount_1292 = GameUI.CustomUIConfig().SetComponent_BackpackCount($('#BackpackCount_1292'), '1292');
    BackpackCount_1292.BackpackCount._SetLabelStyle({ font_size: 16, color: '#fffffe' });
    const BackpackCount_1293 = GameUI.CustomUIConfig().SetComponent_BackpackCount($('#BackpackCount_1293'), '1293');
    BackpackCount_1293.BackpackCount._SetLabelStyle({ font_size: 16, color: '#fffffe' });
    UppderSkillBtn.enabled = false;
    skill_sub_tree = {};
    for (const id in ServerSkillful) {
        const sub_data = ServerSkillful[id as keyof typeof ServerSkillful];
        const main_id = '' + sub_data.type;
        if (skill_sub_tree[main_id] == null) {
            skill_sub_tree[main_id] = [];
        }
    ...
    for (const id in ServerSkillExp) {
        const data = ServerSkillExp[id as keyof typeof ServerSkillExp];
        const SkillRows = $.CreatePanel('RadioButton', SkillList, id, {
            group: 'PersonalSkillGroup',
        });
        SkillRows.BLoadLayoutSnippet('SkillRows');
        SkillRows.SetHasClass('Small', data.type == 1);
        const SkillIcon = SkillRows.FindChildTraverse('SkillIcon') as ImagePanel;
        const img = data.img;
        const src = GetTextureSrc(img);
        SkillIcon.SetImage(src);

        const diff_x = data.type == 1 ? 44 : 61;
        const diff_y = data.type == 1 ? 88 / 2 : 121 / 2; // 23
    ...
    PopupsButtonConfirm.SetPanelEvent('onactivate', () => {
        OpenUpperPopups(false);
        const key_id = SkillPopups.Data<PanelDataObject>().id as string;
...
const CGED_Init = () => {
    GameEvents.Subscribe('ServiceInterface_GetPlayerServerSkillData', event => {
        ...
        for (let i = 0; i < SkillList.GetChildCount(); i++) {
            const SkillRows = SkillList.GetChild(i)!;
            const id = SkillRows.id;
            const row_main_data = main_skill_level.level[id];
...
const ViewSkillMainOfID = (id: string) => {
    ...
    const main_skill_data = ServerSkillExp[id as keyof typeof ServerSkillExp];
    const skill_src = GetTextureSrc(main_skill_data.img);
    ...
    // 读取子id
    const sub_id_list = skill_sub_tree[id];
    for (let i = 0; i < sub_id_list.length; i++) {
        const is_only = sub_id_list.length == 1;
        const is_first = i == 0 && !is_only;
        const is_last = i == sub_id_list.length - 1 && !is_only;

        const sub_id = sub_id_list[i];

        const sub_data = ServerSkillful[sub_id as keyof typeof ServerSkillful];
        const skill_level = sub_skill_level[sub_id] ? sub_skill_level[sub_id].lv : 0;

        const UpperSubSkill = $.CreatePanel('Panel', UpperSkillSub, '');
        ...
        const SubSkill = $.CreatePanel('Panel', UpperSubList, '');
        ...
        const SubUppderIcon = SubSkill.FindChildTraverse('SubUppderIcon') as ImagePanel;

        const sub_src = GetTextureSrc(sub_data.img);
        SubUppderIcon.SetImage(sub_src);
    }

    const main_data = main_skill_level.level[id];
    if (main_data == null) {
        return;
    }
    const is_adv = main_data.is_adv == 1;
    ...
    let progress_pct = 0;
    const diff_value = main_data.level_exp - main_data.cur_exp;

    const diff_cost = is_adv ? diff_value : Math.ceil(diff_value / 1000);
...
const OpenUpperPopups = (show: boolean) => {
    ...
    const need_item = UppderSkillBtn.Data<PanelDataObject>().need_item as { [item_id: number]: number };
    for (const item_id in need_item) {
