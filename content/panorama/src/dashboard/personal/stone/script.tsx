...
export function Init() {
    ...
    for (let i = 1; i <= 6; i++) {
        const EquipStoneRows = $.CreatePanel('Panel', EquipStoneList, `${i}`);
...
function InitSubscribe() {
    GameEvents.Subscribe('ServiceSoul_GetPlayerServerSoulData', event => {
        ...
        stone_attr_object = {};
        const equip_data = data.list.i;
        for (const id in equip_data) {
            // $.Msg([id,equip_data[id]])
            const _data = equip_data[id];
            const EquipStoneRows = EquipStoneList.FindChildTraverse(`${id}`)!;
            EquipStoneRows.SetDialogVariableInt('level', _data.z);
            const attr_obj = _data.d;
            const attr_label: string[] = [];

            for (const k in attr_obj) {
                const row_data = attr_obj[k];
                const key = row_data.k;
                const attr_data = server_soul_attr[key as keyof typeof server_soul_attr];
                const MainProperty = attr_data.MainProperty;
                const TypeProperty = attr_data.TypeProperty;
                const attr_name = `${$.Localize(`#custom_attribute_${MainProperty}`).replace('%', '')}`;
                const num_fixed = attr_data.float;
                const attr_value = parseFloat(row_data.v.toFixed(num_fixed));
        ...
        for (const attr_main in stone_attr_object) {
            const attr_row_obj = stone_attr_object[attr_main];
            for (const attr_type in attr_row_obj) {
                const value = parseFloat(attr_row_obj[attr_type].toFixed(2));
                const _Panel = $.CreatePanel('Panel', AttributeList, '', {
                ...
                const is_pct = CheckAttrIsPercent(attr_main, attr_type);
