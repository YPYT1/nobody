...
export function Init() {
    ...
    //初始化基础魂石列表
    g_slot_attr_index = {};
    const temp_object: { [id: string]: number } = {};
    for (const index in server_soul_attr) {
        const row_data = server_soul_attr[index as keyof typeof server_soul_attr];
        const item_list = row_data.item_id;
        const item_id = '' + item_list[0];
        if (temp_object[item_id] == null) {
            temp_object[item_id] = 1;
        }
        const type_id = '' + row_data.box_type;
        if (g_slot_attr_index[type_id] == null) {
            g_slot_attr_index[type_id] = {};
        }
    ...
    for (let i = 1; i <= 6; i++) {
        const SoulEquipBorder = $.CreatePanel('RadioButton', SoulEquipList, `${i}`, {
    ...
    for (const level of STONE_UNLOCK_LEVEL) {
        const AttributeRows = $.CreatePanel('Panel', SSAttributeList, '');
    ...
    GameEvents.Subscribe('ServiceSoul_GetPlayerServerSoulData', event => {
        ...
        const data_object = data.list.i;
        for (const id in data_object) {
            const _data = data_object[id];
            const rowPanel = SoulEquipList.FindChildTraverse(id);
            if (rowPanel) {
...
function InitAddStoneBtnList() {
    for (const item_id of g_base_stone_ids) {
        const ItemStoneAttrRows = $.CreatePanel('RadioButton', AttributreAddList, item_id, {
        ...
        ItemStoneAttrRows.SetPanelEvent('onactivate', () => {
            // 魂石附加功能
            const key = ItemStoneAttrRows.Data<PanelDataObject>().key;
    ...
    GameEvents.Subscribe('ServiceInterface_GetPlayerServerPackageData', event => {
        ...
        const ids_backpack: { [id: string]: AM2_Server_Backpack } = {};
        ...
        for (const item_id of g_base_stone_ids) {
            const item_data = ids_backpack[item_id];
            if (item_data == null) {
                continue;
            }
            const count = item_data.number;
            if (count <= 0) {
                continue;
            }
...
function InitButton() {
    ...
    PopupsOptionDeltetBtn.SetPanelEvent('onactivate', () => {
        // 删除的词条
        const soul_obj = ServerSoulData.i;
        const data_list = Object.values(soul_obj[g_select_slot].d);
        const del_attr = data_list[g_action_attr_index];
    ...
    AddExtendItemBtn.SetPanelEvent('onactivate', () => {
        ...
        for (let i = 0; i < ShowExtendItemList.GetChildCount(); i++) {
            const itemPanel = ShowExtendItemList.GetChild(i)!;
            const serverItem = itemPanel.GetChild(0) as Component_ServerItem;
            serverItem._UpdateCount();
            const count = serverItem._GetCount();
    ...
    for (const item_id of ss_upgrade_protect) {
        const e = $.CreatePanel('Button', ShowExtendItemList, '', {
            class: 'ServerItemBorder',
        });
        const ItemPanel = CreateCustomComponent(e, 'server_item', `${item_id}`);
        ItemPanel._SetServerItemInfo({ item_id: item_id, show_count: true, show_tips: true });
        ItemPanel.SetPanelEvent('onactivate', () => {
            const count = ItemPanel._GetCount();
            if (count <= 0) {
                return;
            }
            ...
            // 更新成功率
            const item_success = ITEM_SUCCESS_RATIO[`${item_id}`] ?? 0;
            const final_succes = Math.min(100, item_success + g_base_success);
...
function InitExcelData() {
    ...
    for (const key in server_soul_config) {
        const data = server_soul_config[key as keyof typeof server_soul_config];
        if (data.type == 1) {
...
function SetSelectSlot_Attr(slot: number) {
    ...
    for (let i = 0; i < SSAttributeList.GetChildCount(); i++) {
        const rowPanel = SSAttributeList.GetChild(i)!;
        const need_level = STONE_UNLOCK_LEVEL[i];
    ...
    const haved_key_list: string[] = [];
    if (ServerSoulData != null) {
        const SSRowData = ServerSoulData.i[`${slot}`].d;
        const SSRowLen = Object.keys(SSRowData).length;
        let attr_index = 0;
        for (const _k in SSRowData) {
            const data = SSRowData[_k];
            const key = data.k;
            const level = data.l;
            if (haved_key_list.indexOf(key) == -1) {
                haved_key_list.push(key);
            }
            ...
            const num_fixed = attr_data.float;
            const attr_name = `${$.Localize(`#custom_attribute_${MainProperty}`).replace('%', '')}`;
            const attr_value = parseFloat(data.v.toFixed(num_fixed));
            ...
            const rowPanel = SSAttributeList.GetChild(attr_index)!;
...
function SetSelectSlot_Add(slot: number, haved_key_list: string[]) {
    const slot_attr_ids = g_slot_attr_index[slot];
    // $.Msg(slot_attr_ids);
    const is_limit = haved_key_list.length >= STONE_ADD_LIMIT;
    for (let i = 0; i < AttributreAddList.GetChildCount(); i++) {
        const RowPanel = AttributreAddList.GetChild(i)!;
        const panel_id = RowPanel.id;
        const attr_index = slot_attr_ids[panel_id];
        ...
        if (attr_index != null) {
            ...
            const row_data = server_soul_attr[attr_index as keyof typeof server_soul_attr];
            const up_value = row_data.up_value.split('-');
            const value1 = up_value[0];
            const value2 = up_value[1];
            const pct_symbol = CheckAttrIsPercent(row_data.MainProperty as AttributeMainKey, row_data.TypeProperty as AttributeSubKey) ? '%' : '';
...
function SetSelectAttrRowsAttr(e: Panel, key: string, is_up: boolean, target_level: number, data: CGEDGetSoulListData, attr_index: number) {
    ...
    let per = 100;
    if (is_up) {
        const level = target_level - 1;
        if (level > 0) {
            // 1-4
            const value_per = attr_data.value_per_1_5;
            if (level >= 4) {
                per += value_per * 4;
            } else {
                per += value_per * level;
        ...
        if (level > 4) {
            //5-9
            const value_per = attr_data.value_per_6_10;
            if (level >= 9) {
                per += value_per * 5;
            } else {
        ...
        if (level > 9) {
            // 10-14
            const value_per = attr_data.value_per_11_15;
            if (per >= 14) {
                per += value_per * 5;
            } else {
        ...
        if (level > 14) {
            // 15-19
            const value_per = attr_data.value_per_16_20;
            if (per >= 19) {
                per += value_per * 5;
            } else {
    ...
    // 属性降级
    const SSAttributeDownRow = $.CreatePanel('RadioButton', e, '', {
        group: 'StoneActionGroup',
    });
    const pct_symbol = CheckAttrIsPercent(MainProperty, TypeProperty) ? '%' : '';
    ...
    if (is_up) {
        ...
        const value_arr = attr_data.up_value.split('-');
        ...
        const value_arr = attr_data.drop_value.split('-');
        value1 = (parseFloat(value_arr[0]) * (per * 0.01)).toFixed(num_fixed);
        value2 = (parseFloat(value_arr[1]) * (per * 0.01)).toFixed(num_fixed);
    }
...
function StoneOption_Upper(key: string, level: number) {
    g_confirm_action = 'Upper';
    const config_key = level_u_d_config.up[level];
    g_soul_attr_key = config_key;
    const up_config = server_soul_config[config_key as keyof typeof server_soul_config];
...
function StoneOption_Down(key: string, level: number) {
    ...
    const config_key = level_u_d_config.drop[level];
    g_soul_attr_key = config_key;
    const up_config = server_soul_config[config_key as keyof typeof server_soul_config];
    // 成功率
...
function StoneOption_Add(slot: number, key: string) {
    ...
    const slot_item_list = g_slot_attr_index['' + slot];
    g_soul_attr_key = key;
    const up_config = server_soul_config['1'];
    SetSSActionResultsInfo(key, up_config);
}
...
function SetSSActionResultsInfo(key: string, up_config: SoulConfigType, is_up: boolean = true) {
    ...
    const soul_attr = server_soul_attr[key as keyof typeof server_soul_attr];
    const attr_items = soul_attr.item_id;
    // 成功率
    g_base_success = up_config.pro;
    SSActionResults.SetDialogVariableInt('success', up_config.pro);

    // 消耗素材
    const items = up_config.items;
    const price_cost: { item: string; count: number }[] = [];
    for (const item of items) {
        if (item == 'null') {
            continue;
        }
        const item_arr = item.split('_');
        const temp_obj = { item: item_arr[0], count: parseInt(item_arr[1]) };
    ...
    for (const item_info of price_cost) {
        const costPanel = $.CreatePanel('Panel', ActionCostItemList, '');
        costPanel.BLoadLayoutSnippet('CostPanel');
        const PriceIcon = costPanel.FindChildTraverse('PriceIcon')!;
        const ServerItemPanel = LoadCustomComponent(PriceIcon, 'server_item');
        ServerItemPanel._SetServerItemInfo({ item_id: item_info.item, hide_bg: true });

        const self_cost_cont = currency_count != null ? currency_count[item_info.item] ?? 0 : 0;
    ...
    // 消耗魂石或者卷轴
    const consume = up_config.consume.split('_').map((v, k) => {
        return parseInt(v);
    });
    const ss_index = consume[0];
    const ss_count = consume[1];
    let item_id = '';
    if (is_up) {
    ...
    const owned_count = backpack_count_table ? backpack_count_table[item_id] ?? 0 : 0;
