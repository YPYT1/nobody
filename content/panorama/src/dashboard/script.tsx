...
const CreateMenuButtons = () => {
    ...
    // SettingsButton
    const SettingsButton = $.CreatePanel('Button', DashboardButtonList, 'settingsButton', {
    ...
    $.Each(DASHBOARD_NAVBAR_LIST, (data, i) => {
        ...
        if (row_data.Show) {
            const DashboardButton = $.CreatePanel('Button', DashboardButtonList, dashboard_id + 'Button', {
                class: 'DashboardButton',
            });
...
const RegisterKeyEscDashBoard = (e: Panel) => {
    $.RegisterKeyBind(e, 'key_Escape', () => {
        ...
        for (const id in DASHBOARD_NAVBAR) {
            const row_board = DashboardList.FindChildTraverse(id);
            const row_button = DashboardButtonList.FindChildTraverse(id + 'Button');
...
function GetServerTime() {
    GameEvents.Subscribe('ServiceInterface_GetServerTime', event => {
        ...
        const time_string = event.data.time_string;
        GameUI.CustomUIConfig().setStorage('unix_time', data.time);
        const unix_date = new Date(Date.UTC(time_string.yy, time_string.mm - 1, time_string.dd, 0, 0, 0));
        const today_time = Math.floor(unix_date.getTime() / 1000) - 28800;
...
function DashboardRoute<Key extends keyof typeof DASHBOARD_NAVBAR, T2 extends (typeof DASHBOARD_NAVBAR)[Key]>(
    dashboard_id: Key,
    nav: keyof T2['Sub']
) {
