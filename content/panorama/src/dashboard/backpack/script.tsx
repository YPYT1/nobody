...
const CGE_Subscribe = () => {
    GameEvents.Subscribe('ServiceInterface_GetPlayerServerPackageData', event => {
        ...
        const backpack_count_table: { [item_id: string]: number } = {};
        ...
        for (const ItemData of ItemList) {
            ...
            const item_class = item_data.affiliation_class;
            if (item_class == 23) {
                continue;
            }
            const ItemRadio = $.CreatePanel('RadioButton', BackpackItemList, '', { group: 'backpack_group' });
            ItemRadio.BLoadLayoutSnippet('BackpackItem');
            const ServerItem = ItemRadio.FindChildTraverse('ServerItem')!;
            const ItemPanel = LoadCustomComponent(ServerItem, 'server_item');
            ...
            ItemRadio.SetPanelEvent('onactivate', () => {
                const amount = ItemRadio.Data<PanelDataObject>().amount as number;
...
const InitAllBackpackItemList = () => {
    BackpackItemList.RemoveAndDeleteChildren();
    for (const item_id in ServerItemList) {
        ...
        const ServerItemImage = ItemRadio.FindChildTraverse('ServerItemImage') as ImagePanel;
        const texture_name = data.AbilityTextureName;
...
const InitItemDetails = () => {
    ...
    UseBackpackItemBtn.SetPanelEvent('onactivate', () => {
        const text = UseBackpackCount.text.length <= 0 ? '1' : UseBackpackCount.text;
        const count = Math.max(1, parseInt(text));
    ...
    UseBackpackCount.SetPanelEvent('ontextentrychange', () => {
        const text = UseBackpackCount.text;
        if (text.indexOf('-') != -1) {
...
const ViewItem = (item_id: string, count: number) => {
    ...
    const item_data = ServerItemList[item_id as keyof typeof ServerItemList];
    const item_rare = item_data.quality;
    // let item_name = $.Localize("#custom_serveritem_" + item_id)
    const item_desc = $.Localize('#custom_serveritem_' + item_id + '_desc');
    ...
    const can_use = item_data.uses == 1;
