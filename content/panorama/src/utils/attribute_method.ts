...
export const GetItemAttribute = (item_name: string) => {
    ...
    if (item_data.AttributeValues) {
        const AttributeValues = item_data.AttributeValues as CustomAttributeTableType;
 ...
 * @param level
 */
export const ConvertAttributeValues = (keyValue: CustomAttributeTableType, level: number = 1, split_str: string = '<br>', multiple: number = 1) => {
    const attr_list: string[] = [];
    for (const key in AttributeConst) {
        const main_key = key as AttributeMainKey;
        const main_table = keyValue[main_key];
        if (main_table != null) {
            for (const sub_key in main_table) {
                const sub_value = main_table[sub_key as keyof typeof main_table];
                if (sub_value) {
                    const row_key = `${main_key}_${sub_key}`;
                    const row_text = FormatKeyValueToText(main_key, sub_key, sub_value);
...
const pre_sub_key = ['Base', 'Bonus', 'Fixed', 'PreLvBase', 'PreLvBonus', 'PreLvFixed'];

const percent_sub_key = ['BasePercent', 'BonusPercent', 'TotalPercent'];

export const FormatKeyValueToText = (
    main_key: string,
    sub_key: string,
    v: string | number,
    level: number = 1,
    multiple: number = 1,
    extra_ratio: number = 0
) => {
    let value = 0;
    if (typeof v == 'number') {
        value = v;
    ...
    const character = is_negative ? '-' : '+';

    value = Math.abs(value);
    const attr_label_main = $.Localize('#custom_attribute_' + main_key);
    const attr_label_sub = $.Localize('#custom_attribute_sub_' + sub_key);
    const percent_main = attr_label_main.search(/%/g);
    const percent_sub = attr_label_sub.search(/%/g);
    let attr_text = '';
    ...
    const is_percent = percent_sub_key.indexOf(sub_key) != -1;
    const p = attr_text.search(/%/g);
    let symbol = '';
    let attr_value = value * multiple;
    // 这里处理过长的百分比
    const attr_str = String(attr_value);
    ...
    if (extra_ratio > 0) {
        const extra_value = (value * extra_ratio).toFixed(2);
        let pct_str = '';
        if (p == 0) {
            pct_str = '%';
        }
        const extra_text = `<span class="extra_val">(+${extra_value}${pct_str})</span>`;
        text += extra_text;
    }
