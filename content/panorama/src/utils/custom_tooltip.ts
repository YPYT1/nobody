export function ShowCustomTextTooltip(panel: Panel, title: string = '', description: string = '') {
    const post_args = `title=${title}&description=${description}&tooltip=text`;

    for (const k in panel.Data<PanelDataObject>()) {
        const value = panel.Data<PanelDataObject>()[k];
...
export function ShowCustomTooltip(
    ...
    if (tooltip_type == 'ability') {
        ...
        $.DispatchEvent(
            ...
            'name=' + name + '&item_level=' + item_level + '&entityIndex=' + entityIndex
        );
