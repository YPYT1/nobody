...
export const FormatIntToString = (value: number) => {
    ...
    const value_len = value_str.length; // 13/ 4 = 3
    if (value_len < 9) {
        return value_str;
    }
    //push()
    const strlist: string[] = [];
    const cut_count = Math.ceil(value_len / 4);
    const y = value_len % 4;
...
export function FormatDescription(
    ...
    for (const key in AbilityValues) {
        ...
        const special_key = AbilityValues[key];
        let special_num: number[] = [];
        if (typeof special_key == 'string') {
            const _arr = special_key.split(' ').map((v, k) => {
                return parseFloat(v);
            });
            special_num = _arr;
        } else {
        ...
        const is_percent = original_description_txt.indexOf(`%${key}%%%`) != -1;
        // $.Msg(["is_percent", is_percent, key])
        if (show_all) {
            const special_value: string[] = [];
            ...
            for (let i = 1; i <= special_num.length; i++) {
                ...
                const is_negative = special_num[i - 1] < 0;
                const value = Math.abs(special_num[i - 1]);
                const col_value = `<span class="${class_name} ${is_negative ? 'is_negative' : ''}">${value}${is_percent ? '%' : ''}</span>`;
            ...
            const is_negative = special_num[_curr_level - 1] < 0;
            const value = Math.abs(special_num[_curr_level - 1]);
            const col_value = `<span class="GameplayVariable Current ${is_negative ? 'is_negative' : ''}">${value}${is_percent ? '%' : ''}</span>`;
            original_description_txt = original_description_txt.replaceAll(`%${key}%%%`, col_value);
            original_description_txt = original_description_txt.replaceAll(`%${key}%`, col_value);
        }
    }
    original_description_txt = original_description_txt.replaceAll('\n', '<br>');
...
export function GetUnitModifierStack(unit: EntityIndex, modifier_name: string) {
    const buff_count = Entities.GetNumBuffs(unit);
    for (let i = 0; i < buff_count; i++) {
        const buff_id = Entities.GetBuff(unit, i);
        const buff_name = Buffs.GetName(unit, buff_id);
...
export function FormatDescriptionExtra(
    ...
    for (const key in ObjectValues) {
        for (const sub_key in ObjectValues[key]) {
            const special_key = ObjectValues[key][sub_key];
            let special_num: number[] = [];
            if (typeof special_key == 'string') {
                const _arr = special_key.split(' ').map((v, k) => {
                    return parseFloat(v);
            ...
            const is_percent = description_txt.indexOf(`%${key}.${sub_key}%%%`) != -1;
            // $.Msg(["is_percent", is_percent, key])
            if (show_all) {
                const special_value: string[] = [];
                ...
                for (let i = 1; i <= special_num.length; i++) {
                    ...
                    const value = special_num[i - 1];
                    const is_negative = value < 0;
                    const col_value = `<span class="${class_name} ${is_negative ? 'is_negative' : ''}">${Math.abs(value)}${
                        is_percent ? '%' : ''
                    }</span>`;
                    special_value.push(col_value);
                }
                ...
                const value = special_num[curr_level - 1] * ObjectPercent * 0.01;
                // $.Msg(["value",value])
                const is_negative = value < 0;
                const col_value = `<span class="GameplayVariable Current ${is_negative ? 'is_negative' : ''}">${Math.abs(value)}${
                    is_percent ? '%' : ''
                }</span>`;
                description_txt = description_txt.replaceAll(`%${key}.${sub_key}%%%`, col_value);
                description_txt = description_txt.replaceAll(`%${key}.${sub_key}%`, col_value);
            }
        }
    }
